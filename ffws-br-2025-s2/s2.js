(() => {
  ...
  const CONFIG = window.FFWS_BR_2025_S2_CONFIG || {
    teamsUrl: 'ffws-br-2025-s2/teams.json',
    stagesUrl: 'ffws-br-2025-s2/stages.json',
    playersUrl: 'ffws-br-2025-s2/players.json',
    datesUrl: 'ffws-br-2025-s2/dates.json'
  };
  const PAGE_PREFIX = 'ffws-br-2025-s2-';
  const PAGE_IDS = new Set([
    'ffws-br-2025-s2-classificatoria', 'ffws-br-2025-s2-contra-squad', 'ffws-br-2025-s2-final',
    'ffws-br-2025-s2-mvp', 'ffws-br-2025-s2-equipes', 'ffws-br-2025-s2-datas', 'ffws-br-2025-s2-selecoes',
    'ffws-br-2025-s2-stats', 'ffws-br-2025-s2-notas', 'ffws-br-2025-s2-comparar'
  ]);
  const state = {
    loading: null,
    loaded: false,
  ...
  function hero(title, subtitle) {
    return `<section class="ffws-s2-hero">
      <div class="ffws-s2-kicker">FFWS BRASIL 2025 SPLIT 2</div>
    ...
    if (stageKey === 'classificatoria') {
      return `<section class="ffws-s2-format">
        ...
        <div class="ffws-s2-format-grid">
          <div class="ffws-s2-format-card"><small>Equipes</small><strong>16</strong><span>Divididas em quatro grupos</span></div>
          <div class="ffws-s2-format-card"><small>Semanas</small><strong>6</strong><span>Battle Royale aos sábados e domingos</span></div>
          <div class="ffws-s2-format-card"><small>Dias BR</small><strong>12</strong><span>Três grupos por dia</span></div>
          <div class="ffws-s2-format-card"><small>Avançam</small><strong>12</strong><span>Classificados à reta final</span></div>
        </div>
        <div class="ffws-s2-legend"><span class="advance"><i></i>1º ao 12º • Classificados</span><span class="relegated"><i></i>13º ao 16º • Eliminados</span></div>
      </section>`;
    }
    return `<section class="ffws-s2-format"><div><h2>${stageKey === 'segundaFase' ? 'Contra Squad' : 'Final'}</h2><p>${escapeHtml(format.description || '')}</p></div></section>`;
  }
  ...
  function stageFilters(stageKey) {
    const total = stageKey === 'classificatoria' ? 12 : (stageKey === 'segundaFase' ? 6 : 2);
    ...
    return `<div class="ffws-s2-filters">
      <label class="ffws-s2-filter"><span>${label}:</span><select onchange="setFFWS25S2StageFilter('${stageKey}','period',this.value)"><option value="all">Geral</option>${Array.from({ length: total }, (_, index) => `<option value="${index + 1}"${String(selected.period) === String(index + 1) ? ' selected' : ''}>${label} ${index + 1}</option>`).join('')}</select></label>
      <label class="ffws-s2-filter"><span>Mapa:</span><select onchange="setFFWS25S2StageFilter('${stageKey}','map',this.value)"${maps.length ? '' : ' disabled title="Disponível após o início da etapa"'}><option value="all">Todos os mapas</option>${maps.map(map => `<option value="${escapeHtml(map)}"${normalize(selected.map) === normalize(map) ? ' selected' : ''}>${escapeHtml(map)}</option>`).join('')}</select></label>
      <label class="ffws-s2-filter"><span>Queda:</span><select onchange="setFFWS25S2StageFilter('${stageKey}','drop',this.value)"${drops.length ? '' : ' disabled title="Disponível após o início da etapa"'}><option value="all">Todas as quedas</option>${drops.map(drop => `<option value="${drop}"${String(selected.drop) === String(drop) ? ' selected' : ''}>Queda ${drop}</option>`).join('')}</select></label>
  ...
  function renderClassificatoria(rootId) {
    ...
    layoutApi.renderClassification({
      ...
      filters: {
        period: {
          ...
          onchange: "setFFWS25S2StageFilter('classificatoria','period',this.value)", selected: selected.period,
        ...
        map: {
          ...
          onchange: "setFFWS25S2StageFilter('classificatoria','map',this.value)", selected: selected.map,
          options: [{ value: 'all', label: 'Todos os Mapas' }].concat(mapOptions)
        },
        drop: {
          containerId: 'ffws25-s2-drop-filter-container', id: 'ffws25-s2-filter-drop', label: 'Queda:',
          onchange: "setFFWS25S2StageFilter('classificatoria','drop',this.value)", selected: selected.period === 'all' ? 'all' : selected.drop,
          visible: true,
    ...
    const dropContainer = root.querySelector('#ffws25-s2-drop-filter-container');
    const mapLabel = filterRow?.querySelector('.ffws-season-map-label');
    const dropLabel = dropContainer?.querySelector('label');
    const dropSelect = root.querySelector('#ffws25-s2-filter-drop');
  ...
  function multiFilter(key, title, options) {
    ...
    return `<div class="ffws-s2-filter"><span>${escapeHtml(title)}:</span><div class="ffws-s2-multi" data-s2-multi="${escapeHtml(key)}">
      <button type="button" onclick="toggleFFWS25S2Multi('${key}')"><b>${escapeHtml(label)}</b><span>⌄</span></button>
      <div class="ffws25-s2-multi-menu" id="ffws25-s2-multi-${escapeHtml(key)}" hidden>
        <label><input type="checkbox" ${selected.length === 0 ? 'checked' : ''} onchange="clearFFWS25S2Multi('${key}')"> Todos</label>
        ${options.map(option => `<label><input type="checkbox" value="${escapeHtml(option.value)}" ${selected.includes(String(option.value)) ? 'checked' : ''} onchange="setFFWS25S2Multi('${key}',this.value,this.checked)"> ${option.flag ? `<span>${escapeHtml(option.flag)}</span>` : ''}${escapeHtml(option.label)}</label>`).join('')}
      </div>
    </div></div>`;
  }
  ...
  function playerFilterOptions() {
    ...
    return {
      stage: [{ value: 'classificatoria', label: 'Classificatória' }, { value: 'segundaFase', label: 'Contra Squad' }, { value: 'final', label: 'Final' }],
  ...
  function renderMvp() {
    const root = document.getElementById('ffws-br-2025-s2-mvp-content');
    if (!root) return;
    ...
    const pager = rows.length > pageSize ? `<div class="ffws-s2-mvp-pager">
      <button type="button" aria-label="Top 10 anterior" ${state.mvpPage <= 0 ? 'disabled' : ''} onclick="setFFWS25S2MvpPage(${state.mvpPage - 1})">‹</button>
      <span>Top ${pageStart + 1}–${pageEnd} de ${rows.length}</span>
      <button type="button" aria-label="Próximo Top 10" ${state.mvpPage >= pageCount - 1 ? 'disabled' : ''} onclick="setFFWS25S2MvpPage(${state.mvpPage + 1})">›</button>
    </div>` : '';
    root.innerHTML = `<div class="ffws-s2-shell">${hero('Ranking MVP', 'Classificação individual da WB 2025 S2')}
  ...
  function renderTeams() {
    const root = document.getElementById('ffws-br-2025-s2-equipes-content');
    if (!root) return;
    const totalPlayers = currentRosterPlayers().length;
    const totalRookies = 0;
    root.innerHTML = `<div class="ffws-s2-shell">${hero('Equipes', 'As 16 organizações e os elencos da WB 2025 S2')}
      <section class="ffws-s2-panel"><div class="ffws-s2-panel-inner"><div class="ffws-s2-panel-head"><div><h2>Diretório de Equipes</h2><p>Confira os elencos cadastrados da temporada.</p></div><span class="ffws-s2-badge">${totalPlayers} jogadores</span></div>
      <div class="ffws-s2-teams-grid ffws-s2-rosters-grid">${state.teams.map(team => {
        ...
        return `<article class="ffws-s2-team-card ffws-s2-team-roster-card" role="button" tabindex="0" onclick="openCurrentSeasonTeam('${jsAttr(team.name)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openCurrentSeasonTeam('${jsAttr(team.name)}')}">
          <header class="ffws-s2-team-roster-head"><img loading="lazy" decoding="async" src="${escapeHtml(logo(team.name))}" alt="${escapeHtml(team.name)}" onerror="this.onerror=null;this.src='escudo.webp'"><div><strong>${escapeHtml(team.name)}</strong><small>${escapeHtml(team.abbreviation)} • Brasil</small><small class="${team.logoPending ? 'ffws-s2-logo-pending' : ''}">${roster.length} jogadores cadastrados</small></div></header>
  ...
  function scheduleStageMeta(stageKey) {
    const map = {
      classificatoria: { title: 'Classificatória • Battle Royale', description: '12 dias de Battle Royale em seis semanas. Em cada dia, três dos quatro grupos entram em jogo.', badge: '12 dias' },
      segundaFase: { title: 'Contra Squad', description: 'Sextas-feiras da fase classificatória. Os resultados detalhados serão adicionados depois.', badge: '6 semanas' },
      final: { title: 'Reta Final', description: 'Point Rush Battle Royale e Grande Final.', badge: '2 dias' }
    };
        ...
        <div class="ffws-s2-calendar-list">
          ${rows.map(item => {
            ...
            const detail = stageKey === 'classificatoria'
              ? '<span class="ffws-s2-calendar-rest"><b>Formato:</b> Battle Royale</span>'
              : stageKey === 'segundaFase'
                ? '<span class="ffws-s2-calendar-rest"><b>Formato:</b> Contra Squad</span>'
                : '<span class="ffws-s2-calendar-rest"><b>Formato:</b> Point Rush / Final</span>';
  ...
  function renderDates() {
    const root = document.getElementById('ffws-br-2025-s2-datas-content');
    if (!root) return;
    ...
    root.innerHTML = `<div class="ffws-s2-shell">${hero('Datas', 'Calendário oficial da WB 2025 S2')}
  ...
  function selectionTabsHtml() {
    ...
    return Object.entries(S2_SELECTION_PHASES).map(([key, config]) => {
      ...
      return `<button type="button" class="season-selection-tab ${active ? 'active' : ''} ${unlocked ? '' : 'locked'}" style="--selection-color:${config.color}" ${unlocked ? `onclick="setFFWS25S2SelectionTab('${key}')"` : 'disabled aria-disabled="true"'}>
  ...
  function renderSelections() {
    const root = document.getElementById('ffws-br-2025-s2-selecoes-content');
    if (!root) return;
    ...
    if (phaseKey === 'semanal') {
      ...
      const phaseFilter = `<div class="season-selection-week-stage-filter"><span>Fase:</span><button type="button" class="active">Classificatória</button><button type="button" disabled aria-disabled="true">Contra Squad <small>EM BREVE</small></button></div>`;
      const weekButtons = allWeeks.map(key => {
        const unlocked = availableWeeks.includes(key);
        const active = unlocked && week === key;
        return `<button type="button" class="btn-day season-selection-week-btn ${active ? 'active' : ''} ${unlocked ? '' : 'is-locked'}" ${unlocked ? `onclick="setFFWS25S2SelectionWeek('${key}')"` : 'disabled aria-disabled="true"'} style="${active ? 'background:#ff0000;border-color:#ff0000;color:#fff;' : ''}">SEMANA ${key}${unlocked ? '' : '<small>EM BREVE</small>'}</button>`;
      }).join('');
      ...
      description = 'A seleção da Contra Squad será liberada quando essa etapa começar.';
    ...
    root.innerHTML = `<div class="ffws-s2-selection-page">
      <div class="season-selection-hero">
        <div class="season-selection-kicker">WB 2025 S2</div>
  ...
  function statsMultiFilter(key, title, options) {
    ...
    return `<div class="ffws-s2-filter"><span>${escapeHtml(title)}:</span><div class="ffws-s2-multi" data-s2-multi="stats-${escapeHtml(key)}"><button type="button" onclick="toggleFFWS25S2StatsMulti('${key}')"><b>${escapeHtml(label)}</b><span>⌄</span></button><div class="ffws25-s2-multi-menu" id="ffws25-s2-stats-multi-${escapeHtml(key)}" hidden><label><input type="checkbox" ${selected.length === 0 ? 'checked' : ''} onchange="clearFFWS25S2StatsMulti('${key}')"> Todos</label>${options.map(option => `<label><input type="checkbox" value="${escapeHtml(option.value)}" ${selected.includes(String(option.value)) ? 'checked' : ''} onchange="setFFWS25S2StatsMulti('${key}',this.value,this.checked)"> ${escapeHtml(option.label)}</label>`).join('')}</div></div></div>`;
  }
  ...
  function s2StatsPageState() {
    if (!window.__ffws25S2StatsPages) window.__ffws25S2StatsPages = {};
    return window.__ffws25S2StatsPages;
  }
  ...
  function s2StatsPaginatedList(key, rows, renderRow, headerHtml, pageSize = 4) {
    ...
    const prevButton = page === 0
      ...
      : `<button onclick="event.stopPropagation(); ffws25S2StatsPageNav('${key}', -1)" style="background:rgba(255,255,255,0.07); border:1px solid #333; color:#aaa; border-radius:50%; width:26px; height:26px; cursor:pointer; font-size:1em; display:flex; align-items:center; justify-content:center;">&#8249;</button>`;
    const nextButton = page >= totalPages - 1
      ? `<button onclick="event.stopPropagation(); ffws25S2StatsPageNav('${key}', 'reset')" title="Voltar ao início" style="background:rgba(255,255,255,0.07); border:1px solid #333; color:var(--accent); border-radius:50%; width:26px; height:26px; cursor:pointer; font-size:0.85em; display:flex; align-items:center; justify-content:center;">&#8635;</button>`
      : `<button onclick="event.stopPropagation(); ffws25S2StatsPageNav('${key}', 1)" style="background:rgba(255,255,255,0.07); border:1px solid #333; color:#aaa; border-radius:50%; width:26px; height:26px; cursor:pointer; font-size:1em; display:flex; align-items:center; justify-content:center;">&#8250;</button>`;
  ...
  function s2StatsStageLabel(stage) {
    return stage === 'classificatoria' ? 'Classificatória' : stage === 'segundaFase' ? 'Contra Squad' : stage === 'final' ? 'Final' : String(stage || '');
  }
  ...
  function s2DropReportCache() {
    if (!window.__ffws25S2DropReportCache) window.__ffws25S2DropReportCache = { drops: new Map(), days: new Map(), context: {} };
    const cache = window.__ffws25S2DropReportCache;
  ...
  function s2DropReportHighlightCard(label, row, metric, formatter, rank = 0, isTeam = false) {
    ...
    const team = row.team || '';
    const action = isTeam ? `showFFWS25S2DropReportTeam('${jsAttr(team)}')` : `openCurrentSeasonPlayer('${jsAttr(name)}','${jsAttr(team)}')`;
  ...
  function s2DropReportBooyahHtml(report) {
    if (report.mode === 'drop') {
      ...
      if (!booyah) return '';
      return `<button type="button" class="ffws-s2-drop-booyah" onclick="showFFWS25S2DropReportTeam('${jsAttr(booyah.team)}')"><small>BOOYAH</small><span><img src="${escapeHtml(logo(booyah.team))}" onerror="this.onerror=null;this.src='escudo.webp'"><strong>${escapeHtml(booyah.team)}</strong></span></button>`;
    }
    ...
    return `<div class="ffws-s2-day-booyahs"><small>BOOYAHS DO DIA</small><div>${rows.map(row => `<button type="button" onclick="showFFWS25S2DropReportTeam('${jsAttr(row.team)}')"><img src="${escapeHtml(logo(row.team))}" onerror="this.onerror=null;this.src='escudo.webp'"><span>${escapeHtml(abbreviation(row.team))}</span><b>${row.count}</b></button>`).join('')}</div></div>`;
  }
  ...
  function s2DropReportTeamsHtml(report) {
    ...
    return `<div class="ffws-s2-drop-report-list-page">${detail}<div class="ffws-s2-drop-report-toolbar"><div><strong>Relatório das equipes</strong><span>${isDay ? 'A posição à esquerda é o ranking acumulado do dia por pontos.' : 'O número à esquerda é a posição real da equipe na queda.'}</span></div><label>Ordenar por <select onchange="setFFWS25S2DropReportTeamSort(this.value)">${options.map(([value,label]) => `<option value="${value}"${sortKey===value?' selected':''}>${label}</option>`).join('')}</select></label></div>
      ...
      <div class="ffws-s2-drop-team-list">${rows.map(row => `<div class="ffws-s2-drop-team-row place-${Math.min(row.position || 12,12)}">
        ...
        <button type="button" class="ffws-s2-drop-team-name" onclick="showFFWS25S2DropReportTeam('${jsAttr(row.team)}')"><img src="${escapeHtml(logo(row.team))}" onerror="this.onerror=null;this.src='escudo.webp'"><span><strong>${escapeHtml(abbreviation(row.team))}</strong><small>${escapeHtml(row.team)}</small></span></button>
  ...
  function s2DropReportPlayersHtml(report) {
    ...
    return `<div class="ffws-s2-drop-report-list-page"><div class="ffws-s2-drop-report-toolbar"><div><strong>Relatório dos jogadores</strong><span>${rows.length}${selectedTeams.size ? ` de ${allRows.length}` : ''} jogadores neste ${report.mode === 'day' ? 'dia' : 'recorte'}.</span></div><label>Ordenar por <select onchange="setFFWS25S2DropReportPlayerSort(this.value)">${options.map(([value,label]) => `<option value="${value}"${sortKey===value?' selected':''}>${label}</option>`).join('')}</select></label></div>
      ...
      <div class="ffws-s2-drop-player-list">${rows.length ? rows.map((row,index) => `<div class="ffws-s2-drop-player-row">
        ...
        <button type="button" class="ffws-s2-drop-player-team" title="${escapeHtml(row.team)}" onclick="toggleFFWS25S2DropReportPlayerTeam('${jsAttr(row.team)}')"><img src="${escapeHtml(logo(row.team))}" alt="${escapeHtml(abbreviation(row.team))}" onerror="this.onerror=null;this.src='escudo.webp'"></button>
  ...
  function s2DropReportTabsHtml(tab) {
    return `<div class="ffws-s2-drop-report-tabs" role="tablist"><button type="button" class="${tab==='summary'?'active':''}" onclick="setFFWS25S2DropReportTab('summary')">Resumo</button><button type="button" class="${tab==='teams'?'active':''}" onclick="setFFWS25S2DropReportTab('teams')">Equipes</button><button type="button" class="${tab==='players'?'active':''}" onclick="setFFWS25S2DropReportTab('players')">Jogadores</button></div>`;
  }
  ...
  function ensureFFWS25S2DropReportModal() {
    let modal = document.getElementById('ffws25-s2-drop-report-modal');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'ffws25-s2-drop-report-modal';
      modal.className = 'ffws-s2-drop-report-modal';
      modal.hidden = true;
      modal.setAttribute('onclick', 'if(event.target===this)closeFFWS25S2DropReport()');
      modal.innerHTML = '<div class="ffws-s2-drop-report-dialog" role="dialog" aria-modal="true" aria-label="Relatório do dia ou da queda"><div id="ffws25-s2-drop-report-body"></div></div>';
  ...
  function renderFFWS25S2DropReportModal() {
    const modal = document.getElementById('ffws25-s2-drop-report-modal');
    const body = document.getElementById('ffws25-s2-drop-report-body');
    ...
    const nextDropAction = nav.nextDrop ? `openFFWS25S2DropReport('${s2DropReportEventKey(nav.nextDrop)}')` : '';
    const nextDayAction = nav.nextDay ? `openFFWS25S2DayReport('${jsAttr(nav.nextDay.stage)}',${number(nav.nextDay.day)})` : '';
    ...
    body.innerHTML = `<div class="ffws-s2-drop-report-top"><div class="ffws-s2-drop-report-title"><small>${escapeHtml(s2StatsStageLabel(report.stage))}</small><strong>${escapeHtml(topTitle)}</strong><span>${escapeHtml(topSub)}</span></div><button type="button" class="ffws-s2-drop-report-close" onclick="closeFFWS25S2DropReport()" aria-label="Fechar relatório">×</button></div><div class="ffws-s2-drop-report-nav-v11"><button type="button" class="ffws-s2-drop-report-navbtn" ${nav.nextDrop?'':'disabled'} onclick="${nextDropAction}" aria-label="Ir para ${escapeHtml(nextDropLabel)}"><span class="ffws-s2-drop-report-navcopy"><small>PRÓX. QUEDA</small><strong>${escapeHtml(nextDropLabel)}</strong></span><span class="ffws-s2-drop-report-navarrow" aria-hidden="true">→</span></button><button type="button" class="ffws-s2-drop-report-navbtn" ${nav.nextDay?'':'disabled'} onclick="${nextDayAction}" aria-label="Ir para ${escapeHtml(nextDayLabel)}"><span class="ffws-s2-drop-report-navcopy"><small>PRÓX. DIA</small><strong>${escapeHtml(nextDayLabel)}</strong></span><span class="ffws-s2-drop-report-navarrow" aria-hidden="true">→</span></button></div>${s2DropReportTabsHtml(tab)}<div class="ffws-s2-drop-report-tabbody">${tab === 'teams' ? s2DropReportTeamsHtml(report) : tab === 'players' ? s2DropReportPlayersHtml(report) : s2DropReportSummaryHtml(report)}</div>`;
  }
  ...
  function s2StatsStickyNavHtml() {
    const items = [
      ['anchor-25s2-resumo','Resumo'], ['anchor-25s2-filtros','Filtros'], ['anchor-25s2-totais-equipe','Totais Equipe'],
      ['anchor-25s2-medias-equipe','Médias Equipe'], ['anchor-25s2-top3-equipe','Equipes/Queda'], ['anchor-25s2-totais-jogador','Totais Jog.'],
      ['anchor-25s2-medias-jogador','Médias Jog.'], ['anchor-25s2-participacao','Participação'], ['anchor-25s2-novatos','Novatos'],
      ['anchor-25s2-rec-drop-player','Rec. Q Jog.'], ['anchor-25s2-rec-day-player','Rec. Dia Jog.'], ['anchor-25s2-rec-drop-team','Rec. Q Times'],
      ['anchor-25s2-rec-day-team','Rec. Dia Times'], ['anchor-25s2-relatorio','Relatório'], ['anchor-25s2-evolucao','Evolução'],
      ['anchor-25s2-ranking-medias','Médias'], ['anchor-25s2-ranking-totais','Totais']
    ];
    const cut = Math.ceil(items.length / 2);
    const rows = [items.slice(0, cut), items.slice(cut)];
    return `<div class="cff-stats-sticky-nav" data-stats-sticky-nav="s2"><span>Ir para:</span><div class="cff-stats-sticky-rows">${rows.map(row => `<div class="cff-stats-sticky-row">${row.map(([id,label]) => `<button type="button" onclick="scrollFFWS25S2StatsSection('${id}')">${escapeHtml(label)}</button>`).join('')}</div>`).join('')}</div></div>`;
  }
  ...
  function s2StatsEvolutionHtml(events) {
    ...
    return `<section id="anchor-25s2-evolucao" class="ffws-s2-panel ffws-s2-stats-evolution"><div class="ffws-s2-panel-inner">
      ...
      <div class="ffws-s2-evolution-controls"><label class="ffws-s2-filter"><span>Métrica do gráfico:</span><select onchange="setFFWS25S2StatsEvolutionMetric(this.value)"><option value="position"${state.statsEvolution.metric==='position'?' selected':''}>Posição geral (acumulada)</option><option value="points"${state.statsEvolution.metric==='points'?' selected':''}>Pontos por dia</option><option value="kills"${state.statsEvolution.metric==='kills'?' selected':''}>Abates por dia</option></select></label><div class="ffws-s2-evolution-team-wrap"><span>Equipes (máx. 5):</span><div class="ffws-s2-evolution-team-buttons">${teams.map(team => `<button type="button" class="${selected.has(normalize(team)) ? 'active' : ''}" onclick="toggleFFWS25S2StatsEvolutionTeam('${jsAttr(team)}')" title="${escapeHtml(team)}"><img src="${escapeHtml(logo(team))}" alt="" onerror="this.onerror=null;this.src='escudo.webp'"><b>${escapeHtml(abbreviation(team))}</b></button>`).join('')}</div></div></div>
      <div class="ffws-s2-evolution-canvas-wrap"><canvas id="ffws25-s2-stats-evolution-chart"></canvas><div id="ffws25-s2-stats-evolution-empty" class="ffws-s2-empty" hidden><div><strong>Sem dados suficientes</strong>Selecione ao menos uma equipe com resultados no recorte.</div></div></div>
    </div></section>`;
  }
  ...
  function s2StatsEvolutionSeries(events, teamName, metric) {
    ...
    periods.forEach(period => {
      ...
      if (metric === 'position') {
        ...
        values.push(row ? (metric === 'kills' ? row.kills : row.points) : null);
      }
    });
    return values;
  }
  ...
  function renderS2StatsEvolutionChart() {
    const canvas = document.getElementById('ffws25-s2-stats-evolution-chart');
    if (!canvas) return;
    const events = filteredStatsEvents();
    const periods = s2StatsEvolutionPeriods(events);
    const selected = (state.statsEvolution.teams || []).slice(0,5);
    const empty = document.getElementById('ffws25-s2-stats-evolution-empty');
    ...
    const draw = () => {
      if (!window.Chart || !document.getElementById('ffws25-s2-stats-evolution-chart')) return;
  ...
  function renderStats() {
    const root = document.getElementById('ffws-br-2025-s2-stats-content');
    if (!root) return;
    if (s2StatsEvolutionChart) { try { s2StatsEvolutionChart.destroy(); } catch (_) {} s2StatsEvolutionChart = null; }
    ...
    const stageOptions = [['classificatoria','Classificatória'],['geral','Geral']];
    ...
    const summaryBlock = events.length ? `<section id="anchor-25s2-resumo" class="ffws-s2-panel"><div class="ffws-s2-panel-inner"><div class="ffws-s2-panel-head"><div><h2>Resumo do recorte</h2><p>Visão rápida do recorte atualmente selecionado.</p></div><span class="ffws-s2-badge">${events.length} quedas</span></div>
    ...
    const filterBlock = `<section id="anchor-25s2-filtros" class="ffws-s2-panel ffws-s2-stats-filter-panel"><div class="ffws-s2-panel-inner"><div class="ffws-s2-panel-head"><div><h2>Filtros do torneio</h2><p>Todos os blocos abaixo usam exatamente o mesmo recorte.</p></div><span class="ffws-s2-badge">${events.length} quedas</span></div>
      <div class="ffws-s2-filters"><label class="ffws-s2-filter"><span>Etapa:</span><select onchange="setFFWS25S2StatsStage(this.value)">${stageOptions.map(([value,label]) => `<option value="${value}"${state.statsFilters.stage === value ? ' selected' : ''}>${label}</option>`).join('')}</select></label>${statsMultiFilter('days','Dias',options.days)}${statsMultiFilter('maps','Mapa',options.maps)}</div></div></section>`;

    const statsBlocks = events.length ? `<div class="ffws-s2-stats-body" style="margin-top: 4px;">
        <h4 id="anchor-25s2-totais-equipe" style="color:#66b3ff; margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Totais por Equipe</h4>
        ...
        <h4 id="anchor-25s2-medias-equipe" style="color:#66b3ff; margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Médias por Equipe</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-top3-equipe" style="color:#66b3ff; margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Top 3 &amp; Último Lugar por Equipe <span style="font-size:0.55em; color:#888; text-transform:none; font-weight:normal;">(número de quedas ao lado)</span></h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-totais-jogador" style="color:var(--accent); margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Totais por Jogador</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-medias-jogador" style="color:var(--accent); margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Médias por Jogador</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-novatos" style="color:var(--accent); margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Jogadores Novatos</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-rec-drop-player" style="color:var(--accent); margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Recordes por Queda - Jogadores</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-rec-day-player" style="color:var(--accent); margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Recordes por Dia - Jogadores</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-rec-drop-team" style="color:#66b3ff; margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Recordes por Queda - Times</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
        ...
        <h4 id="anchor-25s2-rec-day-team" style="color:#66b3ff; margin: 0 0 15px 0; text-transform: uppercase; border-bottom: 1px solid #333; padding-bottom: 5px;">Recordes por Dia - Times</h4>
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 40px;">
      ...
      <section class="ffws-s2-panel"><div class="ffws-s2-panel-inner"><div class="ffws-s2-ranking-columns"><div id="anchor-25s2-ranking-medias"><div class="ffws-s2-panel-head"><div><h2>Ranking de Médias</h2><p>Desempenho por queda.</p></div></div>${statsRankingTable(avgRanking, true)}</div><div id="anchor-25s2-ranking-totais"><div class="ffws-s2-panel-head"><div><h2>Ranking de Totais</h2><p>Desempenho acumulado.</p></div></div>${statsRankingTable(totalRanking, false)}</div></div></div></section>
    ...
    root.innerHTML = `<div class="ffws-s2-shell">${hero('Estatísticas Gerais', 'Indicadores e rankings das equipes da WB 2025 S2')}
    ...
    requestAnimationFrame(() => { renderS2StatsEvolutionChart(); updateCff25StatsBackTopVisibility(); });
  }
  ...
  function renderNotes() {
    const root = document.getElementById('ffws-br-2025-s2-notas-content');
    if (!root) return;
    const f = state.notesFilters;
    ...
    const noteRecord=(key,title,list,subtitle,action)=>{
      ...
      const pager=list.length>pageSize?`<div class="ffws-s2-mvp-pager ffws-s2-note-record-pager"><button type="button" aria-label="Página anterior" ${current<=0?'disabled':''} onclick="setFFWS25S2NotesRecordPage('${key}',${current-1})">‹</button><span>${start+1}–${end} de ${list.length}</span><button type="button" aria-label="Próxima página" ${current>=pageCount-1?'disabled':''} onclick="setFFWS25S2NotesRecordPage('${key}',${current+1})">›</button></div>`:'';
    ...
    const pageRows=rows.slice(pageStart,pageStart+pageSize);
    const pageEnd=Math.min(pageStart+pageRows.length,rows.length);
    const pager=rows.length>pageSize?`<div class="ffws-s2-mvp-pager ffws-s2-notes-pager"><button type="button" aria-label="Ranking anterior" ${state.notesPage<=0?'disabled':''} onclick="setFFWS25S2NotesPage(${state.notesPage-1})">‹</button><span>${pageStart+1}–${pageEnd} de ${rows.length}</span><button type="button" aria-label="Próximo ranking" ${state.notesPage>=pageCount-1?'disabled':''} onclick="setFFWS25S2NotesPage(${state.notesPage+1})">›</button></div>`:'';
    ...
    root.innerHTML = `<div class="ffws-s2-shell">${hero('Notas CFF', 'Avaliações por queda e por dia da Central Free Fire')}
      ...
      ${notesModeControl()}
      <div class="ffws-s2-filters ffws-s2-notes-filters">${notesMultiFilter('stage','Etapa',[{value:'classificatoria',label:'Classificatória'},{value:'segundaFase',label:'Contra Squad'},{value:'final',label:'Final'}])}${notesMultiFilter('team','Equipe',teams.map(v=>({value:v,label:v})))}${notesMultiFilter('role','Posição',[{value:'RUSH',label:'Rush'},{value:'GRAN',label:'Granadeiro'},{value:'SUP',label:'Suporte'},{value:'3',label:'3º homem'}])}${notesMultiFilter('day','Dia',days.map(v=>({value:String(v),label:`Dia ${v}`})))}${notesMultiFilter('drop','Queda',drops.map(v=>({value:String(v),label:`Queda ${v}`})),{disabled:!f.day.length,title:'Escolha ao menos um dia para liberar o filtro de queda',disabledLabel:'Selecione um dia'})}${notesMultiFilter('map','Mapa',maps.map(v=>({value:v,label:v})))}${notesHalfMatchesControl()}</div>
      ...
      <section class="ffws-s2-panel"><div class="ffws-s2-panel-inner"><div class="ffws-s2-panel-head"><div><h2>Recordes de avaliação</h2><p>Melhores atuações individuais por queda, dia e média do recorte. Use as setas para ver as próximas posições.</p></div></div><div class="ffws-s2-top-grid">${noteRecord('drops','Top Quedas',topDrops,row=>`Dia ${row.day} • Q${row.drop} • ${row.kills} K`,row=>`openFFWS25S2NoteDropReport('${jsAttr(row.stage)}',${number(row.day)},${number(row.drop)})`)}${noteRecord('days','Top Dias',topDays,row=>`Dia ${row.day} • ${row.kills} K • ${row.damage.toLocaleString('pt-BR')} dano`,row=>`openFFWS25S2NoteDayReport('${jsAttr(row.stage)}',${number(row.day)})`)}${noteRecord('averages','Médias Gerais',generalAverageRows,row=>mode==='day'?`${row.dayCount} dia${row.dayCount===1?'':'s'} • ${row.matches} quedas`:`${row.matches} quedas • ${row.kills} K`,row=>`openCurrentSeasonPlayer('${jsAttr(row.name)}','${jsAttr(row.team)}')`)}</div></div></section>
      ${s2NotesBackTopButtonHtml()}
    </div>`;
    requestAnimationFrame(() => window.updateCff25StatsBackTopVisibility?.());
  }

  function notesModeControl() {
    const selected = state.notesFilters.mode === 'drop' ? 'drop' : 'day';
    return `<div class="ffws-s2-notes-mode-row"><label class="ffws-s2-filter ffws-s2-notes-mode-filter"><span>Cálculo do ranking:</span><select onchange="setFFWS25S2NotesFilter('mode',this.value)"><option value="day"${selected==='day'?' selected':''}>Por dia</option><option value="drop"${selected==='drop'?' selected':''}>Por queda</option></select></label></div>`;
  }
  ...
  function notesMultiFilter(key, label, options, config = {}) {
    ...
    return `<div class="ffws-s2-filter"><span>${escapeHtml(label)}:</span><div class="ffws-s2-multi ffws-s2-notes-multi" data-s2-notes-multi="${escapeHtml(key)}">
      <button type="button" onclick="toggleFFWS25S2NotesMulti('${key}')"${disabled?' disabled':''}${config.title?` title="${escapeHtml(config.title)}"`:''}><b>${escapeHtml(buttonLabel)}</b><span>⌄</span></button>
      <div class="ffws25-s2-multi-menu" id="ffws25-s2-notes-multi-${escapeHtml(key)}"${open?'':' hidden'}>
        <label><input type="checkbox" ${selected.length===0?'checked ':''}onchange="clearFFWS25S2NotesMulti('${key}')"> Todos</label>
        ${options.map(option=>`<label><input type="checkbox" value="${escapeHtml(option.value)}" ${selected.includes(String(option.value))?'checked ':''}onchange="setFFWS25S2NotesMulti('${key}',this.value,this.checked)"> ${escapeHtml(option.label)}</label>`).join('')}
      </div>
    </div></div>`;
  }
  ...
  function notesHalfMatchesControl() {
    ...
    return `<div class="ffws-s2-filter ffws-s2-notes-half-filter"><span>Participação:</span><label class="ffws-s2-notes-half-check" title="Exibe somente jogadores que disputaram pelo menos 50% das quedas realizadas pela própria equipe em todo o recorte atual"><input type="checkbox" ${checked?'checked ':''}onchange="setFFWS25S2NotesHalfMatches(this.checked)"><b>50% das quedas</b></label></div>`;
  }
  ...
  function comparePlayerPicker(side, rows, selectedRow, label) {
    ...
    return `<label class="ffws25-s2-compare-picker-label"><span>${escapeHtml(label)}</span><div class="ffws-s2-compare-picker" data-compare-picker="${side}">
      <div class="ffws25-s2-compare-search-box"><span aria-hidden="true">⌕</span><input id="ffws25-s2-compare-search-${side}" type="search" value="${escapeHtml(selectedText)}" placeholder="Pesquisar jogador..." autocomplete="off" spellcheck="false" onfocus="this.select();openFFWS25S2ComparePicker('${side}')" oninput="filterFFWS25S2ComparePicker('${side}',this.value)" onkeydown="handleFFWS25S2ComparePickerKey(event,'${side}')"></div>
      <div class="ffws25-s2-compare-picker-menu" id="ffws25-s2-compare-picker-${side}" hidden>
        <div class="ffws25-s2-compare-picker-hint">Digite o nome do jogador</div>
        <div class="ffws25-s2-compare-picker-options">${ordered.map(row => `<button type="button" class="ffws25-s2-compare-picker-option${selectedRow?.key === row.key ? ' is-selected' : ''}" data-compare-search="${escapeHtml(`${row.name} ${row.team} ${abbreviation(row.team)}`)}" onclick="chooseFFWS25S2ComparePlayer('${side}','${jsAttr(row.key)}')"><img src="${escapeHtml(logo(row.team))}" alt="" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='escudo.webp'"><span><strong>${escapeHtml(row.name)}</strong><small>${escapeHtml(row.team)}</small></span>${selectedRow?.key === row.key ? '<b>✓</b>' : ''}</button>`).join('')}</div>
        <div class="ffws25-s2-compare-picker-empty" hidden>Nenhum jogador encontrado.</div>
      </div>
    </div></label>`;
  }

  function renderCompare() {
    const root = document.getElementById('ffws-br-2025-s2-comparar-content');
    if (!root) return;
    ...
    root.innerHTML = `<div class="ffws-s2-shell">${hero('Comparar 1V1', 'Compare dois jogadores da WB 2025 S2')}
      ...
      <div class="ffws-s2-filters">${compareSelect('stage','Etapa',[['classificatoria','Classificatória'],['geral','Geral']])}${compareMultiFilter('roles','Posição',roleOptions)}${compareSelect('day','Dia',[['all','Todos'],...days.map(v=>[String(v),`Dia ${v}`])])}${compareSelect('map','Mapa',[['all','Todos'],...maps.map(v=>[v,v])])}</div>
  ...
  function compareSelect(key,label,options){const selected=String(state.compareFilters[key]);return `<label class="ffws-s2-filter"><span>${escapeHtml(label)}:</span><select onchange="setFFWS25S2CompareFilter('${key}',this.value)">${options.map(([value,text])=>`<option value="${escapeHtml(value)}"${selected===String(value)?' selected':''}>${escapeHtml(text)}</option>`).join('')}</select></label>`;}
  ...
  function compareMultiFilter(key, title, options) {
    ...
    return `<div class="ffws-s2-filter"><span>${escapeHtml(title)}:</span><div class="ffws-s2-multi" data-s2-compare-multi="${escapeHtml(key)}">
      <button type="button" onclick="toggleFFWS25S2CompareMulti('${key}')"><b>${escapeHtml(label)}</b><span>⌄</span></button>
      <div class="ffws25-s2-multi-menu" id="ffws25-s2-compare-multi-${escapeHtml(key)}" hidden>
        <label><input type="checkbox" ${selected.length === 0 ? 'checked' : ''} onchange="clearFFWS25S2CompareMulti('${key}')"> Todos</label>
        ${options.map(option => `<label><input type="checkbox" value="${escapeHtml(option.value)}" ${selected.includes(String(option.value)) ? 'checked' : ''} onchange="setFFWS25S2CompareMulti('${key}',this.value,this.checked)"> ${escapeHtml(option.label)}</label>`).join('')}
      </div>
    </div></div>`;
  }

  function render2025ComingSoon(rootId, title, subtitle) {
    const root = document.getElementById(rootId);
    if (!root) return;
    root.innerHTML = `<div class="ffws-s2-shell">${hero(title, subtitle)}<section class="ffws-s2-panel"><div class="ffws-s2-panel-inner"><div class="ffws-s2-empty"><div><strong>EM BREVE</strong>Os dados detalhados desta seção serão adicionados aos poucos.</div></div></div></section></div>`;
  }

  function renderPage(pageId) {
    if (!state.loaded) return;
    switch (pageId) {
      case 'ffws-br-2025-s2-classificatoria': renderStage('classificatoria', 'ffws-br-2025-s2-classificatoria-content', 'Classificatória', 'WB 2025 S2 • dados detalhados adicionados dia a dia'); break;
      case 'ffws-br-2025-s2-contra-squad': render2025ComingSoon('ffws-br-2025-s2-contra-squad-content', 'Contra Squad', 'WB 2025 S2'); break;
      case 'ffws-br-2025-s2-final': render2025ComingSoon('ffws-br-2025-s2-final-content', 'Final', 'WB 2025 S2'); break;
      case 'ffws-br-2025-s2-mvp': renderMvp(); break;
      case 'ffws-br-2025-s2-equipes': renderTeams(); break;
      case 'ffws-br-2025-s2-datas': renderDates(); break;
      case 'ffws-br-2025-s2-selecoes': render2025ComingSoon('ffws-br-2025-s2-selecoes-content', 'Seleções da Season', 'WB 2025 S2'); break;
      case 'ffws-br-2025-s2-stats': renderStats(); break;
      case 'ffws-br-2025-s2-notas': renderNotes(); break;
      case 'ffws-br-2025-s2-comparar': renderCompare(); break;
    }
  }

  function activate(pageId) {
    if (!PAGE_IDS.has(pageId)) return;
    if (pageId !== 'ffws-br-2025-s2-stats' && state.dropReport?.bodyOverflow !== undefined) {
      const modal = document.getElementById('ffws25-s2-drop-report-modal');
    ...
    if (root && !root.dataset.s2Loading) {
      ...
      root.innerHTML = '<div class="ffws-s2-empty"><div><strong>Carregando WB 2025 S2...</strong></div></div>';
    }
    const needsPlayers = new Set(['ffws-br-2025-s2-mvp', 'ffws-br-2025-s2-equipes', 'ffws-br-2025-s2-selecoes', 'ffws-br-2025-s2-stats', 'ffws-br-2025-s2-notas', 'ffws-br-2025-s2-comparar']).has(pageId);
    const needsPhotos = new Set(['ffws-br-2025-s2-equipes', 'ffws-br-2025-s2-selecoes']).has(pageId);
    loadData()
  ...
  window.setFFWS25S2StageFilter = (stageKey, type, value) => {
    ...
    const map = { classificatoria: 'ffws-br-2025-s2-classificatoria', segundaFase: 'ffws-br-2025-s2-contra-squad', final: 'ffws-br-2025-s2-final' };
    renderPage(map[stageKey]);
  };
  window.setFFWS25S2SelectionWeek = week => { state.selectionTab = 'semanal'; state.selectionWeek = String(week || '1'); renderSelections(); };
  window.setFFWS25S2SelectionTab = tab => { state.selectionTab = S2_SELECTION_PHASES[tab] ? tab : 'semanal'; renderSelections(); };
  window.setFFWS25S2StatsStage = value => { state.statsStage = value; state.statsFilters.stage = String(value || 'classificatoria'); state.statsFilters.days = []; state.statsFilters.maps = []; renderStats(); };
  window.toggleFFWS25S2StatsMulti = key => { document.querySelectorAll('.ffws25-s2-multi-menu').forEach(menu => { if (menu.id !== `ffws25-s2-stats-multi-${key}`) menu.hidden = true; }); const menu = document.getElementById(`ffws25-s2-stats-multi-${key}`); if (menu) menu.hidden = !menu.hidden; };
  window.clearFFWS25S2StatsMulti = key => { state.statsFilters[key] = []; renderStats(); };
  window.setFFWS25S2StatsMulti = (key, value, checked) => { const selected = new Set(state.statsFilters[key] || []); checked ? selected.add(String(value)) : selected.delete(String(value)); state.statsFilters[key] = [...selected]; renderStats(); };
  window.ffws25S2StatsPageNav = (key, delta) => { const pages = s2StatsPageState(); pages[key] = delta === 'reset' ? 0 : number(pages[key]) + number(delta); renderStats(); };
  window.scrollFFWS25S2StatsSection = id => document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  window.setFFWS25S2StatsEvolutionMetric = value => { state.statsEvolution.metric = ['position','points','kills'].includes(String(value)) ? String(value) : 'position'; renderS2StatsEvolutionChart(); };
  window.toggleFFWS25S2StatsEvolutionTeam = team => {
  ...
  window.updateCff25StatsBackTopVisibility = () => {
    document.querySelectorAll('.cff-stats-back-top').forEach(button => {
      const owner = button.dataset.statsBackTop;
      const active = owner === 's2'
        ? document.getElementById('ffws-br-2025-s2-stats')?.classList.contains('active')
        : owner === 'notes'
          ? document.getElementById('ffws-br-2025-s2-notas')?.classList.contains('active')
  ...
  window.openFFWS25S2DropReport = key => {
    ...
    const modal = ensureFFWS25S2DropReportModal();
    renderFFWS25S2DropReportModal();
    const reportBody = document.getElementById('ffws25-s2-drop-report-body');
  ...
  window.openFFWS25S2DayReport = (stage, day) => {
  ...
  window.closeFFWS25S2DropReport = () => {
    const modal = document.getElementById('ffws25-s2-drop-report-modal');
    if (modal) modal.hidden = true;
    document.body.style.overflow = state.dropReport.bodyOverflow ?? '';
    delete state.dropReport.bodyOverflow;
  ...
  window.setFFWS25S2DropReportTab = tab => {
  ...
  window.setFFWS25S2DropReportTeamSort = value => { state.dropReport.teamSort = String(value || 'points'); renderFFWS25S2DropReportModal(); };
  window.setFFWS25S2DropReportPlayerSort = value => { state.dropReport.playerSort = String(value || 'kills'); renderFFWS25S2DropReportModal(); };
  window.showFFWS25S2DropReportTeam = team => { state.dropReport.tab = 'teams'; state.dropReport.teamDetail = String(team || ''); renderFFWS25S2DropReportModal(); document.querySelector('.ffws-s2-drop-team-detail')?.scrollIntoView({ block: 'start' }); };
  window.hideFFWS25S2DropReportTeam = () => { state.dropReport.teamDetail = ''; renderFFWS25S2DropReportModal(); };
  window.toggleFFWS25S2DropReportPlayerTeam = team => {
    const key = normalize(team);
    const selected = new Set(state.dropReport.playerTeams || []);
  ...
  window.clearFFWS25S2DropReportPlayerTeams = () => { state.dropReport.playerTeams = []; renderFFWS25S2DropReportModal(); };
  window.setFFWS25S2NotesFilter = (key, value) => {
    if (key !== 'mode') return;
    state.notesFilters.mode = String(value) === 'drop' ? 'drop' : 'day';
    resetNotesPagination();
    renderNotes();
  };
  window.toggleFFWS25S2NotesMulti = key => {
    if (key === 'drop' && !state.notesFilters.day.length) return;
    const nextOpen = state.notesOpenMulti === key ? '' : key;
    state.notesOpenMulti = nextOpen;
    document.querySelectorAll('.ffws25-s2-multi-menu').forEach(menu => {
      menu.hidden = menu.id !== `ffws25-s2-notes-multi-${key}` || !nextOpen;
    });
  };
  window.clearFFWS25S2NotesMulti = key => {
  ...
  window.setFFWS25S2NotesMulti = (key, value, checked) => {
    if (!Array.isArray(state.notesFilters[key])) return;
    if (key === 'drop' && !state.notesFilters.day.length) return;
  ...
  window.setFFWS25S2NotesHalfMatches = checked => { state.notesFilters.halfMatches = Boolean(checked); resetNotesPagination(); renderNotes(); };
  window.setFFWS25S2NotesPage = page => { state.notesPage = Math.max(0, number(page)); renderNotes(); };
  window.setFFWS25S2NotesRecordPage = (key, page) => {
  ...
  function openS2StatsReportFromNotes(mode, stage, day, drop = 0) {
    ...
    ensureFFWS25S2DropReportModal();
    if (mode === 'day') {
      window.openFFWS25S2DayReport(targetStage, number(day));
      return;
    }
    ...
    if (event) window.openFFWS25S2DropReport(s2DropReportEventKey(event));
  }
  window.openFFWS25S2NoteDropReport = (stage, day, drop) => openS2StatsReportFromNotes('drop', stage, day, drop);
  window.openFFWS25S2NoteDayReport = (stage, day) => openS2StatsReportFromNotes('day', stage, day, 0);
  window.setFFWS25S2CompareFilter = (key, value) => { state.compareFilters[key] = String(value); if (key === 'stage') { state.compareFilters.day = 'all'; state.compareFilters.map = 'all'; } renderCompare(); };
  window.toggleFFWS25S2CompareMulti = key => {
    document.querySelectorAll('.ffws25-s2-multi-menu').forEach(menu => { if (menu.id !== `ffws25-s2-compare-multi-${key}`) menu.hidden = true; });
    const menu = document.getElementById(`ffws25-s2-compare-multi-${key}`);
    if (menu) menu.hidden = !menu.hidden;
  };
  window.clearFFWS25S2CompareMulti = key => { if (Array.isArray(state.compareFilters[key])) state.compareFilters[key] = []; renderCompare(); };
  window.setFFWS25S2CompareMulti = (key, value, checked) => {
  ...
  window.setFFWS25S2ComparePlayer = (side, value) => { state.comparePlayers[side] = String(value); renderCompare(); };
  window.openFFWS25S2ComparePicker = side => {
    document.querySelectorAll('.ffws25-s2-compare-picker-menu').forEach(menu => { menu.hidden = menu.id !== `ffws25-s2-compare-picker-${side}`; });
    const menu = document.getElementById(`ffws25-s2-compare-picker-${side}`);
    if (menu) menu.hidden = false;
    window.filterFFWS25S2ComparePicker(side, '');
  };
  window.filterFFWS25S2ComparePicker = (side, term) => {
    const menu = document.getElementById(`ffws25-s2-compare-picker-${side}`);
    if (!menu) return;
    menu.hidden = false;
    const query = normalize(term);
    let visible = 0;
    menu.querySelectorAll('.ffws25-s2-compare-picker-option').forEach(option => {
      const match = !query || normalize(option.dataset.compareSearch).includes(query);
      option.hidden = !match;
      if (match) visible += 1;
    });
    const empty = menu.querySelector('.ffws25-s2-compare-picker-empty');
    if (empty) empty.hidden = visible > 0;
  };
  window.chooseFFWS25S2ComparePlayer = (side, value) => {
    state.comparePlayers[side] = String(value);
    renderCompare();
  };
  window.handleFFWS25S2ComparePickerKey = (event, side) => {
    const menu = document.getElementById(`ffws25-s2-compare-picker-${side}`);
    if (!menu) return;
    if (event.key === 'Escape') { menu.hidden = true; event.currentTarget.blur(); return; }
    if (event.key === 'Enter') {
      const first = [...menu.querySelectorAll('.ffws25-s2-compare-picker-option')].find(option => !option.hidden);
      if (first) { event.preventDefault(); first.click(); }
    }
  };
  window.toggleFFWS25S2MvpDetails = button => {
  ...
  window.toggleFFWS25S2Multi = key => {
    document.querySelectorAll('.ffws25-s2-multi-menu').forEach(menu => { if (menu.id !== `ffws25-s2-multi-${key}`) menu.hidden = true; });
    const menu = document.getElementById(`ffws25-s2-multi-${key}`);
    if (menu) menu.hidden = !menu.hidden;
  };
  window.setFFWS25S2MvpPage = page => { state.mvpPage = Math.max(0, number(page)); renderMvp(); };
  window.clearFFWS25S2Multi = key => { state.playerFilters[key] = []; state.mvpPage = 0; renderMvp(); };
  window.setFFWS25S2Multi = (key, value, checked) => {
  ...
  document.addEventListener('click', event => {
    if (!event.target.closest('.ffws-s2-multi')) {
      document.querySelectorAll('.ffws25-s2-multi-menu').forEach(menu => { menu.hidden = true; });
      state.notesOpenMulti = '';
    }
    if (!event.target.closest('.ffws-s2-compare-picker')) document.querySelectorAll('.ffws25-s2-compare-picker-menu').forEach(menu => { menu.hidden = true; });
  });

  window.addEventListener('scroll', () => window.updateCff25StatsBackTopVisibility?.(), { passive: true });
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && !document.getElementById('ffws25-s2-drop-report-modal')?.hidden) closeFFWS25S2DropReport();
  });
