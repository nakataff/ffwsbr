...
function isTruthyNovatoValue(value) {
    const v = String(value || '').trim().toLowerCase();
    return ['true', 'sim', 's', '1', 'yes', 'y', 'verdadeiro', 'novato'].includes(v);
}

function parseSimpleTSV(text) {
    return String(text || '')
        .replace(/^\uFEFF/, '')
        .split(/\r?\n/)
...
async function loadNovatos() {
    novatosPlayers = {};
    const NOVATOS_TSV_URL = window.CFF_CONFIG?.sheets?.novatos;
    if (!NOVATOS_TSV_URL) return;

    try {
        const res = await fetch(`${NOVATOS_TSV_URL}&nocache=${Date.now()}`, { cache: 'no-store' });
        if (!res.ok) throw new Error('Erro ao carregar planilha de novatos');

        const rows = parseSimpleTSV(await res.text());
        if (!rows.length) return;

        const headers = rows[0].map(h => String(h || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim());
        const nameIdx = headers.findIndex(h => ['jogador', 'player', 'nome', 'nick', 'apelido'].includes(h) || h.includes('jogador'));
        const novatoIdx = headers.findIndex(h => h.includes('novato') || h.includes('rookie') || h.includes('true'));
        const hasHeader = nameIdx >= 0 || novatoIdx >= 0;
        const dataRows = hasHeader ? rows.slice(1) : rows;

        dataRows.forEach(row => {
            const nome = (row[nameIdx >= 0 ? nameIdx : 0] || '').trim();
            const status = row[novatoIdx >= 0 ? novatoIdx : 1];
            if (!nome || !isTruthyNovatoValue(status)) return;

            const canonical = typeof getCanonicalPlayerName === 'function' ? getCanonicalPlayerName(nome) : nome;
            const keys = new Set([nome, canonical]);
            if (typeof getPlayerAliasList === 'function') {
                getPlayerAliasList(canonical).forEach(alias => keys.add(alias));
            }

            keys.forEach(value => {
                const key = typeof normalizePlayerAliasKey === 'function'
                    ? normalizePlayerAliasKey(value)
                    : String(value || '').toLowerCase().replace(/[^a-z0-9]/gi, '');
                if (key) novatosPlayers[key] = true;
            });
        });

        console.log('Planilha de novatos carregada:', Object.keys(novatosPlayers).length, 'entradas');
    } catch (error) {
        console.warn('Não foi possível carregar a planilha de novatos:', error);
    }
}

function isNovatoPlayer(nome) {
    if (!nome) return false;
    const canonical = typeof getCanonicalPlayerName === 'function' ? getCanonicalPlayerName(nome) : nome;
    const keys = [nome, canonical];
    if (typeof getPlayerAliasList === 'function') {
        getPlayerAliasList(canonical).forEach(alias => keys.push(alias));
    }
    return keys.some(value => {
        const key = typeof normalizePlayerAliasKey === 'function'
            ? normalizePlayerAliasKey(value)
            : String(value || '').toLowerCase().replace(/[^a-z0-9]/gi, '');
        return !!novatosPlayers[key];
    });
}
