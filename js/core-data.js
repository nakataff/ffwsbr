...
const CFF_CLASSIFICATORIA_MAX_DAY = 20;
const CFF_FINAL_DAY_START = 21;

function cffNormalizeStage(stage) {
    return ['classificatoria', 'final', 'geral'].includes(String(stage || '').toLowerCase()) ? String(stage || '').toLowerCase() : 'classificatoria';
}

function cffIsDayInStage(day, stage) {
    const d = Number(day);
    const st = cffNormalizeStage(stage);
    if (st === 'classificatoria') return d > 0 && d <= CFF_CLASSIFICATORIA_MAX_DAY;
    if (st === 'final') return d >= CFF_FINAL_DAY_START;
    return d > 0;
}

function cffGetDbDaysByStage(stage) {
    return Object.keys(dbQuedas || {})
        .filter(d => cffIsDayInStage(d, stage))
        .sort((a, b) => Number(a) - Number(b))
        .map(String);
}

function cffGetFinalDayOptions() {
    const days = cffGetDbDaysByStage('final');
    return days.length ? days : ['21', '22'];
}

function cffFilterDaysByStage(days, stage) {
    const base = (Array.isArray(days) && days.length) ? days.map(String) : cffGetDbDaysByStage(stage);
    return base.filter(d => cffIsDayInStage(d, stage));
}

function cffGetStageLabel(stage) {
    const st = cffNormalizeStage(stage);
    if (st === 'final') return 'Final';
    if (st === 'geral') return 'Geral';
    return 'Classificatória';
}

function cffAggregateTeamsByDays(days) {
    const daySet = new Set((days || []).map(String));
    const teamsData = db.teams.map(t => ({
        ...t,
        pontos: 0,
        booyah: 0,
        abates: 0,
    ...
    for (const d in (dbQuedas || {})) {
        if (daySet.size && !daySet.has(String(d))) continue;
        for (const q in dbQuedas[d]) {
            const drop = dbQuedas[d][q];
            (drop.resultados || []).forEach(res => {
                const tIndex = teamsData.findIndex(t => String(t.equipe).toUpperCase() === String(res.equipe).toUpperCase());
                if (tIndex < 0) return;
                const pPos = posPoints[res.posicao] || 0;
                teamsData[tIndex].pontos += pPos + (Number(res.kills) || 0);
                teamsData[tIndex].abates += Number(res.kills) || 0;
                teamsData[tIndex].booyah += Number(res.booyah) || 0;
                teamsData[tIndex].quedas += 1;
                teamsData[tIndex].somaPosicao += Number(res.posicao) || 0;
                teamsData[tIndex].didNotPlay = false;
            });
        }
    }
    return teamsData;
...
function cffGetClassificatoriaTeams() {
    const days = cffGetDbDaysByStage('classificatoria');
    return cffAggregateTeamsByDays(days)
        .filter(t => !t.didNotPlay)
        .sort((a, b) => (Number(b.pontos) || 0) - (Number(a.pontos) || 0) || (Number(b.abates) || 0) - (Number(a.abates) || 0))
        .map((t, i) => ({ ...t, posGeral: i + 1 }));
}

function applyClassificatoriaTotalsToDbTeams() {
    const classTeams = cffGetClassificatoriaTeams();
    const byName = new Map(classTeams.map(t => [String(t.equipe).toUpperCase(), t]));
    db.teams.forEach(team => {
        const clean = byName.get(String(team.equipe).toUpperCase());
        if (!clean) return;
        team.pontos = clean.pontos;
        team.booyah = clean.booyah;
        team.abates = clean.abates;
        team.quedas = clean.quedas;
        team.posGeral = clean.posGeral;
    });
}
...
let selectedFinalDays = [];
let selectedStatsStage = 'classificatoria';
let selectedMvpStage = 'classificatoria';
...
const data = await response.json();
    ...
    distribuirNovosResultados();
    autoCalculateTotals();
    applyClassificatoriaTotalsToDbTeams();
    calculatePlayerRanks();
    db.teams.sort((a,b) => b.pontos - a.pontos || b.abates - a.abates).forEach((t, i) => t.posGeral = i + 1);
    ...
    // Carrega aliases e passagens antes do Hall da Fama, pois a segunda busca usa a planilha de passagens.
