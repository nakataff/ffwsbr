...
// Utilitário leve para jogar tarefas pesadas para depois da primeira renderização.
// Evita que Home, LCP e interação inicial disputem CPU com rankings/gráficos ocultos.
window.cffRunWhenIdle = window.cffRunWhenIdle || function(callback, timeout = 900) {
...
window.cffLazyLoadVisibleImages = window.cffLazyLoadVisibleImages || function(root = document) {
    try {
        root.querySelectorAll('img:not([loading])').forEach((img, idx) => {
            const rect = img.getBoundingClientRect ? img.getBoundingClientRect() : null;
            const isHero = rect && rect.top < (window.innerHeight || 900) + 120;
            if (!isHero && idx > 6) img.loading = 'lazy';
...
let playerPhotos = {
    ...
    "HAK": "hak.webp", "NICKZ": "nickz7.webp", "TRAP": "trap.webp", "GUAXA": "guaxa.webp", "BZP": "bzp.webp",
    ...
    "Bahiaz7": "bahia.webp", "BIELGOD": "bielgod.webp", "Rojão": "silhueta.webp", "ZETSU9": "zetsu.webp", "Mts007": "mts007.webp",
...
const data = await response.json();
    ...
    const initialPage = initialHashPage && document.getElementById(initialHashPage) ? initialHashPage : 'home';
    navigate(initialPage);

    // Renderização inicial enxuta: a Home não deve montar todas as páginas ocultas.
    // As seções pesadas são renderizadas sob demanda pelo menu em navigation-search.js.
    renderHomeStats();
    renderHomeGroups();
    if (typeof cffLazyLoadVisibleImages === 'function') cffLazyLoadVisibleImages();

    if (initialPage !== 'home' && typeof navigateAndClose === 'function') {
        cffRunWhenIdle(() => navigateAndClose(initialPage), 250);
    }

    buildDayFilters();
    populateSelects();

    // Auto-seleciona a semana mais recente com dados para quando a seção for aberta.
    ...
    // Diretórios e integrações que não são essenciais para o primeiro paint carregam em idle.
    buildDesktopTeamNav();
    initSearchListeners();
    if (initialHashPage && typeof cffResolveHashRoute === 'function') cffResolveHashRoute(initialHashPage);

    cffRunWhenIdle(() => {
        try { renderOutrosTorneiosList(); } catch(e) {}
        try { renderSchedule(); } catch(e) {}
        try { renderOutrasEquipesGrid(); } catch(e) {}
    }, 1200);

    cffRunWhenIdle(() => {
        try { loadLives(); } catch(e) {}
        try { loadNoticias(); } catch(e) {}
        try { loadSobre(); } catch(e) {}
        try { loadSocials(); } catch(e) {}
        try { loadLeagueLogos(); } catch(e) {}
        try { if (typeof loadNovosTorneios === 'function') loadNovosTorneios(); } catch(e) {}
        try { loadCampeonatos(); } catch(e) {}
        try { loadTitles(); } catch(e) {}
        try { loadStaff(); } catch(e) {}
    }, 1800);

    // Fotos e SEA são mais pesados; carregam depois e seguem disponíveis quando suas páginas forem abertas.
    cffRunWhenIdle(() => { try { loadPhotos(); } catch(e) {} }, 2600);
    cffRunWhenIdle(() => { try { loadSEAData(true); } catch(e) {} }, 3200);
