...
function getCacheBusterValue() {
    return localStorage.getItem(CFF_CACHE_BUSTER_KEY) || String(Date.now());
}

function withCacheBuster(url) {
    if (!url) return url;
    const separator = url.includes('?') ? '&' : '?';
    return `${url}${separator}nocache=${encodeURIComponent(getCacheBusterValue())}`;
}

async function purgeSiteCache() {
    try {
        localStorage.setItem(CFF_CACHE_BUSTER_KEY, String(Date.now()));
...
window.onload = async () => {
    ...
    try {
        const response = await fetch(withCacheBuster('dados.json'));
...
const data = await response.json();
    ...
    navigate(initialPage, { replaceHash: true });
...
function getPageFromHash() {
    ...
    const pageId = decodeURIComponent(rawHash.replace('#', '')).trim();

    if (!pageId) return 'home';

    const pageEl = document.getElementById(pageId);
    if (!pageEl || !pageEl.classList.contains('page')) return 'home';

    // Evita abrir páginas internas vazias direto pelo F5, como perfil de jogador/time.
    // Nesses casos, volta para a última página principal salva ou para home.
    if (pageId.includes('profile')) return lastVisitedPage || 'home';

    return pageId;
}

function updatePageHash(pageId, replaceHash = true) {
    if (!pageId || pageId.includes('profile')) return;
...
function navigate(pageId, options = {}) {
    ...
    const shouldReplaceHash = options.replaceHash !== false;

    // Força a página a voltar para o topo instantaneamente
    window.scrollTo(0, 0);

    // Salva a aba atual na memória (ignora se for página de perfil)
    if (!targetPage.includes('profile')) {
        lastVisitedPage = targetPage;
        updatePageHash(targetPage, shouldReplaceHash);
    }
    ...
    if (mobileTabMap[targetPage]) {
        const activeTab = document.getElementById(mobileTabMap[targetPage]);
        if (activeTab) activeTab.classList.add('active');
    }

    // Atualiza estado dos botões da sidebar
    document.querySelectorAll('.sidebar-nav button').forEach(b => b.classList.remove('active'));
    const sbBtn = document.getElementById('sb-' + targetPage);
    if (sbBtn) sbBtn.classList.add('active');
}

// Permite trocar de página ao editar o hash manualmente ou usar voltar/avançar do navegador.
window.addEventListener('hashchange', () => {
    navigate(getPageFromHash(), { replaceHash: false });
});
