...
function cffGetSelectedStatsStage() {
    return document.getElementById('filter-stage-stats')?.value || 'classificatoria';
}
    ...
    const lazyHistoryLoad = async () => {
        try {
            if (typeof loadPassagens === 'function' && (!Array.isArray(dbPassagens) || dbPassagens.length === 0)) {
                await loadPassagens();
                if (typeof buildPlayerAliasesFromPassagens === 'function') buildPlayerAliasesFromPassagens();
            }
        } catch (e) { console.warn('[init] passagens em segundo plano falhou:', e); }
        try {
