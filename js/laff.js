...
(function () {
        ...
        wrap.innerHTML = `<div class="table-container"><table class="laff-players-table laff-table" id="laff-table-players">
            ...
            <tbody>${rows.map((p, i) => {
                ...
                return `<tr onclick="openLAFFPlayerProfile(${escape(jsArg(p.name))})" style="cursor:pointer;">
                    ...
                    <td><img src="${escape(tLogo || 'escudo.webp')}" onerror="this.onerror=null;this.src='escudo.webp'" alt="" style="width:18px;height:18px;object-fit:contain;vertical-align:middle;margin-right:5px;"> <span class="hide-mobile">${escape(p.team || '—')}</span></td>
                    <td class="hide-mobile">${escape(p.funcao || '—')}</td>
    ...
    async function openLAFFTeamProfile(name) {
        ...
        root.innerHTML = `<section class="laff-profile-hero"><img class="laff-profile-logo" src="${escape(team.logo || 'escudo.webp')}" onerror="this.onerror=null;this.src='escudo.webp'" alt="${escape(team.name)}"><div><div class="laff-kicker">LAFF 2026 Split 1</div><h1>${escape(team.name)}</h1><p>${escape(getShortOrigin(team.origem))}${group ? ` • Grupo ${escape(group)}` : ''}</p></div></section>
            <section class="laff-panel laff-profile-section"><h2>Elenco atual</h2><div class="laff-players-grid">${team.players.length ? team.players.map(renderPlayerCard).join('') : `<div class="laff-empty">Elenco será exibido conforme os jogadores forem confirmados.</div>`}</div></section>
            ...
            <section class="laff-panel laff-profile-section"><h2>Desempenho histórico</h2>${buildLAFFTeamHistoricalSummary(team)}</section>
    ...
    async function openLAFFPlayerProfile(name) {
        ...
        // Se o jogador também está na WB com uma equipe WB ativa, abre o perfil WB completo (evita perfil duplicado)
        const wbTeamNames = new Set((db.teams || []).map(t => key(t.equipe)));
        const isWBPlayer = typeof openPlayerProfile === 'function'
            && (typeof db !== 'undefined')
            && (db.players || []).some(x => {
                const matchFn = typeof checkNameMatch === 'function' ? checkNameMatch : (a, b) => a.toLowerCase() === b.toLowerCase();
                return matchFn(x.jogador, player.name) && wbTeamNames.has(key(x.equipe));
            });
        if (isWBPlayer) {
