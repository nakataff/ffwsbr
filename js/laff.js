...
(function () {
    ...
    function getPlayerPhoto(playerName) {
        if (typeof playerPhotos === 'undefined' || !playerPhotos) return 'silhueta.webp';
        ...
        return found ? playerPhotos[found] : 'silhueta.webp';
    }
    ...
    function getLAFFTeamShortName(teamOrName) {
        const team = typeof teamOrName === 'object' && teamOrName ? teamOrName : getLAFFTeamByName(teamOrName);
        const name = typeof teamOrName === 'object' && teamOrName ? team.name : String(teamOrName || '').trim();
        const manual = String(team?.shortName || team?.abreviacao || '').trim();
        if (manual) return manual;
        if (typeof shortNames !== 'undefined' && shortNames && shortNames[name]) return shortNames[name];
        return name;
    }

    function renderResponsiveTeamName(teamOrName) {
        const team = typeof teamOrName === 'object' && teamOrName ? teamOrName : getLAFFTeamByName(teamOrName);
        const full = String(team?.name || teamOrName || '').trim();
        const short = getLAFFTeamShortName(team || full);
        if (!full) return '';
        if (!short || norm(short) === norm(full)) return escape(full);
        return `<span class="laff-team-name-full">${escape(full)}</span><span class="laff-team-name-short">${escape(short)}</span>`;
    }

    function formatLAFFRole(role, compact = false) {
        const n = norm(role);
        ...
        return compact ? String(role || '').toUpperCase().slice(0, 4) : String(role || '').toUpperCase();
    }
    ...
    function playerMatches(a, b) {
        ...
        if (typeof getCanonicalPlayerName === 'function') {
            const ca = getCanonicalPlayerName(a);
            const cb = getCanonicalPlayerName(b);
            if (key(ca) && key(ca) === key(cb)) return true;
        }
        return key(a) === key(b);
    }
    ...
    function upsertTeam(rawTeam, data = {}) {
        ...
        if (!name) return null;
        const k = key(name);
        const existing = laffTeamIndex.get(k) || { name, grupo: '', origem: '', shortName: '', logo: '', players: [] };
        ...
        existing.shortName = data.shortName !== undefined ? data.shortName : (existing.shortName || '');
    ...
    function upsertPlayer(playerData) {
        ...
        laffTeams.forEach(t => {
            if (!team || key(t.name) !== key(team.name)) {
                t.players = (t.players || []).filter(p => key(p.name) !== pk);
            }
        });
        laffTeamIndex.forEach(t => {
            if (!team || key(t.name) !== key(team.name)) {
                t.players = (t.players || []).filter(p => key(p.name) !== pk);
            }
        });
    ...
    function ingestTeams(rows) {
        (rows || []).forEach(row => {
            ...
            upsertTeam(teamName, {
                ...
                shortName: rowValue(row, ['abreviação', 'abreviacao', 'abreviação mobile', 'abreviacao mobile', 'sigla', 'short', 'short name'])
            });
        });
    }
    ...
    function applyMercadoArrivalsToLAFF() {
        ...
        rows.forEach(row => {
            ...
            const official = ['FECHADO', 'OFICIAL', 'CONFIRMADO', 'CONFIRMADA', 'SAIDA', 'SAÍDA', 'SAIU'].includes(status);
            const origin = String(row.timeOrigem || '').trim();
            ...
            if (!official || !players.length) return;

            if (origin) {
                const oldTeam = getLAFFTeamByName(origin);
                if (oldTeam) oldTeam.players = (oldTeam.players || []).filter(p => !players.some(name => playerMatches(name, p.name)));
            }

            if (!dest || (typeof cffMercadoIsEmptyDestination === 'function' && cffMercadoIsEmptyDestination(dest))) return;
    ...
    function isLAFFStaffRole(role) {
        const n = norm(role);
        return ['STAFF', 'COACH', 'TREINADOR', 'TECNICO', 'TÉCNICO', 'ANALISTA', 'ANALYST', 'MANAGER', 'GESTOR', 'HEAD COACH'].some(term => n.includes(norm(term)));
    }

    function renderLAFFParticipantPerson(player, type = 'player') {
        const flag = getFlagIcon(player.pais || 'br');
        const role = formatLAFFRole(player.funcao || '', true);
        const cls = type === 'staff' ? ' is-staff' : '';
        const label = role && !['JOGA', 'PLAY'].includes(role) ? `<small>${escape(role)}</small>` : '';
        return `<button class="laff-participant-person${cls}" type="button" onclick="openLAFFPlayerProfile(${escape(jsArg(player.name))})" title="Abrir perfil de ${escape(player.name)}">
            ${flag ? `<img src="${escape(flag)}" onerror="this.style.display='none'" alt="">` : ''}
            <span>${escape(player.name)}</span>${label}
        </button>`;
    }

    function renderLAFFParticipantCard(team) {
        const group = String(team.grupo || '').trim();
        const origin = getShortOrigin(team.origem);
        const roster = (team.players || []).filter(p => !isLAFFStaffRole(p.funcao));
        const staff = (team.players || []).filter(p => isLAFFStaffRole(p.funcao));
        const hasStaff = staff.length > 0;
        const meta = [group ? `Grupo ${group}` : '', origin].filter(Boolean).join(' • ');
        return `<article class="laff-participant-card">
            <button class="laff-participant-head" type="button" onclick="openLAFFTeamProfile(${escape(jsArg(team.name))})" title="Abrir perfil de ${escape(team.name)}">
                <span class="laff-participant-logo"><img src="${escape(team.logo || 'escudo.webp')}" onerror="this.onerror=null;this.src='escudo.webp'" alt="${escape(team.name)}"></span>
                <span class="laff-participant-title"><strong>${renderResponsiveTeamName(team)}</strong>${meta ? `<small>${escape(meta)}</small>` : ''}</span>
                <span class="laff-participant-count">${roster.length || 0} J</span>
            </button>
            <div class="laff-participant-roster">
                <div class="laff-participant-subhead"><span>Jogadores</span><small>${roster.length}</small></div>
                <div class="laff-participant-people">
                    ${roster.length ? roster.map(p => renderLAFFParticipantPerson(p, 'player')).join('') : `<div class="laff-participant-empty">Elenco em breve</div>`}
                </div>
                ${hasStaff ? `<div class="laff-participant-subhead is-staff"><span>Staff</span><small>${staff.length}</small></div><div class="laff-participant-people is-staff">${staff.map(p => renderLAFFParticipantPerson(p, 'staff')).join('')}</div>` : ''}
            </div>
        </article>`;
    }

    function renderLAFFParticipantsDirectory(containerId = 'laff-participants-wrap') {
        const wrap = document.getElementById(containerId);
        if (!wrap) return;
        if (!laffTeams.length) { wrap.innerHTML = `<div class="laff-empty">As equipes serão exibidas aqui conforme forem confirmadas.</div>`; return; }
        const q = norm(document.getElementById('laff-participant-search')?.value || '');
        let teams = laffTeams.filter(team => {
            if (!q) return true;
            const playersText = (team.players || []).map(p => `${p.name} ${p.funcao}`).join(' ');
            return norm(`${team.name} ${team.origem} ${team.grupo} ${playersText}`).includes(q);
        });
        if (!teams.length) { wrap.innerHTML = `<div class="laff-empty">Nenhuma equipe encontrada com esse filtro.</div>`; return; }
        const hasGroups = teams.some(t => String(t.grupo || '').trim());
        if (!hasGroups) {
            wrap.innerHTML = `<div class="laff-participants-grid">${teams.map(renderLAFFParticipantCard).join('')}</div>`;
            return;
        }
        const grouped = teams.reduce((acc, team) => { const group = String(team.grupo || '').trim() || 'Sem grupo'; (acc[group] ||= []).push(team); return acc; }, {});
        wrap.innerHTML = Object.keys(grouped).sort((a,b)=>a.localeCompare(b,'pt-BR',{numeric:true})).map(group => `
            <section class="laff-group-block laff-participants-group">
                ...
                <div class="laff-participants-grid">${grouped[group].map(renderLAFFParticipantCard).join('')}</div>
            </section>`).join('');
    }
    ...
    function renderLAFFPlayers() {
        ...
        const teamFilter = document.getElementById('laff-player-team-filter')?.value || 'all';
        let rows = q ? laffPlayers.filter(p => norm(`${p.name} ${p.team} ${p.funcao}`).includes(q)) : laffPlayers;
        if (teamFilter !== 'all') rows = rows.filter(p => key(p.team) === key(teamFilter));
        if (!rows.length) { wrap.innerHTML = `<div class="laff-empty">Nenhum jogador encontrado.</div>`; return; }
        const started = hasLAFFDrops();
        wrap.innerHTML = `<div class="table-container"><table class="laff-players-table laff-table" id="laff-table-players">
            <thead><tr>
                <th>#</th>
                <th style="text-align:left;">Jogador</th>
                <th>Equipe</th>
                <th class="hide-mobile">Função</th>
                ${started ? `<th>K</th><th class="hide-mobile">D</th><th class="hide-mobile">Q</th><th>MVP</th>` : ''}
                <th title="Nota CFF — será atribuída após o início do torneio">Nota CFF</th>
            </tr></thead>
            <tbody>${rows.map((p, i) => {
                const team = getLAFFTeamByName(p.team);
                const tLogo = team?.logo || getTeamLogo(p.team);
                const cur = started ? getLAFFPlayerCurrentStats(p.name) : null;
    ...
    function renderLAFFEquipesPage() {
        ...
        if (!root) return;
        root.innerHTML = `${laffHero('Equipes', 'Elencos confirmados na LAFF')}
            <section class="laff-panel laff-participants-panel">
                <div class="laff-section-head laff-participants-head">
                    ...
                    <div class="laff-participants-tools">
                        <span class="laff-updated">${laffTeams.length} equipe${laffTeams.length === 1 ? '' : 's'}</span>
        ...
        renderLAFFParticipantsDirectory('laff-participants-wrap');
    }
    ...
    function getHistoricalStats(playerName) {
        ...
        const rawEditions = data[entryKey] || {};
        const editions = (typeof cffMergeCurrentEditionIntoEditions === 'function')
            ? cffMergeCurrentEditionIntoEditions(playerName, rawEditions)
            : rawEditions;
    ...
    function buildLAFFTeamPlayerHistory(team) {
        const current = (team.players || []).map(p => ({ ...p, state: 'ATUAL' }));
        const exits = (typeof cffGetTeamMercadoDepartures === 'function') ? cffGetTeamMercadoDepartures(team.name) : [];
        const exPlayers = exits
            .filter(x => !(team.players || []).some(p => playerMatches(p.name, x.jogador)))
            .map(x => ({ name: x.jogador, team: x.equipeDestino || '', photo: getPlayerPhoto(x.jogador), state: 'EX-JOGADOR' }));
        const all = [...current, ...exPlayers];
        ...
        return `<div class="laff-team-history-grid">${all.map(p => {
            ...
            return `<button type="button" class="laff-history-player-card ${p.state === 'ATUAL' ? 'is-current' : 'is-ex'}" onclick="openLAFFPlayerProfile(${escape(jsArg(p.name))})">
                <span class="laff-history-avatar">${logo}<img src="${escape(p.photo || 'silhueta.webp')}" onerror="this.src='silhueta.webp'" alt="${escape(p.name)}"></span>
                <strong>${escape(p.name)}</strong><small>${escape(p.state)}</small>
            </button>`;
        }).join('')}</div>`;
    }

    function buildLAFFTeamHistoricalSummary(team) {
        const canonical = (typeof getTeamCanonicalName === 'function') ? getTeamCanonicalName(team.name) : team.name;
        const results = [
        ...
        const titles = (typeof cffGetTeamTitles === 'function') ? cffGetTeamTitles(canonical) : { collective: [], individual: [] };
        if (!results.length && !titles.collective.length && !titles.individual.length) {
            return `<div class="laff-empty">Desempenho histórico será atualizado conforme os registros da organização forem encontrados.</div>`;
        }
        const best = results
            .map(r => ({ ...r, n: typeof cffPlaceToNumber === 'function' ? cffPlaceToNumber(r.place || r.posicao || r.posição) : null }))
            .filter(r => r.n)
            .sort((a, b) => a.n - b.n)[0];
        return `<div class="grid-cards laff-stats-grid">
            ${statCard('Melhor posição', best?.n ? `${best.n}º` : '-')}
            ${statCard('Resultados', results.length)}
            ${statCard('Títulos coletivos', titles.collective.length)}
            ${statCard('Prêmios individuais', titles.individual.length)}
        </div>`;
    }
    ...
    async function openLAFFTeamProfile(name) {
        ...
        if (typeof window.cffOpenLAFFTeamInTraditionalProfile === 'function') {
            const opened = await window.cffOpenLAFFTeamInTraditionalProfile(team.name);
            if (opened) return;
        }
        setHash(team.name);
    ...
    window.renderLAFFParticipantsDirectory = renderLAFFParticipantsDirectory;
    ...
    window.getLAFFTeamShortName = getLAFFTeamShortName;
