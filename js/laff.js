...
(function () {
    ...
    function buildPlayerHistoryCards(player) {
        ...
        <div class="laff-history-list">${hist.editions.map(ed => `<div class="laff-history-row"><strong>${escape(ed.edition)}</strong><span>${ed.kills} K • ${ed.matches} quedas • média ${ed.avg}</span></div>`).join('')}</div>`;
    }
    ...
    async function openLAFFPlayerProfile(name) {
        ...
        const teamLogo = team?.logo || getTeamLogo(player.team);
        const hist = getHistoricalStats(player.name);
        const bestText = hist.bestEdition ? `Melhor edição: ${hist.bestEdition.edition} com ${hist.bestEdition.kills} abates.` : 'Histórico será atualizado conforme novos registros forem encontrados.';
        root.innerHTML = `<section class="laff-profile-hero laff-player-profile-hero"><div class="laff-player-card-portrait"><img class="laff-profile-player-photo" src="${escape(player.photo || 'silhueta.png')}" onerror="this.onerror=null;this.src='silhueta.png'" alt="${escape(player.name)}"><span>${escape(player.funcao || 'LAFF')}</span></div><div><div class="laff-kicker">Jogador LAFF 2026 Split 1</div><h1>${escape(player.name)}</h1><p>${escape(player.funcao || 'Função em breve')} • ${escape(player.status || 'Confirmado')}</p><button class="btn-action laff-team-mini-link" type="button" onclick="openLAFFTeamProfile(${escape(jsArg(player.team))})"><img src="${escape(teamLogo || 'escudo.webp')}" onerror="this.onerror=null;this.src='escudo.webp'" alt=""> ${escape(player.team || 'Equipe a confirmar')}</button></div></section>
            <section class="laff-panel laff-profile-section"><h2>Resumo histórico</h2><p><strong>${escape(player.name)}</strong> está listado para a disputa da <strong>LAFF 2026 Split 1</strong>${player.team ? ` pela equipe <strong>${escape(player.team)}</strong>` : ''}. ${escape(bestText)}</p></section>
            <section class="laff-panel laff-profile-section"><h2>Números LBFF/WB</h2>${buildPlayerHistoryCards(player)}</section>
            <section class="laff-panel laff-profile-section"><h2>Dados do jogador</h2><div class="grid-cards laff-stats-grid">${statCard('Equipe', player.team || 'Em breve')}${statCard('Função', player.funcao || 'Em breve')}${statCard('País', player.pais || 'Em breve')}${statCard('Status', player.status || 'Confirmado')}</div></section>`;
    }
