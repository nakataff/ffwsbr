...
(function () {
    ...
    function renderPlayerCard(player) {
        ...
        return `<button class="laff-player-card" type="button" onclick="openLAFFPlayerProfile(${escape(jsArg(player.name))})"><img class="laff-player-photo" src="${escape(player.photo || 'silhueta.png')}" onerror="this.onerror=null;this.src='silhueta.png'" alt="${escape(player.name)}"><div class="laff-player-info"><strong>${escape(player.name)}</strong><span><img src="${escape(teamLogo || 'escudo.webp')}" onerror="this.onerror=null;this.src='escudo.webp'" alt=""> ${escape(player.team || 'Equipe a confirmar')}</span><small><span class="laff-role-full">${escape(roleFull)}</span><span class="laff-role-short">${escape(roleShort)}</span></small></div></button>`;
    }
