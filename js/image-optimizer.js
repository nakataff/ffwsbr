/* Central Free Fire - resolvedor seguro de imagens PNG -> WebP
 * Corrige referências legadas como E1.png -> E1.webp e evita fallback para PNG removido.
 */
(function () {
    'use strict';
    ...
    const MAP_URL = 'image-webp-map.json?v=20260601-webp-fix-v2';
    ...
    function resolveImageSrc(src) {
        ...
        const mapped = imageMap[fileName] || imageMap[fileName.toLowerCase()];
        return mapped ? withSamePrefix(src, mapped) : src;
    }

    function fallbackFor(img) {
        const src = fileNameOf(img && img.getAttribute ? img.getAttribute('src') : '').toLowerCase();
        const alt = String((img && img.getAttribute && img.getAttribute('alt')) || '').toLowerCase();
        const cls = String((img && img.className) || '').toLowerCase();
        if (src === 'br.webp' || alt.includes('brasil') || cls.includes('flag') || cls.includes('bandeira')) return 'br.webp';
    ...
    document.addEventListener('error', function (event) {
        ...
        const current = normalizePath(img.getAttribute('src') || '');
        const original = normalizePath(img.dataset && img.dataset.cffOriginalPng || '');

        // Se o arquivo atual já é WebP ou o PNG original foi removido do repositório,
        // não volta para .png. Usa fallback público limpo.
        if (/\.webp$/i.test(current) || /\.png$/i.test(original)) {
            const fallback = fallbackFor(img);
            if (fileNameOf(current) !== fileNameOf(fallback)) {
                img.onerror = null;
                img.src = fallback;
            }
            return;
