...
function getAvailableDaysForFilter(containerId) {
    if (containerId === 'pp-cff-day-filters') {
        ...
        const stage = (typeof cffGetSelectedPpcffStage === 'function') ? cffGetSelectedPpcffStage() : 'geral';
        const jogadoresQuedas = (typeof cffGetStageJogadoresQuedas === 'function') ? cffGetStageJogadoresQuedas(stage) : dbJogadoresQuedas;
        let daysPlayed = new Set();
        for (let d in (jogadoresQuedas || {})) {
            for (let q in (jogadoresQuedas[d] || {})) {
                if ((jogadoresQuedas[d][q] || []).some(p => checkNameMatch(p.nome, currentPlayerView))) {
    ...
    if (containerId === 'tp-day-filters' && currentTeamView) {
        const stage = (typeof cffTeamProfileGetStage === 'function') ? cffTeamProfileGetStage() : 'classificatoria';
        if (typeof cffGetStageQuedas === 'function') {
            const days = Object.keys(cffGetStageQuedas(stage) || {});
            if (days.length) return days.map(Number).sort((a,b)=>a-b).map(String);
        }
        if (db.teamDaily[currentTeamView]) return [...new Set(db.teamDaily[currentTeamView].map(d => d.dia))].sort((a,b)=>a-b).map(String);
    }
