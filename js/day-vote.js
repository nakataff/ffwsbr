...
function getDayFilterArray(containerId) {
    ...
    if (containerId === 'final-day-filters') return selectedFinalDays;
...
function getAvailableDaysForFilter(containerId) {
    ...
    if (containerId === 'team-day-filters') {
        const days = (typeof cffGetDbDaysByStage === 'function') ? cffGetDbDaysByStage('classificatoria') : [];
        if (days.length) return days;
    }

    if (containerId === 'player-day-filters') {
        return (typeof cffGetDbDaysByStage === 'function') ? cffGetDbDaysByStage(selectedMvpStage || 'classificatoria') : [];
    }

    if (containerId === 'avg-day-filters' || containerId === 'total-day-filters' || containerId === 'top5-day-filters') {
        return (typeof cffGetDbDaysByStage === 'function') ? cffGetDbDaysByStage(selectedStatsStage || 'classificatoria') : [];
    }

    if (containerId === 'final-day-filters') {
        return (typeof cffGetFinalDayOptions === 'function') ? cffGetFinalDayOptions() : [];
    }
...
function buildDayFilters() {
    ...
    renderMultiDayFilter('final-day-filters', selectedFinalDays, 'onFinalDayFilterChanged');
...
function onFinalDayFilterChanged() { buildDayFilters(); if (typeof renderFinalStandingsPage === 'function') renderFinalStandingsPage(); }
