...
function cffFindLbffPlayerEntry(playerName) {
    const data = (typeof lbffData !== 'undefined' && lbffData) ? lbffData : {};
    const direct = data[playerName];
    if (direct) return { name: playerName, editions: direct };
    const entry = Object.entries(data).find(([name]) => {
        if (typeof cffPlayerNameMatches === 'function') return cffPlayerNameMatches(name, playerName);
        if (typeof checkNameMatch === 'function') return checkNameMatch(name, playerName);
        return String(name || '').toLowerCase() === String(playerName || '').toLowerCase();
    });
...
function cffHallCurrentTotalsForPlayer(playerName) {
    ...
    let activeP = (typeof db !== 'undefined' && db.players) ? db.players.find(x => String(x.jogador || '').toLowerCase() === String(activeName).toLowerCase() || String(x.jogador || '').toLowerCase() === String(playerName).toLowerCase()) : null;

    const lbffEntry = cffFindLbffPlayerEntry(activeName) || cffFindLbffPlayerEntry(playerName);
    const lbffCurrent = lbffEntry?.editions?.[HALL_CURRENT_EDITION] || lbffEntry?.editions?.[CFF_HALL_CURRENT_EDITION];
    if (lbffCurrent) {
        k = Number(lbffCurrent.k ?? lbffCurrent.kills ?? 0) || 0;
        q = Number(lbffCurrent.q ?? lbffCurrent.quedas ?? lbffCurrent.matches ?? 0) || 0;
        equipe = lbffCurrent.team || lbffCurrent.equipe || activeP?.equipe || null;
        isPlaying = !!activeP && !activeP.isEx;
        return { k, q, equipe, isPlaying, activeName: activeP?.jogador || lbffEntry.name || activeName, activeP };
    }

    // Fallback público: se o lbffData ainda não carregou, usa a base já processada da classificatória.
    if (activeP) {
