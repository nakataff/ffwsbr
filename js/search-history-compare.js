    ...
    return `
        <div onclick="${_safePPAttr(p.jogador)}" style="cursor:pointer; width: 280px; height: 420px; background: #000; border: 3px solid ${accentColor}; border-radius: 15px; position: relative; overflow: hidden; font-family: sans-serif; ${glow} ${marginStyle}">
            ...
            <div style="position: absolute; top: 40px; left: 25px; z-index: 4; text-align: center; color: ${accentColor};">
                ...
                <img src="${flagSrc}" onerror="this.onerror=null;this.src='br.png'" style="width: 35px; border-radius: 2px; display: block; margin: 8px auto;">
...
let __cffLbffDataLoadPromise = null;
async function loadLbffData() {
    if (lbffLoaded) return lbffData; // Se já estiver cheia, não faz nada
    if (__cffLbffDataLoadPromise) return __cffLbffDataLoadPromise;
    __cffLbffDataLoadPromise = (async () => {
    try {
        ...
        __cffLbffDataLoadPromise = null;
    }
    return lbffData;
    })();
    return __cffLbffDataLoadPromise;
}
