...
function renderUnifiedTrophies(name, containerId) {
    ...
    container.innerHTML = allTitles.map(t => {
        ...
        const logoSrc = (typeof getTeamLogoByAliases === 'function') ? getTeamLogoByAliases(t.team) : (logos[t.team] || 'escudo.webp');
...
function checkNameMatch(n1, n2) {
    ...
    const key1 = typeof normalizePlayerAliasKey === 'function' ? normalizePlayerAliasKey(raw1) : raw1.toLowerCase().replace(/[^a-z0-9]/gi, '');
    const key2 = typeof normalizePlayerAliasKey === 'function' ? normalizePlayerAliasKey(raw2) : raw2.toLowerCase().replace(/[^a-z0-9]/gi, '');
    if (key1 && key1 === key2) return true;

    const canon1 = typeof getCanonicalPlayerName === 'function' ? getCanonicalPlayerName(raw1) : (historicalAliases[raw1] || raw1);
    const canon2 = typeof getCanonicalPlayerName === 'function' ? getCanonicalPlayerName(raw2) : (historicalAliases[raw2] || raw2);

    const canonKey1 = typeof normalizePlayerAliasKey === 'function' ? normalizePlayerAliasKey(canon1) : String(canon1).toLowerCase();
    const canonKey2 = typeof normalizePlayerAliasKey === 'function' ? normalizePlayerAliasKey(canon2) : String(canon2).toLowerCase();

    return canonKey1 === canonKey2;
}
...
function filterHallByTeam(rows, searchValue) {
    ...
    return rows.filter(row => normalizeHallText(getHallTeamSearchValue(row)).includes(query));
}
...
function renderEditionRanking() {
    ...
    results = filterHallByTeam(results, hallEditionTeamSearch);
