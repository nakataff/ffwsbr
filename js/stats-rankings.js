...
function populateStatsPlayerDaySelect() {
    const select = document.getElementById('filter-player-day');
    if (!select) return;
    const previous = select.value || 'all';
    const days = (typeof cffGetDbDaysByStage === 'function') ? cffGetDbDaysByStage(selectedStatsStage || 'classificatoria') : [];
    select.innerHTML = '<option value="all">Todos os Dias</option>' + days.map(d => `<option value="${d}">Dia ${d}</option>`).join('');
    select.value = days.includes(previous) ? previous : 'all';
}

function onStatsStageFilterChanged() {
    const select = document.getElementById('stats-stage-filter');
    selectedStatsStage = (typeof cffNormalizeStage === 'function') ? cffNormalizeStage(select?.value || 'classificatoria') : (select?.value || 'classificatoria');

    selectedTop5Days = [];
    selectedAvgDays = [];
    selectedTotalDays = [];

    ['avg-conf-filter', 'total-conf-filter', 'filter-player-conf', 'filter-player-day'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = 'all';
        if (window._genericMultiSelectFilters) window._genericMultiSelectFilters[id] = new Set();
    });

    populateStatsPlayerDaySelect();
...
function renderHistoricoEquipes(nome, containerId) {
    ...
    container.innerHTML = passagensArray.map((passagem, index) => {
        ...
        let clickAction = isClickable ? `onclick="openTeamProfile(${JSON.stringify(resolvedEquipe).replace(/&/g, '&amp;').replace(/\"/g, '&quot;')})"` : '';
...
function renderPlayerStats() {
    const stageSelect = document.getElementById('stats-stage-filter');
    selectedStatsStage = (typeof cffNormalizeStage === 'function') ? cffNormalizeStage(stageSelect?.value || selectedStatsStage || 'classificatoria') : (stageSelect?.value || selectedStatsStage || 'classificatoria');
    populateStatsPlayerDaySelect();
    ...
    for (let d in dbJogadoresQuedas) {
        if ((typeof cffIsDayInStage === 'function') && !cffIsDayInStage(d, selectedStatsStage)) continue;
...
function getAggregatedTeamsForStats(daysFilter, mapFilter) {
    if ((!Array.isArray(daysFilter) || daysFilter.length === 0) && typeof cffGetDbDaysByStage === 'function') {
        daysFilter = cffGetDbDaysByStage(selectedStatsStage || 'classificatoria');
    }
    let teamsMapStats = {};
    db.teams.forEach(t => {
