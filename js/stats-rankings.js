...
async function openTeamProfile(teamName) {
    ...
    // Histórico de jogadores é carregado sob demanda pela própria seção.
    // Não bloqueia mais a abertura da página da equipe.
    if (typeof loadPlayerAliases === 'function') {
        try { await loadPlayerAliases(); } catch (e) { console.warn('[TeamProfile] aliases de jogadores não carregaram:', e); }
    }
    if (typeof loadMercado === 'function') {
        try {
            if (typeof cffLoadMercadoNonBlocking === 'function') await cffLoadMercadoNonBlocking();
            else await loadMercado({ silent: true });
        } catch (e) { console.warn('[TeamProfile] Mercado não carregou antes do perfil:', e); }
    }
    ...
    // Normaliza nomes históricos para a organização atual/canônica.
    // Ex.: Keyd/Keyd Stars -> VIVO KEYD, LOUD -> LOUD SNICKERS.
    if (typeof getTeamCanonicalName === 'function') {
        teamName = getTeamCanonicalName(teamName);
    }

    currentTeamView = teamName;
    selectedTpDays = [];
    if (typeof cffTeamProfileStage !== 'undefined') cffTeamProfileStage = 'geral';
    if (typeof cffTeamChartStage !== 'undefined') cffTeamChartStage = 'classificatoria';
    const tpStageSelect = document.getElementById('tp-stage-filter');
    if (tpStageSelect) tpStageSelect.value = 'geral';
    const chartStageSelect = document.getElementById('chart-stage');
    if (chartStageSelect) chartStageSelect.value = 'classificatoria';
    ...
    const finalRow = (typeof cffTeamProfileFinalRow === 'function') ? cffTeamProfileFinalRow(t.equipe || teamName) : null;
    ...
    document.getElementById('tp-pos').innerText = finalRow ? `${finalRow.rank}º` : ((t.posGeral === "-") ? "N/A" : `${t.posGeral}º`);
    if (typeof cffTeamProfileEnsureFinalSummary === 'function') cffTeamProfileEnsureFinalSummary(t, finalRow);
    ...
    if (typeof renderTeamHistorySummary === 'function') renderTeamHistorySummary(t.equipe || teamName);
    if (typeof renderTeamPlayersHistory === 'function') renderTeamPlayersHistory(t.equipe || teamName);
...
function renderHistoricoEquipes(nome, containerId) {
    ...
    // O histórico de equipes depende da planilha de passagens. Como ela deixou
    // de carregar automaticamente para não travar páginas de times, o perfil do
    // jogador precisa pedir esse dado sob demanda e redesenhar o bloco ao terminar.
    const hasPassagensLoaded = typeof dbPassagens !== 'undefined' && Array.isArray(dbPassagens) && dbPassagens.length > 0;
    if (!hasPassagensLoaded && typeof loadPassagens === 'function') {
        window.__cffPlayerHistoryPassagensPromise = window.__cffPlayerHistoryPassagensPromise || Promise.resolve()
            .then(() => (typeof loadPlayerAliases === 'function' ? loadPlayerAliases() : null))
            .catch(() => null)
            .then(() => loadPassagens())
            .catch(err => {
                console.warn('[PlayerProfile] Histórico de equipes não carregou:', err);
                return null;
            })
            .finally(() => {
                window.__cffPlayerHistoryPassagensPromise = null;
            });

        if (!container.dataset.cffHistoryRefreshQueued) {
            container.dataset.cffHistoryRefreshQueued = '1';
            window.__cffPlayerHistoryPassagensPromise.then(() => {
                delete container.dataset.cffHistoryRefreshQueued;
                renderHistoricoEquipes(nome, containerId);
            });
        }
    }

    // Busca o histórico usando player-aliases.json, não nome exato.
    let hist = typeof dbPassagens !== 'undefined' && Array.isArray(dbPassagens) ? dbPassagens.find(p => (typeof checkNameMatch === 'function' ? checkNameMatch(p.jogador, cleanName) : String(p.jogador || '').toLowerCase() === String(cleanName || '').toLowerCase())) : null;
...
function cffRefreshPlayerStatsFiltersForStage() {
    const stage = cffStatsStageValue();
    const jogadores = cffStatsJogadoresQuedas();
    const daySelect = document.getElementById('filter-player-day');
    if (daySelect) {
        const days = Object.keys(jogadores || {}).map(Number).filter(n => !Number.isNaN(n)).sort((a, b) => a - b);
        const current = daySelect.value || 'all';
        daySelect.innerHTML = '<option value="all">Todos os Dias</option>' + days.map(d => `<option value="${d}">Dia ${d}</option>`).join('');
        daySelect.value = days.map(String).includes(String(current)) ? current : 'all';
    }

    const teamSelect = document.getElementById('filter-player-team');
    if (teamSelect) {
        const currentTeam = teamSelect.value || 'all';
        const teams = new Set();
        Object.values(jogadores || {}).forEach(day => {
            Object.values(day || {}).forEach(drop => {
                (drop || []).forEach(p => { if (p.equipe) teams.add(p.equipe); });
            });
        });
        Array.from(db.teams || []).forEach(t => { if (t.equipe) teams.add(t.equipe); });
        const sortedTeams = Array.from(teams).sort((a, b) => a.localeCompare(b));
        teamSelect.innerHTML = '<option value="all">Todas as Equipes</option>' + sortedTeams.map(team => `<option value="${team}">${team}</option>`).join('');
        teamSelect.value = sortedTeams.includes(currentTeam) ? currentTeam : 'all';
    }

    if (typeof resetGenericMultiState === 'function') {
        // Não existe em todas as versões. Se existir no futuro, ele pode limpar estados antigos.
    }
}

function renderPlayerStats() {
    cffRefreshPlayerStatsFiltersForStage();
    if (typeof cffSyncStatsStageControls === 'function') cffSyncStatsStageControls(cffStatsStageValue());
