...
async function openTeamProfile(teamName) {
    ...
    if (typeof loadMercado === 'function') {
        try { await loadMercado(); } catch (e) { console.warn('[TeamProfile] Mercado não carregou antes do perfil:', e); }
    }
...
function renderHistoricoEquipes(nome, containerId) {
    ...
    if (activePlayer && !activePlayer.isEx && !(typeof cffPlayerHasLeftTeam === 'function' && cffPlayerHasLeftTeam(activePlayer.jogador, activePlayer.equipe))) {
