...
function renderPlayerStats() {
    ...
    let novatoFilter = document.getElementById('filter-player-novato')?.value || 'all';
    ...
    for (let d in dbJogadoresQuedas) {
        ...
        for (let q in dbJogadoresQuedas[d]) {
            ...
            playersInDrop.forEach(p => {
                ...
                if (novatoFilter === 'novatos' && !(typeof isNovatoPlayer === 'function' && isNovatoPlayer(p.nome))) return;
...
function renderTop5Stats() {
    ...
    let validNovatos = validPlayers.filter(p => typeof isNovatoPlayer === 'function' && isNovatoPlayer(p.jogador));
    ...
    document.getElementById('top5-container').innerHTML = `
        ...
        <div class="grid-cards" style="grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
            <div class="card"><div class="card-top-border"></div><h3>Kills</h3>${getPlayerHtml('pl-novatos-kills', validNovatos, p=>p.kills, v=>v, "Kills")}</div>
            <div class="card"><div class="card-top-border"></div><h3>Dano</h3>${getPlayerHtml('pl-novatos-dano', validNovatos, p=>p.dano, v=>v, "Dano")}</div>
            <div class="card"><div class="card-top-border"></div><h3>Assistência</h3>${getPlayerHtml('pl-novatos-assists', validNovatos, p=>p.assists, v=>v, "Ast")}</div>
        </div>
    `;
}
