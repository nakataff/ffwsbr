...
function formatChartSummaryValue(value, unit) {
    if (value === null || value === undefined || value === '') return '-';
    const n = Number(value);
    const formatted = Number.isFinite(n) ? (Number.isInteger(n) ? String(n) : n.toFixed(2)) : String(value);
    return unit ? (unit === 'º' ? `${formatted}${unit}` : `${formatted} ${unit}`) : formatted;
}
        ...
        <div class="chart-summary-list">
            ${rows.map(item => `
                <div class="chart-summary-item">
                    ...
                    <span class="chart-summary-value">${safe(formatChartSummaryValue(item.value, item.unit))}</span>
                </div>`).join('')}
        </div>`;
}
...
function renderTeamChart() {
    ...
    if (typeof updateChartSummary === 'function') {
        updateChartSummary(
            ...
            labels.map((label, index) => ({ label, value: dataset[index], unit: metric.includes('pontos') ? 'PTS' : (metric.includes('kills') ? 'K' : '') }))
        );
    }
}
