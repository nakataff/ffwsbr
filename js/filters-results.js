...
function cffTeamChartNormTeam(name) {
    if (typeof normalizeTeamAlias === 'function') return normalizeTeamAlias(name);
    return String(name || '').trim().toUpperCase();
}

function cffTeamChartSameTeam(a, b) {
    if (typeof sameTeamName === 'function') return sameTeamName(a, b);
    return cffTeamChartNormTeam(a) === cffTeamChartNormTeam(b);
}

function cffTeamChartStageDrops(stage) {
    if (typeof cffGetStageQuedas === 'function') return cffGetStageQuedas(stage);
    return dbQuedas || {};
}

function cffTeamChartAggregateUpToDay(stage, limitDay, teamName) {
    const drops = cffTeamChartStageDrops(stage);
    const rows = {};
    Object.keys(drops || {}).sort((a,b)=>Number(a)-Number(b)).forEach(day => {
        if (Number(day) > Number(limitDay)) return;
        Object.keys(drops[day] || {}).sort((a,b)=>Number(a)-Number(b)).forEach(round => {
            (drops[day][round]?.resultados || []).forEach(res => {
                const key = cffTeamChartNormTeam(res.equipe);
                if (!rows[key]) rows[key] = { equipe: res.equipe, pontos: 0, abates: 0, booyah: 0, quedas: 0 };
    ...
    const ordered = Object.values(rows).sort((a,b)=>b.pontos-a.pontos || b.booyah-a.booyah || b.abates-a.abates);
    const index = ordered.findIndex(t => cffTeamChartSameTeam(t.equipe, teamName));
    return index >= 0 ? index + 1 : null;
}

function cffTeamChartDailyRows(stage, teamName) {
    const drops = cffTeamChartStageDrops(stage);
    const selectedDays = (typeof selectedTpDays !== 'undefined' ? selectedTpDays : []).map(String);
    const out = [];
    Object.keys(drops || {}).sort((a,b)=>Number(a)-Number(b)).forEach(day => {
        if (selectedDays.length && !selectedDays.includes(String(day))) return;
        let row = { dia: Number(day), pontos: 0, abates: 0, booyah: 0, quedas: 0 };
        Object.keys(drops[day] || {}).sort((a,b)=>Number(a)-Number(b)).forEach(round => {
            const res = (drops[day][round]?.resultados || []).find(r => cffTeamChartSameTeam(r.equipe, teamName));
            if (!res) return;
            row.pontos += (Number(posPoints?.[res.posicao]) || 0) + (Number(res.kills) || 0);
            row.abates += Number(res.kills) || 0;
            row.booyah += Number(res.booyah) || 0;
            row.quedas += 1;
        });
        if (row.quedas) out.push(row);
    });
    return out;
}
...
function renderTeamChart() {
    ...
    let metric = document.getElementById('chart-metric')?.value || 'pos_geral_dia';
    let chartStage = document.getElementById('chart-stage')?.value || (typeof cffTeamChartStage !== 'undefined' ? cffTeamChartStage : 'classificatoria');
    chartStage = chartStage === 'final' ? 'final' : 'classificatoria';
    ...
    const drops = cffTeamChartStageDrops(chartStage);
    ...
    if (!drops || !Object.keys(drops).length) {
        ...
        if (typeof updateChartSummary === 'function') updateChartSummary('team-chart-summary', 'Sem dados', []);
        return;
    }

    if (metric.includes('dia')) {
        const dailyStats = cffTeamChartDailyRows(chartStage, tName);
        dailyStats.forEach(d => {
            labels.push(`${chartStage === 'final' ? 'Final D' : 'Dia '}${d.dia}`);
            if (metric === 'pontos_dia') {
                dataset.push(d.pontos);
                chartLabel = chartStage === 'final' ? 'Pontos por Dia da Final' : 'Pontos Conquistados no Dia';
                chartColor = '#ffd700';
            } else if (metric === 'kills_dia') {
                dataset.push(d.abates);
                chartLabel = chartStage === 'final' ? 'Abates por Dia da Final' : 'Abates no Dia';
                chartColor = '#ff4444';
            } else if (metric === 'pos_geral_dia') {
                dataset.push(cffTeamChartAggregateUpToDay(chartStage, d.dia, tName));
                chartLabel = chartStage === 'final' ? 'Posição Geral na Final ao Fim do Dia' : 'Posição na Tabela Geral ao Fim do Dia';
                chartColor = '#00c8ff';
                isReverse = true;
            }
        });
        ...
        const selectedDays = (typeof selectedTpDays !== 'undefined' ? selectedTpDays : []).map(String);
        Object.keys(drops || {}).sort((a,b)=>Number(a)-Number(b)).forEach(d => {
            if (selectedDays.length && !selectedDays.includes(String(d))) return;
            Object.keys(drops[d] || {}).sort((a,b)=>Number(a)-Number(b)).forEach(q => {
                let drop = drops[d][q];
                let res = (drop.resultados || []).find(r => cffTeamChartSameTeam(r.equipe, tName));
                if (res) {
                    labels.push(`${chartStage === 'final' ? 'F' : 'D'}${d}-Q${q}`);
                    if (metric === 'pos_queda') {
                        dataset.push(res.posicao);
                        chartLabel = 'Posição na Queda';
                        chartColor = '#4caf50';
                        isReverse = true;
                        ...
                        let pPos = posPoints[res.posicao] || 0;
                        dataset.push(pPos + (Number(res.kills) || 0));
                        chartLabel = 'Pontos na Queda';
                        chartColor = '#ffd700';
                    }
                }
            });
    ...
    teamChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            layout: { padding: { top: 30, right: 10, bottom: 6, left: 10 } },
            scales: {
                y: {
                    reverse: isReverse,
                    min: isReverse ? 1 : 0,
                    max: isReverse ? (metric === 'pos_queda' ? 12 : undefined) : undefined,
