...
function getCurrentWBFinalResultForTeam(teamName) {
    if (typeof db === 'undefined' || !Array.isArray(db.teams)) return null;
    ...
    const idx = sorted.findIndex(t => normalize(t.equipe) === target);
    ...
    return {
        ...
        tier: 'S-Tier',
        place: `${pos}º`,
        torneio: 'FFWS BR 2026 S1',
        isCurrentWBResult: true,
        pontos: Number(team.pontos) || 0
    };
}

function addCurrentWBFinalResultIfNeeded(results, teamName) {
    const wbResult = getCurrentWBFinalResultForTeam(teamName);
    if (!wbResult) return results;
    const alreadyExists = results.some(r => {
        const torneio = String(r.torneio || '').toUpperCase();
        return torneio.includes('FFWS BR 2026') || torneio.includes('WB 2026');
    });
    return alreadyExists ? results : [wbResult, ...results];
}
...
function renderTeamResults() {
    ...
    results = addCurrentWBFinalResultIfNeeded(results, tName);
    ...
    tbody.innerHTML = results.map(r => {
        ...
        let rowClass = r.isCurrentWBResult ? 'row-current-wb-result' : (isSTier ? 'row-tier-s' : '');
        let tierClass = r.tier.toLowerCase().split('-')[0];

        let placeBR = String(r.place || '-').replace(/st|nd|rd|th/ig, 'º');
