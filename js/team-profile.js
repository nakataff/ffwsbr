...
function cffFindCurrentPlayerInfo(playerName) {
    ...
    const latestTeam = cffGetLatestActiveTeamForPlayer(active.jogador, active.equipe);
    return { player: active, equipe: latestTeam || active.equipe };
}
...
function renderPlayerHistorySummary(playerName, context = {}) {
    ...
    const lbffDebut = editions.length ? { tournament: editions[0][0], pos: 0, team: '' } : null;
    ...
    const sentences = [];
    if (debut) {
        sentences.push(`<strong>${cffEscapeHTML(displayName)}</strong> estreou na <strong>${cffEscapeHTML(debut.tournament)}</strong>${debut.pos ? ` e ficou na <strong>${debut.pos}ª posição</strong>` : ''}${debut.team ? ` com <strong>${cffEscapeHTML(debut.team)}</strong>` : ''}.`);
    }
    if (bestKill) {
    ...
    if (bestPlace) {
        sentences.push(`Sua melhor colocação como equipe foi <strong>${cffEscapeHTML(cffOrdinalBR(bestPlace.pos))}</strong> na <strong>${cffEscapeHTML(bestPlace.tournament)}</strong>.`);
    }
    ...
    sentences.push(`<strong>${cffEscapeHTML(displayName)}</strong> já conquistou <strong>${titles.collective.length} título(s) coletivo(s)</strong> e <strong>${titles.individual.length} título(s) individual(is)</strong>.`);
