...
if (cardsContainer) {
        ...
        if(tbody) {
            tbody.innerHTML = playersList.sort((a,b) => b.abates - a.abates).map(p => `<tr><td style="text-align:left;"><span class="clickable" onclick="${_safePPAttr(p.jogador)}">${p.jogador}</span></td><td style="color:var(--accent); font-weight:bold;">${p.abates}</td><td data-sort-value="${p.dano}">${cffFormatCompactStat(p.dano)}</td><td>${p.assists}</td><td>${p.quedas}</td><td>${p.mvp || 0}</td></tr>`).join('');
        }
...
function cffBuildInactivePlayerHero(playerName, lastEd) {
    ...
    return `
        <div class="cff-inactive-player-hero">
            ...
            <div class="cff-inactive-player-status">INATIVO</div>
...
let officialName = cffCanonicalPlayerDisplayName(safeName);
    ...
    if (playerEntries) {
        ...
        let editions = (typeof cffSortEditions === 'function')
            ? cffSortEditions(Object.entries(editionsData)).map(([ed]) => ed)
            : Object.keys(editionsData);
...
function renderPlayerHistorySummary(playerName, context = {}) {
    ...
    const editions = lbff ? cffSortEditions(Object.entries(lbff.editions || {})) : [];
    ...
    const activeInfo = cffFindCurrentPlayerInfo(playerName);
    const activePlayer = activeInfo?.player || null;
    if (activePlayer) {
        totalKills += Number(activePlayer.abates || 0);
        totalMatches += Number(activePlayer.quedas || 0);
    }

    // Estreia e última participação priorizam lbffData, porque é a fonte mais direta para jogador.
    ...
    const bestPlaceTitle = bestPlace ? cffFindPlayerCollectiveTitleForEvent(playerName, bestPlace.tournament) : null;
    const currentTeam = activeInfo?.equipe || activePlayer?.equipe || context.currentTeam || '';
    const currentText = activePlayer
        ? `Atualmente defende <strong>${cffEscapeHTML(currentTeam)}</strong>.`
        : `Atualmente não está jogando por nenhum time na WB; sua última participação registrada foi em <strong>${cffEscapeHTML(last?.tournament || context.lastEdition || 'N/A')}</strong>.`;

    const sentences = [];
    if (debut) {
        sentences.push(`<strong>${cffEscapeHTML(displayName)}</strong> estreou na <strong>${cffEscapeHTML(debut.tournament)}</strong>${debut.team ? ` pela equipe <strong>${cffEscapeHTML(debut.team)}</strong>` : ''}${debut.pos ? ` e ficou na <strong>${debut.pos}ª posição</strong>` : ''}.`);
    }
    if (bestKill) {
        sentences.push(`Seu melhor desempenho individual foi de <strong>${bestKill.kills} abates</strong> na <strong>${cffEscapeHTML(bestKill.edition)}</strong>.`);
    }
    if (bestPlace) {
        const bestTeam = bestPlace.team || cffGetTitleTeamName(bestPlaceTitle);
        sentences.push(`Sua melhor colocação como equipe foi <strong>${cffEscapeHTML(cffOrdinalBR(bestPlace.pos))}</strong> na <strong>${cffEscapeHTML(bestPlace.tournament)}</strong>${bestTeam ? ` atuando pelo time <strong>${cffEscapeHTML(bestTeam)}</strong>` : ''}.`);
    }
    sentences.push(`<strong>${cffEscapeHTML(displayName)}</strong> soma <strong>${totalKills} abates</strong> em <strong>${totalMatches} quedas</strong> na história da LBFF/WB.`);
    sentences.push(currentText);
    sentences.push(cffTitleCountsSentence(displayName, titles.collective.length, titles.individual.length));
    ...
    box.innerHTML = `
        ...
        <div class="cff-summary-mini-grid">
            ...
            <span><strong>${bestPlace?.pos ? bestPlace.pos + 'º' : '-'}</strong><small>melhor posição</small></span>
...
function cffGetPlayersFromTeamPassagens(teamName) {
    ...
    return dbPassagens.map(row => {
        ...
        const activeInfo = cffFindCurrentPlayerInfo(row.jogador);
        ...
        const isCurrent = !!activeInfo && cffTeamMatchesHistoryTeam(activeTeam, canonical);
...
async function renderTeamPlayersHistory(teamName) {
    ...
    // Garante que a planilha de fotos tenha chance de carregar antes de montar os cards.
    ...
    const inactiveCount = players.length - currentCount;
    const topPlayers = players.slice(0, 48);
    ...
    section.innerHTML = `
        <div class="team-player-history-head">
            ...
            <div class="team-player-history-stats">
                ...
                <span><strong>${inactiveCount}</strong><small>inativos</small></span>
            </div>
        </div>
        <div class="team-player-history-grid team-player-history-photo-grid">
            ${topPlayers.map(p => {
                ...
                const status = p.isCurrent ? 'ATUAL' : 'INATIVO';
                return `
                    <button type="button" class="team-player-history-card team-player-history-photo-card${p.isCurrent ? ' is-current' : ' is-inactive'}" onclick="${_safePPAttr(p.jogador)}" title="Abrir perfil de ${p.displayName}">
                        <span class="team-player-history-avatar">
                            <img src="${photo}" alt="${p.displayName}" onerror="this.src='silhueta.png'">
                        </span>
