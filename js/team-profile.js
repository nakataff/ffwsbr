...
async function openPlayerProfile(playerName) {
    ...
    if (typeof loadPhotos === 'function' && !window.__cffPlayerProfilePhotosLoaded) {
        window.__cffPlayerProfilePhotosLoaded = true;
        try { await loadPhotos(); } catch (e) { console.warn('[openPlayerProfile] fotos não carregaram:', e); }
    }
...
let officialName = cffCanonicalPlayerDisplayName(safeName);
    ...
    const ppPhotoEl = document.getElementById('pp-photo');
    if (ppPhotoEl) {
        const photoName = p?.jogador || safeName;
        ppPhotoEl.src = (typeof cffResolvePlayerHistoryPhoto === 'function')
            ? cffResolvePlayerHistoryPhoto(photoName)
            : 'silhueta.png';
    }
    const ppStatusLabel = document.getElementById('pp-player-status-label');
    if (ppStatusLabel) {
        ppStatusLabel.textContent = isInactive ? 'INATIVO' : 'ATIVO';
    }
    ...
    if (playerEntries) {
        ...
        let editions = cffSortLbffEditions(Object.entries(editionsData)).map(([edition]) => edition);
    ...
    if (isInactive) {
        ...
        // A informação de INATIVO agora fica limpa abaixo da foto do jogador.
...
function cffGetEditionSortNumber(label) {
    const match = String(label || '').match(/(\d+)/);
    return match ? Number(match[1]) : 9999;
}

function cffSortLbffEditions(entries) {
    return (entries || []).slice().sort((a, b) => {
        const na = cffGetEditionSortNumber(a[0]);
        const nb = cffGetEditionSortNumber(b[0]);
        if (na !== nb) return na - nb;
        return String(a[0]).localeCompare(String(b[0]), 'pt-BR');
    });
}

function cffGetLbffDebut(playerName, lbffEntry, appearances = []) {
    const editions = cffSortLbffEditions(Object.entries(lbffEntry?.editions || {}));
    if (!editions.length) return null;

    const firstEdition = editions[0][0];
    const appearance = (appearances || []).find(a =>
        String(a.tournament || '').toLowerCase() === String(firstEdition || '').toLowerCase()
        || String(a.id || '').toLowerCase() === String(firstEdition || '').toLowerCase()
    );

    return {
        tournament: firstEdition,
        pos: appearance?.pos || 0,
...
function renderPlayerHistorySummary(playerName, context = {}) {
    ...
    const editions = lbff ? cffSortLbffEditions(Object.entries(lbff.editions || {})) : [];
    ...
    // A estreia vem primeiro do lbffData.json, que é a fonte mais confiável para primeira edição jogada.
    // Isso corrige casos como Nobru, que está no lbffData desde a LBFF 1, mesmo que outro JSON só tenha elenco a partir da LBFF 3.
    const debut = cffGetLbffDebut(playerName, lbff, appearances) || appearances[0] || (editions.length ? { tournament: editions[0][0], pos: 0, team: '' } : null);
