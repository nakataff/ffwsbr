...
function renderAllPlayers() {
    ...
    let novatoSelect = document.getElementById('filter-novato-players');
    let novatoFilter = novatoSelect ? novatoSelect.value : 'all';
    ...
    setTimeout(() => {
        ...
        let data = Object.values(aggregated)
            ...
            .filter(p => {
                if (novatoFilter !== 'novatos') return true;
                return typeof isNovatoPlayer === 'function' && isNovatoPlayer(p.jogador);
            })
            .filter(p => p.quedas > 0);
