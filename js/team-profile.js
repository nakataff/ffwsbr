...
async function renderTeamPlayersHistory(teamName) {
    ...
    section.innerHTML = `
        ...
        <div class="team-player-history-empty">Carregando histórico de jogadores...</div>`;
    ...
    if (typeof loadPhotos === 'function' && !window.__cffTeamHistoryPhotosLoaded) {
        ...
        try {
            Promise.resolve(loadPhotos()).then(() => {
                ...
                const same = current && (typeof sameTeamName === 'function' ? sameTeamName(current, teamName) : current.toUpperCase() === String(teamName || '').toUpperCase());
                if (same) renderTeamPlayersHistory(teamName);
    ...
    const playerCardsHtml = topPlayers.map(p => {
        ...
        const logoHtml = p.activeTeamLogo ? `
            <span class="team-player-current-team-logo" title="Atualmente em ${cffEscapeHTML(p.activeTeam)}">
                <img src="${cffEscapeHTML(p.activeTeamLogo)}" alt="${cffEscapeHTML(p.activeTeam)}" loading="lazy" decoding="async" onerror="this.style.display='none'">
            </span>` : '';
        return `
            <button type="button" class="team-player-history-card team-player-history-photo-card${stateClass}" onclick="${_safePPAttr(p.jogador)}" title="Abrir perfil de ${cffEscapeHTML(p.displayName)}">
                <span class="team-player-history-avatar">
                    ${logoHtml}
                    <img src="${cffEscapeHTML(photo)}" alt="${cffEscapeHTML(p.displayName)}" loading="lazy" decoding="async" onerror="this.src='silhueta.png'">
                </span>
    ...
    const notableHtml = notablePlayers.map((p, index) => {
        ...
        return `
            <button type="button" class="team-notable-player-card" onclick="${_safePPAttr(p.jogador)}" title="Abrir perfil de ${cffEscapeHTML(p.displayName)}">
                ...
                <span class="team-notable-avatar"><img src="${cffEscapeHTML(photo)}" alt="${cffEscapeHTML(p.displayName)}" loading="lazy" decoding="async" onerror="this.src='silhueta.png'"></span>
