...
function cffTeamProfileNormalizeKey(name) {
...
function cffTeamProfileMatchesTeam(a, b) {
    if (!a || !b) return false;
    if (typeof sameTeamName === 'function' && sameTeamName(a, b)) return true;
    const aSet = (typeof getTeamAliasSet === 'function') ? getTeamAliasSet(a) : new Set([cffTeamProfileNormalizeKey(a)]);
    const bSet = (typeof getTeamAliasSet === 'function') ? getTeamAliasSet(b) : new Set([cffTeamProfileNormalizeKey(b)]);
    return [...aSet].some(alias => bSet.has(alias));
}

function cffGetFinalTeamAggregateRowsForProfiles() {
    const drops = (typeof dbFinalQuedas !== 'undefined') ? dbFinalQuedas : {};
    ...
    Object.keys(drops).sort((a, b) => Number(a) - Number(b)).forEach(day => {
        Object.keys(drops[day] || {}).sort((a, b) => Number(a) - Number(b)).forEach(round => {
            ...
            (drop.resultados || []).forEach(res => {
                const team = String(res.equipe || '').trim();
                if (!team) return;
                if (!rows[team]) rows[team] = { equipe: team, pontos: 0, abates: 0, booyah: 0, quedas: 0 };
    ...
    const champion = (typeof cffFinalSimulateChampion === 'function') ? cffFinalSimulateChampion(drops) : null;
    const championName = champion?.equipe || '';
    let list = Object.values(rows).sort((a, b) => b.pontos - a.pontos || b.abates - a.abates || b.booyah - a.booyah);

    if (championName) {
        const champRow = list.find(row => cffTeamProfileMatchesTeam(row.equipe, championName));
        if (champRow) list = [champRow, ...list.filter(row => !cffTeamProfileMatchesTeam(row.equipe, championName))];
    }

    return list.map((row, index) => ({
        ...row,
        rank: index + 1,
        champion: !!championName && cffTeamProfileMatchesTeam(row.equipe, championName)
    }));
}

function cffGetTeamFinalProfileInfo(teamName) {
    const rows = cffGetFinalTeamAggregateRowsForProfiles();
    return rows.find(row => cffTeamProfileMatchesTeam(row.equipe, teamName)) || null;
}

function cffRenderTeamFinalStatus(teamName, finalInfo) {
    const groupEl = document.getElementById('tp-group');
    if (!groupEl) return;

    let status = document.getElementById('tp-final-status');
    if (!status) {
        status = document.createElement('div');
        status.id = 'tp-final-status';
        status.style.marginTop = '8px';
        status.style.display = 'none';
        groupEl.insertAdjacentElement('afterend', status);
    }
    ...
    const label = finalInfo.champion
        ? 'ATUAL CAMPEÃ DA FFWS BRASIL 2026 SPLIT 1'
        : `FINAL FFWS BRASIL 2026 S1 • ${finalInfo.rank}º LUGAR`;
    const border = finalInfo.champion ? 'rgba(255,215,0,.65)' : 'rgba(74,168,255,.45)';
    const color = finalInfo.champion ? '#ffd700' : '#4aa8ff';

    status.style.display = 'block';
    status.innerHTML = `
        <div style="display:inline-flex; align-items:center; justify-content:center; gap:8px; flex-wrap:wrap; padding:7px 11px; border:1px solid ${border}; border-radius:999px; background:rgba(255,255,255,.04); color:${color}; font-size:.72rem; font-weight:950; letter-spacing:.6px; text-transform:uppercase;">
            <span>${finalInfo.champion ? '🏆' : '🏁'}</span>
            <span>${label}</span>
        </div>
        <div style="margin-top:5px; color:var(--text-muted); font-size:.78rem;">
            Final: ${finalInfo.pontos} pts • ${finalInfo.abates} K • ${finalInfo.booyah} B! • ${finalInfo.quedas} quedas
        </div>`;
}

function cffTeamTrophyDedupeKey(t) {
    return [t.event || t.torneio || '', t.year || '', t.type || '', t.team || '']
        .map(v => String(v || '').trim().toUpperCase())
        .join('|');
}

function cffDeduplicateTeamTrophies(list) {
    const seen = new Set();
    return (Array.isArray(list) ? list : []).filter(item => {
        const key = cffTeamTrophyDedupeKey(item);
...
function cffAwardFamilyForTeamTrophy(type) {
    const raw = String(type || '').trim();
    const upper = raw.toUpperCase();
    if ((upper.includes('MVP') || upper.includes('MVA')) && upper.includes('DIA')) return 'MVP/MVA de Dia';
    if (upper.includes('BRABO')) return 'Brabo da Galera';
    if (upper.includes('SELEÇÃO')) return raw.replace(/\s+DIA\s*\d+$/i, '').trim() || 'Seleção';
    return raw.replace(/\s+DIA\s*\d+$/i, '').trim() || 'Prêmio individual';
}

function cffCollapseTeamIndividualAwards(list) {
    const groups = new Map();
    (Array.isArray(list) ? list : []).forEach(item => {
        const family = cffAwardFamilyForTeamTrophy(item.type || item.award || item.premio || '');
        const key = [item.event || '', item.year || '', item.team || '', item.player || '', family]
            .map(v => String(v || '').trim().toUpperCase())
            .join('|');
        if (!groups.has(key)) groups.set(key, { ...item, type: family, _count: 0 });
    ...
    return Array.from(groups.values()).map(item => {
        ...
        if (count > 1) cleaned.type = `${cleaned.type} (${count}x)`;
        return cleaned;
    });
}
...
function renderTeamProfileStats() {
    ...
    if (isHist) {
        ...
        if(statsContainer && cur) {
            ...
            const finalInfo = cffGetTeamFinalProfileInfo(tName);
            const finalSummaryHtml = finalInfo ? `
                <div class="card" style="grid-column:1/-1; border:1px solid ${finalInfo.champion ? 'rgba(255,215,0,.5)' : 'rgba(74,168,255,.35)'}; background:rgba(255,255,255,.035);">
                    <div class="card-top-border" style="background:${finalInfo.champion ? '#ffd700' : '#4aa8ff'}"></div>
                    <h3>${finalInfo.champion ? 'Resultado da Final — Campeã' : 'Resultado da Final'}</h3>
                    <div class="value" style="font-size:1.55em; color:${finalInfo.champion ? '#ffd700' : '#fff'}">${finalInfo.rank}º</div>
                    <div style="color:var(--text-muted);font-size:.86em;margin-top:6px;">${finalInfo.pontos} pts • ${finalInfo.abates} K • ${finalInfo.booyah} B! • ${finalInfo.quedas} quedas</div>
                </div>` : '';
...
function renderTeamTrophies(teamName) {
    ...
    const collectiveTitles = cffDeduplicateTeamTrophies([...staticCollective, ...novosCollective]);
    const collapsedIndividualAwards = cffCollapseTeamIndividualAwards(individualAwards);

    if (collectiveTitles.length === 0 && collapsedIndividualAwards.length === 0) {
    ...
    const individualHtml = collapsedIndividualAwards.length ? `
        ...
        ${collapsedIndividualAwards.map(t => typeof buildUnifiedTrophyCard === 'function'
...
function cffBuildLaffRosterPlayerHero(playerName, teamName, laffInfo = null) {
    ...
    const photo = cffResolvePlayerHistoryPhoto(playerName);
    const team = cffFindLAFFTeamInfo(teamName) || {};
    const logo = team.logo || (typeof logos !== 'undefined' && logos ? logos[teamName] : '') || 'escudo.webp';
    const role = cffNormalizeRoleLabel(laffInfo?.funcao || laffInfo?.role || '', false);
    return `
        <div class="cff-inactive-player-hero cff-laff-roster-player-hero">
            <div class="cff-inactive-player-photo-wrap">
                <img class="cff-inactive-player-photo" src="${cffEscapeHTML(photo)}" alt="${cffEscapeHTML(displayName)}" onerror="this.src='silhueta.png'">
                <span class="cff-laff-roster-team-logo"><img src="${cffEscapeHTML(logo)}" alt="${cffEscapeHTML(teamName)}" onerror="this.src='escudo.webp'"></span>
            </div>
            <h2>${cffEscapeHTML(displayName)}</h2>
            <div class="cff-inactive-player-status">LAFF 2026 S1</div>
            <button type="button" class="btn-action cff-laff-roster-team-btn" onclick="cffOpenTeamProfileSmart('${String(teamName || '').replace(/'/g, "\\'")}')">
                ${cffEscapeHTML(teamName || 'Equipe a confirmar')}
            </button>
            <div class="cff-inactive-player-last">Função: <strong>${cffEscapeHTML(role)}</strong></div>
        </div>`;
}
        ...
        return `
            <button type="button" class="team-player-history-card team-player-history-photo-card${stateClass}" onclick="${_safePPAttr(p.jogador)}" title="Abrir perfil de ${cffEscapeHTML(p.displayName)}">
                <span class="team-player-history-avatar">
                    ${logoHtml}
                    <img src="${cffEscapeHTML(photo)}" alt="${cffEscapeHTML(p.displayName)}" loading="lazy" decoding="async" fetchpriority="low" onerror="this.src='silhueta.png'">
                </span>
    ...
    const notableHtml = notablePlayers.map((p, index) => {
        ...
        return `
            <button type="button" class="team-notable-player-card" onclick="${_safePPAttr(p.jogador)}" title="Abrir perfil de ${cffEscapeHTML(p.displayName)}">
                ...
                <span class="team-notable-avatar"><img src="${cffEscapeHTML(photo)}" alt="${cffEscapeHTML(p.displayName)}" loading="lazy" decoding="async" fetchpriority="low" onerror="this.src='silhueta.png'"></span>
