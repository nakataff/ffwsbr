...
function renderPlayerHistorySummary(playerName, context = {}) {
    ...
    const summaryPlayer = activeInfo?.player || context.currentPlayer || null;
    const activePlayer = mercadoExit ? null : (activeInfo?.player || context.currentPlayer || null);
    const mergedSummaryEditions = cffMergeLAFFPlayerHistoryIntoEditions(playerName, cffMergeCurrentEditionIntoEditions(playerName, lbff ? (lbff.editions || {}) : {}, summaryPlayer || context.currentPlayer || null));
    ...
    const rawCurrentTeam = context.currentTeam || activeInfo?.equipe || (mercadoExit ? '' : (activePlayer?.equipe || '')) || '';
    const currentTeam = (/^sem\s+(equipe|time|clube)$/i.test(String(rawCurrentTeam || '').trim()) || /hist[oó]rico/i.test(String(rawCurrentTeam || '').trim())) ? '' : rawCurrentTeam;
    const hasCurrentTeam = !!currentTeam && !mercadoExit;
