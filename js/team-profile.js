...
function cffOpenTeamProfileSmart(teamName) {
    ...
    try {
        ...
        const hasLAFFTeam = Array.isArray(laffTeams) && laffTeams.some(t => {
            if (typeof sameTeamName === 'function') return sameTeamName(t.name, raw);
            return String(t.name || '').toUpperCase() === raw.toUpperCase();
        });
        if (hasLAFFTeam && typeof window.openLAFFTeamProfile === 'function') return window.openLAFFTeamProfile(raw);
    } catch (e) {}
...
let officialName = cffCanonicalPlayerDisplayName(safeName);
    ...
    if (teamEl) {
        teamEl.innerText = playerMercadoExit ? "SEM EQUIPE" : (isInactive && !playerMercadoCurrent ? "JOGADOR HISTÓRICO" : equipeAtual);
        teamEl.onclick = (isInactive && !playerMercadoCurrent || playerMercadoExit) ? null : () => cffOpenTeamProfileSmart(equipeAtual);
        teamEl.style.cursor = (isInactive && !playerMercadoCurrent || playerMercadoExit) ? "default" : "pointer";
    }
...
if (typeof renderPlayerCFFRating === 'function') renderPlayerCFFRating(p.jogador);
    ...
    if (typeof renderPlayerHistorySummary === 'function') renderPlayerHistorySummary(p.jogador, { isInactive: isInactive || !!playerMercadoExit, currentTeam: playerMercadoExit ? '' : equipeAtual, lastEdition: lastEd, mercadoExit: playerMercadoExit, mercadoCurrent: playerMercadoCurrent, currentPlayer: p });
