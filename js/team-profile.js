...
async function renderTeamPlayersHistory(teamName) {
    ...
    const renderToken = `${requestedTeam}::${Date.now()}::${Math.random().toString(36).slice(2)}`;
    ...
    // Não deixa mensagem permanente de carregamento. O bloco fica reservado e é preenchido
    // quando os dados chegam, sem prender clique nem rolagem da página.
    section.innerHTML = `
    ...
    // Fotos não podem travar a abertura da página de equipe. Renderiza primeiro
    // com o que já existir e carrega a planilha de fotos em segundo plano.
    if (typeof loadPhotos === 'function' && !window.__cffTeamHistoryPhotosLoaded) {
        ...
        try {
            Promise.resolve(loadPhotos()).then(() => {
                const current = String(currentTeamView || '').trim();
                const same = current && (typeof sameTeamName === 'function' ? sameTeamName(current, requestedTeam) : current.toUpperCase() === requestedTeam.toUpperCase());
                if (same) renderTeamPlayersHistory(requestedTeam);
            }).catch(e => console.warn('[renderTeamPlayersHistory] fotos não carregaram:', e));
        } catch (e) { console.warn('[renderTeamPlayersHistory] fotos não iniciaram:', e); }
    }
