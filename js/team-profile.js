...
async function cffRenderPlayerLAFFHistoryPanel(playerName, currentTeam = '') {
    ...
    const rows = (data.teams || []).filter(row => teams.some(team => cffTeamMatchesLAFFName(row.team, team)));
    const currentLaff = cffFindLAFFPlayerInfo(playerName);

    if (!rows.length && !currentLaff) {
        box.innerHTML = '';
    ...
    const rowsHtml = rows.map(row => `<div class="pp-laff-row">
...
if (typeof renderPlayerCFFRating === 'function') renderPlayerCFFRating(p.jogador);
    ...
    if (typeof cffRenderPlayerLAFFHistoryPanel === 'function') cffRenderPlayerLAFFHistoryPanel(p.jogador, playerMercadoExit ? '' : equipeAtual);
