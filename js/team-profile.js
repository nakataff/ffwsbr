...
function cffLoadMercadoNonBlocking(timeoutMs = 900) {
...
function cffOpenTeamProfileSmart(teamName) {
    ...
    // Isso evita travadas em páginas como LOUD/Fluxo quando o clique não é de time LAFF.
...
function cffBuildLAFFTraditionalPlayerCard(player, team) {
    ...
    if (typeof playerRoles !== 'undefined' && name) playerRoles[name] = cffNormalizeRoleLabel(player?.funcao || player?.role || '', true);
    const p = {
        jogador: name,
        equipe: teamName,
        abates: 0,
        ...
        quedas: 0,
        mvp: 0,
        pais: player?.pais || player?.country || 'br',
        flag: player?.pais || player?.country || 'br',
        ovrOverride: 0
    };
    if (typeof createPlayerCardHTML === 'function') return createPlayerCardHTML(p, 0.92);
...
async function cffOpenLAFFTeamInTraditionalProfile(teamName) {
    ...
    if (typeof loadPassagens === 'function' && (!Array.isArray(dbPassagens) || dbPassagens.length === 0)) {
        try { await loadPassagens(); } catch(e) {}
    }
    ...
    if (cards) {
        ...
        cards.style.gridTemplateColumns = roster.length <= 5 ? `repeat(${Math.max(roster.length, 1)}, minmax(220px, 300px))` : 'repeat(auto-fit, minmax(240px, 300px))';
    ...
    if (typeof renderTeamPlayersHistory === 'function') await renderTeamPlayersHistory(team.name);
...
async function openPlayerProfile(playerName) {
    ...
    if (typeof loadMercado === 'function') {
        try { await cffLoadMercadoNonBlocking(); } catch (e) { console.warn('[PlayerProfile] Mercado não carregou antes do perfil:', e); }
    }
