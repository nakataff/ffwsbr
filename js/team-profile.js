...
function cffBuildLAFFTraditionalPlayerCard(player, team) {
    ...
    if (typeof playerRoles !== 'undefined' && name) playerRoles[name] = cffNormalizeRoleLabel(player?.funcao || player?.role || '', false);
    const p = {
        jogador: name,
        equipe: teamName,
        abates: 0,
...
function cffGetTeamActivePeople(teamName, currentPlayerName = '') {
    ...
    const players = (typeof db !== 'undefined' && Array.isArray(db.players) ? db.players : [])
        ...
        .map(p => ({
            ...
            role: cffNormalizeRoleLabel((typeof playerRoles !== 'undefined' && playerRoles[p.jogador]) ? playerRoles[p.jogador] : '', false),
    ...
    const laffPlayers = (typeof window.getLAFFPlayers === 'function' ? (window.getLAFFPlayers() || []) : [])
        ...
        .map(p => ({
            ...
            role: cffNormalizeRoleLabel(p.funcao || p.role || '', false),
...
async function renderTeamPlayersHistory(teamName, options = {}) {
        ...
        section.innerHTML = `
            ...
            <div class="team-history-subsection team-player-history-compact-wrap team-player-history-lazy-shell">
                <div class="team-history-subtitle-row team-player-history-compact-title">
                    ...
                    <p>Para evitar travadas, o histórico carrega só quando você pedir. Primeiro vem compacto; as fotos carregam apenas na expansão.</p>
                </div>
                <div class="team-player-history-actions">
                    <span class="team-player-history-more">Modo leve ativo para abrir a página mais rápido.</span>
                    <button type="button" class="btn-action team-player-history-load-btn" data-team="${cffEscapeHTML(requestedTeam)}" onclick="window.cffLoadCompactTeamPlayersHistory && window.cffLoadCompactTeamPlayersHistory(this.dataset.team)">Carregar histórico compacto</button>
                </div>
            </div>`;
        return;
    }
    ...
    section.innerHTML = `
        ...
        <div class="team-player-history-loading-inline">Carregando histórico compacto...</div>`;
    ...
    if (!expanded) {
        ...
        section.innerHTML = `
            ...
            <div class="team-history-subsection team-player-history-compact-wrap">
                <div class="team-history-subtitle-row team-player-history-compact-title">
                    <h4>Histórico de jogadores</h4>
                    <p>Modo compacto para abrir a página mais rápido. As fotos carregam só ao expandir.</p>
                </div>
...
function cffLoadCompactTeamPlayersHistory(teamName) {
    return renderTeamPlayersHistory(teamName || currentTeamView, { compact: true });
}
window.cffLoadCompactTeamPlayersHistory = cffLoadCompactTeamPlayersHistory;

function cffExpandTeamPlayersHistory(teamName) {
    return renderTeamPlayersHistory(teamName || currentTeamView, { expanded: true, compact: true });
}
