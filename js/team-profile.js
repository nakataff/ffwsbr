...
if (typeof renderPlayerCFFRating === 'function') renderPlayerCFFRating(p.jogador);
    ...
    if (typeof renderPlayerTeammatesSection === 'function') renderPlayerTeammatesSection(p.jogador, equipeAtual, { isInactive });
...
function onMvpStageFilterChanged() {
    const select = document.getElementById('filter-stage-players');
    selectedMvpStage = (typeof cffNormalizeStage === 'function') ? cffNormalizeStage(select?.value || 'classificatoria') : (select?.value || 'classificatoria');
    selectedPlayerDays = [];
...
function renderAllPlayers() {
    let stageSelect = document.getElementById('filter-stage-players');
    selectedMvpStage = (typeof cffNormalizeStage === 'function') ? cffNormalizeStage(stageSelect?.value || selectedMvpStage || 'classificatoria') : (stageSelect?.value || selectedMvpStage || 'classificatoria');
    ...
    setTimeout(() => {
        let dataToAggregate = db.playerDaily.filter(p => (typeof cffIsDayInStage === 'function') ? cffIsDayInStage(p.dia, selectedMvpStage) : true);
...
function cffGetTeamActivePeople(teamName, currentPlayerName = '') {
    ...
    const players = (typeof db !== 'undefined' && Array.isArray(db.players) ? db.players : [])
        .filter(p => !p.isEx && cffTeamMatchesHistoryTeam(p.equipe, canonical))
        .map(p => ({
            type: 'player',
            name: p.jogador,
...
function cffGetPlayersFromTeamPassagens(teamName) {
    ...
    const pushRecord = (record) => {
        ...
        byName.set(key, {
            ...
            isCurrent: Boolean(existing.isCurrent || record.isCurrent),
            isActiveElsewhere: Boolean((existing.isActiveElsewhere || record.isActiveElsewhere) && !(existing.isCurrent || record.isCurrent)),
    ...
    dbPassagens.forEach(row => {
        ...
        const isActiveElsewhere = !!activeInfo && !!activeTeam && !isCurrent;
    ...
    currentPlayers
        .filter(p => p && p.isEx !== true && cffTeamMatchesHistoryTeam(p.equipe, canonical))
        .forEach(p => {
