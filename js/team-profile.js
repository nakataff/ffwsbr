...
function cffGetPlayerTitles(playerName) {
    ...
    const dedupeTitles = (items) => {
        ...
        return items.filter(t => {
            const team = cffDisplayTeamName(t.team || '');
            const key = `${String(t.event || '').toUpperCase()}|${String(t.type || '').toUpperCase()}|${team.toUpperCase()}|${String(t.player || '').toUpperCase()}`;
    ...
    return { collective: dedupeTitles([...oldCol, ...newCol]), individual: dedupeTitles([...oldInd, ...newInd]) };
}

function cffDisplayTeamName(teamName) {
    const raw = String(teamName || '').trim();
    if (!raw) return '';
    const canonical = (typeof getCanonicalTeamNameSafe === 'function')
        ? getCanonicalTeamNameSafe(raw)
        : ((typeof getTeamCanonicalName === 'function') ? getTeamCanonicalName(raw) : raw);
    // Trava de segurança para o nome legado não voltar a aparecer no perfil.
    return String(canonical || raw).toUpperCase() === 'B4STARDOS' ? 'B4 ESPORTS' : (canonical || raw);
}

function cffEditionIndex(editionName) {
    const ed = String(editionName || '').trim();
    if (typeof allEditions !== 'undefined' && Array.isArray(allEditions)) {
        const idx = allEditions.findIndex(x => String(x).toUpperCase() === ed.toUpperCase());
        if (idx >= 0) return idx;
    }
    const lbff = ed.match(/LBFF\s*(\d+)/i);
    if (lbff) return Number(lbff[1]);
    const wb = ed.match(/WB\s*(\d{4})\s*S?(\d+)?|WB\s*(\d{4})\s*F?(\d+)?/i);
    if (wb) return 1000 + Number(wb[1] || wb[3] || 0) * 10 + Number(wb[2] || wb[4] || 0);
    return 99999;
}

function cffGetLbffDebut(playerName, editions) {
    if (!editions || !editions.length) return null;
    const sorted = [...editions].sort((a, b) => cffEditionIndex(a[0]) - cffEditionIndex(b[0]));
    const first = sorted[0];
    return first ? { tournament: first[0], kills: Number(first[1]?.k || first[1]?.kills || 0), matches: Number(first[1]?.q || first[1]?.quedas || 0), source: 'lbffData' } : null;
}

function cffNaturalList(parts) {
...
function cffTitleEventText(title) {
    const event = String(title?.event || 'torneio').trim();
    const team = cffDisplayTeamName(title?.team || '');
    return `${event}${team ? ` pela ${team}` : ''}`;
}

function cffEnsurePlayerSummaryStyles() {
    if (document.getElementById('cff-player-summary-styles')) return;
    const style = document.createElement('style');
    style.id = 'cff-player-summary-styles';
    style.textContent = `
        .cff-summary-box{margin:22px 0 28px;border:1px solid rgba(255,255,255,.10);border-radius:18px;background:linear-gradient(180deg,rgba(255,255,255,.055),rgba(255,255,255,.025));box-shadow:0 14px 35px rgba(0,0,0,.25);overflow:hidden;}
        .cff-summary-toggle{width:100%;border:0;background:rgba(0,200,255,.08);color:#fff;padding:16px 18px;display:flex;align-items:center;justify-content:space-between;gap:12px;cursor:pointer;text-align:left;font-weight:900;text-transform:uppercase;letter-spacing:.8px;}
        .cff-summary-toggle:hover{background:rgba(0,200,255,.14);}
        .cff-summary-kicker{display:flex;align-items:center;gap:10px;color:var(--accent);font-size:.86rem;}
        .cff-summary-chevron{font-size:1.1rem;color:var(--accent);transition:.2s transform;}
        .cff-summary-box.is-collapsed .cff-summary-chevron{transform:rotate(-90deg);}
        .cff-summary-body{padding:18px;display:block;}
        .cff-summary-box.is-collapsed .cff-summary-body{display:none;}
        .cff-summary-body p{color:#d9d9d9;line-height:1.65;margin:0 0 16px;font-size:.96rem;}
        .cff-summary-mini-grid{display:grid;grid-template-columns:repeat(5,minmax(110px,1fr));gap:10px;}
        .cff-summary-mini-grid span{background:rgba(0,0,0,.30);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:12px 10px;text-align:center;min-width:0;}
        .cff-summary-mini-grid strong{display:block;color:#fff;font-size:1.35rem;line-height:1.1;white-space:nowrap;margin-bottom:6px;}
        .cff-summary-mini-grid small{display:block;color:#a9a9a9;font-size:.68rem;text-transform:uppercase;letter-spacing:.6px;line-height:1.2;}
        @media(max-width:780px){.cff-summary-mini-grid{grid-template-columns:repeat(2,minmax(0,1fr));}.cff-summary-mini-grid span:last-child{grid-column:1/-1;}.cff-summary-toggle{padding:14px;}.cff-summary-body{padding:14px;}}
    `;
    document.head.appendChild(style);
}

function togglePlayerHistorySummary() {
    const box = document.getElementById('pp-history-summary-box');
    if (!box) return;
    box.classList.toggle('is-collapsed');
}

function renderPlayerHistorySummary(playerName, context = {}) {
    cffEnsurePlayerSummaryStyles();
    ...
    const orderedEditions = [...editions].sort((a, b) => cffEditionIndex(a[0]) - cffEditionIndex(b[0]));

    let totalKills = 0, totalMatches = 0;
    let bestKill = null;
    orderedEditions.forEach(([edition, data]) => {
    ...
    const lbffDebut = cffGetLbffDebut(playerName, orderedEditions);
    const debutAppearance = appearances[0] || null;
    const debut = lbffDebut || debutAppearance || null;
    const last = appearances[appearances.length - 1] || (orderedEditions.length ? { tournament: orderedEditions[orderedEditions.length - 1][0], pos: 0, team: '' } : null);
    const bestPlace = appearances.filter(a => a.pos).sort((a, b) => a.pos - b.pos)[0] || null;
    const titles = cffGetPlayerTitles(playerName);
    const collectiveTitles = [...titles.collective].sort((a, b) => cffEditionIndex(a.event) - cffEditionIndex(b.event));
    const titleBestPosition = collectiveTitles.length ? 1 : 0;
    const bestPositionValue = titleBestPosition || bestPlace?.pos || 0;
    const avgKills = totalMatches > 0 ? (totalKills / totalMatches).toFixed(2) : '0.00';

    const currentTeam = activePlayer && !activePlayer.isEx ? cffDisplayTeamName(activePlayer.equipe) : '';
    const currentText = currentTeam
        ? `Atualmente defende ${currentTeam}.`
    ...
    const sentences = [];
    if (debut) {
        const debutTeam = !lbffDebut && debut.team ? ` com ${cffDisplayTeamName(debut.team)}` : '';
        const debutPlace = !lbffDebut && debut.pos ? ` e ficou na ${debut.pos}ª posição` : '';
        sentences.push(`${displayName} estreou na ${debut.tournament}${debutPlace}${debutTeam}.`);
    }
    if (bestKill) sentences.push(`Seu melhor desempenho individual foi de ${bestKill.kills} abates na ${bestKill.edition}.`);
    if (collectiveTitles.length) {
        const titleList = cffNaturalList(collectiveTitles.map(t => cffTitleEventText(t)));
        const titleWord = collectiveTitles.length === 1 ? 'o título' : 'os títulos';
        sentences.push(`Sua melhor colocação como equipe foi ${titleWord} da ${titleList}.`);
    } else if (bestPlace) {
        sentences.push(`Sua melhor colocação como equipe foi ${cffOrdinalBR(bestPlace.pos)} na ${bestPlace.tournament}${bestPlace.team ? ` pela ${cffDisplayTeamName(bestPlace.team)}` : ''}.`);
    }
    ...
    if (titles.individual.length) sentences.push(`Além dos títulos coletivos, também possui ${titles.individual.length} prêmio(s) individual(is) registrado(s).`);

    box.className = 'cff-summary-box';
    box.innerHTML = `
        <button class="cff-summary-toggle" type="button" onclick="togglePlayerHistorySummary()">
            <span class="cff-summary-kicker">📌 Resumo histórico do jogador</span>
            <span class="cff-summary-chevron">▾</span>
        </button>
        <div class="cff-summary-body">
            <p>${sentences.join(' ')}</p>
            <div class="cff-summary-mini-grid">
                ...
                <span><strong>${avgKills}</strong><small>média de kills</small></span>
                <span><strong>${bestPositionValue ? bestPositionValue + 'º' : 'N/A'}</strong><small>melhor posição</small></span>
                <span><strong>${titles.collective.length}</strong><small>títulos</small></span>
            </div>
        </div>`;
}
