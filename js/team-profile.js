...
function cffEnsureLAFFTeamHistory(playerName, containerId) {
    ...
    card.type = 'button';
    card.className = 'laff-team-history-card cff-laff-current-team-history';
    card.onclick = () => cffOpenTeamProfileSmart(teamName);
    card.innerHTML = `
        <img src="${cffEscapeHTML(team.logo || 'escudo.webp')}" onerror="this.onerror=null;this.src='escudo.webp'" alt="${cffEscapeHTML(teamName)}">
        <div><strong>${cffEscapeHTML(teamName)}</strong><span>ATUAL • LAFF 2026 S1</span></div>`;
    wrap.prepend(card);
}
...
function cffGetTeamActivePeople(teamName, currentPlayerName = '') {
    ...
    const laffPlayers = (typeof window.getLAFFPlayers === 'function' ? (window.getLAFFPlayers() || []) : [])
        .filter(p => cffTeamMatchesHistoryTeam(p.team, canonical) || cffNormalizeFilterText(p.team) === cffNormalizeFilterText(canonical))
        .filter(p => {
