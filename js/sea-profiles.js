...
(function () {
    ...
    function getSeaTeamLogo(raw) {
        ...
        return (window.logos && (logos[full] || logos[sigla] || logos[raw])) || 'escudo.webp';
    }
    ...
    function renderPlayerHero(player) {
        ...
        const photo = photoFromSheet(player.rawName, player.cleanName) || 'silhueta.png';
        const teamName = player.team || '-';
        return `
            <section class="sea-profile-hero">
                <div class="sea-player-photo-ring ${status === 'inativo' ? 'is-inactive' : 'is-active'}">
                    <img src="${escape(photo)}" onerror="this.onerror=null;this.src='silhueta.png'" alt="${escape(player.displayName)}">
                </div>
                <h1>${escape(player.displayName)}</h1>
                <div class="sea-profile-country">${countryFlag(player.country)} ${escape(player.country || '-')}</div>
    ...
    async function openSEAPlayerProfile(name) {
        ...
        rootEl.innerHTML = `
            ...
            <section class="sea-profile-section">
                ...
                <p><strong>${escape(player.displayName)}</strong> é um jogador ${escape((player.country || '-').toLowerCase())} que atua atualmente pela equipe <strong>${escape(player.team || '-')}</strong>.</p>
            </section>
            <section class="sea-profile-section">
    ...
    function renderSeaTeamPlayers(team) {
        ...
        const renderList = (players, empty) => players.length ? players.map(p => {
            const photo = photoFromSheet(p.rawName, p.cleanName) || 'silhueta.png';
            return `
                <button class="sea-roster-card" onclick="openSEAPlayerProfile(${escape(jsArg(p.rawName))})">
                    <img src="${escape(photo)}" onerror="this.onerror=null;this.src='silhueta.png'" alt="${escape(p.displayName)}">
                    <span>${escape(p.displayName)}</span>
                    <small>${countryFlag(p.country)} ${escape(p.funcao || '-')}</small>
                </button>`;
    ...
    async function openSEATeamProfile(name) {
        ...
        rootEl.innerHTML = `
            ...
            <section class="sea-profile-section">
                <h2>Resumo histórico</h2>
                <p><strong>${escape(team.name)}</strong> é uma equipe da FFWS SEA. No desempenho atual, soma <strong>${escape(team.kills)}</strong> kills, <strong>${escape(formatCompact(team.dano))}</strong> de dano, <strong>${escape(team.assistencias)}</strong> assistências e <strong>${escape(team.mvp)}</strong> MVP.</p>
            </section>
            <section class="sea-profile-section">
                <h2>Jogadores</h2>
    ...
    function patchSearchPools() {
        ...
        window.navBuildPeopleSearchPool = function () {
            ...
            const seaPeople = Array.from(seaPlayerIndex.values()).map(p => ({
                ...
                title: p.displayName,
                sub: `${p.team || '-'} • FFWS SEA`,
                img: photoFromSheet(p.rawName, p.cleanName) || 'silhueta.png',
                priority: 4,
        ...
        window.selectSearchResult = function (type, name) {
            if (type === 'sea-player') return openSEAPlayerProfile(name);
            if (type === 'sea-team') return openSEATeamProfile(name);
    ...
    document.addEventListener('DOMContentLoaded', () => {
        loadSEAProfilesData().then(patchSearchPools).catch(err => console.warn('[SEA Profiles] Falha geral:', err));
    });
})();