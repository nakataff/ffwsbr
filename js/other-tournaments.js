...
function otGetPlayerRanking(t) {
    return t?.playerRanking || t?.rankingJogadores || null;
}
...
function renderOutrosTorneiosList() {
    ...
    container.innerHTML = list.map(t => {
        ...
        const tier = otGetTournamentTier(t);
...
function openOtherTournament(id) {
    ...
    document.getElementById('ot-details-badge').textContent = otGetTournamentTier(t);
...
function renderOtDetailsStats() {
    ...
    const awardsCount = otGetAwards(currentOtData).length;
    const tablesCount = otGetStandings(currentOtData).length + (currentOtData.final ? 1 : 0);

    el.innerHTML = `
        <div><strong>${otEscapeHTML(champion)}</strong><span>Campeão</span></div>
        <div><strong>${teamsCount}</strong><span>Equipes</span></div>
        <div><strong>${tablesCount}</strong><span>Tabelas</span></div>
        <div><strong>${awardsCount}</strong><span>Prêmios</span></div>
    `;
}
...
function renderOtStandings() {
    ...
    container.innerHTML = tables.map((table, tableIndex) => {
        ...
        return `
            <section class="ot-section-block">
                <div class="ot-section-head">
                    <h3>${otEscapeHTML(table.title || `Tabela ${tableIndex + 1}`)}</h3>
                    <span>${otEscapeHTML(table.source || '')}</span>
                </div>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>Total</th>
                                ${rounds.map(r => `<th>${otEscapeHTML(r)}</th>`).join('')}
                            </tr>
                        </thead>
                        <tbody>${tbody}</tbody>
                    </table>
            ...
            <div class="table-container">
                <table>
                    ...
                    ${rows.map(row => {
                        ...
                        return `<tr><td>${pos}º</td><td class="ot-team-click" onclick="openOtTeamRoster('${otEscapeHTML(currentOtData.id)}', '${otEscapeHTML(team)}')"><img src="${otGetTeamLogo(team)}" onerror="this.src='escudo.webp'"><span>${otEscapeHTML(team)}</span></td><td class="ot-total-cell">${points}</td></tr>`;
                    }).join('')}
                    </tbody>
                </table>
            </div>
...
function renderOtFinal() {
    ...
    const tbody = rows.map(row => {
        ...
        return `
            <tr>
                <td class="ot-pos-cell">${pos}º</td>
                <td class="ot-team-click" onclick="openOtTeamRoster('${otEscapeHTML(currentOtData.id)}', '${otEscapeHTML(team)}')"><img src="${otGetTeamLogo(team)}" onerror="this.src='escudo.webp'"><span>${otEscapeHTML(team)}</span></td>
                <td>${headstart}</td>
                <td>${kills}</td>
                <td>${placementPts}</td>
                <td>${killPts}</td>
...
function renderOtPlayerRanking() {
    ...
    container.innerHTML = `
        <section class="ot-section-block">
            <div class="ot-section-head"><h3>Ranking de abates</h3><span>${ranking.source || 'manual'}</span></div>
            <div class="table-container ot-wide-table">
                <table>
                    <thead><tr><th>#</th><th style="text-align:left;">Jogador</th><th>Equipe</th><th>Abates</th><th>Quedas</th><th>Média</th><th>Dano</th><th>Assist.</th><th>MVP</th></tr></thead>
                    <tbody>
                        ${rows.map(row => {
                            ...
                            return `<tr><td>${pos}º</td><td class="ot-player-link" data-player="${otEscapeHTML(player)}">${otEscapeHTML(player)}</td><td>${otEscapeHTML(team)}</td><td class="ot-total-cell">${kills}</td><td>${matches}</td><td>${avg ?? '-'}</td><td>${damage ?? '-'}</td><td>${assists ?? '-'}</td><td>${mvps ?? '-'}</td></tr>`;
                        }).join('')}
                    </tbody>
                </table>
            </div>
...
function renderOtTeams() {
    ...
    container.innerHTML = `
        <section class="ot-section-block">
            <div class="ot-section-head"><h3>Equipes participantes</h3><span>${teams.length} equipes</span></div>
...
window.openNovoTorneioDetails = openNovoTorneioDetails;
window.closeNovoTorneioDetails = closeNovoTorneioDetails;
window.renderNovoTorneioTab = renderNovoTorneioTab;
