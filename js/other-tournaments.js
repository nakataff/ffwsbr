...
function otGetPlayerFlagSrc(playerName, rawFlag = '') {
    ...
    if (!raw) return 'br.png';

    if (/\.(png|webp|jpg|jpeg|svg)$/i.test(raw)) return raw;

    const normalized = raw.toLowerCase();
    if (normalized === 'br' || normalized === 'bra' || normalized === 'brasil' || normalized === 'brazil') return 'br.png';
    if (normalized === 'mx' || normalized === 'mex' || normalized === 'mexico' || normalized === 'méxico') return 'MX.png';
