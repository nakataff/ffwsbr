...
function otParseMoneyNumber(value, fallback = 0) {
    ...
    // Liquipedia normalmente usa 105,000 = cento e cinco mil.
    // Também aceita formatos BR como 105.000,50.
    if (/^-?\d{1,3}(,\d{3})+(\.\d+)?$/.test(raw)) {
        raw = raw.replace(/,/g, '');
    } else if (/^-?\d{1,3}(\.\d{3})+(,\d+)?$/.test(raw)) {
        raw = raw.replace(/\./g, '').replace(',', '.');
    } else if ((raw.match(/,/g) || []).length === 1 && !raw.includes('.')) {
        const [left, right] = raw.split(',');
        raw = right.length === 3 ? left + right : left + '.' + right;
    } else {
        raw = raw.replace(/,/g, '');
    }
...
function otFormatMoney(value, currency = 'BRL') {
    const num = otParseMoneyNumber(value, 0);
...
function otGetTotalPrize(t) {
    ...
    otGetPrizeSections(t).forEach(section => {
        ...
        rows.forEach(row => {
            ...
            total += otParseMoneyNumber(val, 0);
        });
        (section.items || section.slots || []).forEach(item => total += otParseMoneyNumber(item.prize || item.localprize || 0, 0));
    });
    return total;
}
