...
var otShowRosters = (typeof window.otShowRosters === 'boolean') ? window.otShowRosters : true;
var otFinalMatchFilter = window.otFinalMatchFilter || 'all';
var otStandingsFilter = window.otStandingsFilter || 'annual';
...
var otPlayerFlagsMap = window.otPlayerFlagsMap || {};
var otPlayerFlagsLoaded = window.otPlayerFlagsLoaded || false;
var globalTeamAliasesMap = window.globalTeamAliasesMap || null;
var otSlotsSheetLoaded = window.otSlotsSheetLoaded || false;
var otSlotsSheetMap = window.otSlotsSheetMap || {};
var OT_SLOTS_SHEET_URL = window.CFF_CONFIG?.sheets?.outrosTorneiosSlots || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vR6Paknya4E3qRT2mLd0fQMIiBKhuGOPebF0pLK9c0Gk5nRnVWNdY4FxMJV42467JLmwNNumXSc4fCC/pub?gid=1734913915&single=true&output=tsv';

async function otLoadTeamAliases() {
...
async function otLoadPlayerFlags() {
    if (otPlayerFlagsLoaded) return;
    otPlayerFlagsLoaded = true;
    otPlayerFlagsMap = otPlayerFlagsMap || {};

    try {
        const res = await fetch(withCacheBuster('player-flags.json'));
        if (!res.ok) throw new Error('Falha ao carregar player-flags.json');
        const data = await res.json();

        Object.entries(data || {}).forEach(([name, file]) => {
            const key = otNormalizeFilterValue(name);
            if (key && file) otPlayerFlagsMap[key] = String(file).trim();
        });

        window.otPlayerFlagsMap = otPlayerFlagsMap;
        window.otPlayerFlagsLoaded = otPlayerFlagsLoaded;
    } catch (e) {
        console.warn('[otLoadPlayerFlags] Sem player-flags.json, usando br.png como padrão:', e);
    }
}

function otGetPlayerFlagSrc(playerName, rawFlag = '') {
    const mapped = otPlayerFlagsMap?.[otNormalizeFilterValue(playerName)];
    if (mapped) return mapped;

    const raw = String(rawFlag || '').trim();
    if (!raw) return 'br.webp';

    if (/\.(png|webp|jpg|jpeg|svg)$/i.test(raw)) return raw;

    const normalized = raw.toLowerCase();
    if (normalized === 'br' || normalized === 'bra' || normalized === 'brasil' || normalized === 'brazil') return 'br.webp';
    if (normalized === 'mx' || normalized === 'mex' || normalized === 'mexico' || normalized === 'méxico') return 'MX.webp';

    return `${normalized}.png`;
}

function otPlayerFlagHTML(playerName, rawFlag = '') {
    const src = otGetPlayerFlagSrc(playerName, rawFlag);
    const alt = String(src || '').split('/').pop().replace(/\.[^.]+$/, '').toUpperCase();
    return `<img class="ot-player-flag-img" src="${otEscapeHTML(src)}" onerror="this.style.display='none'" alt="${otEscapeHTML(alt)}">`;
}
...
function otParseTournamentDateValue(value) {
    const raw = String(value || '').trim();
    if (!raw) return 0;

    // Aceita formatos comuns: 2021-08-15, 15/08/2021, 08/15/2021 e texto com ano.
    ...
    if (br) {
        const time = new Date(Number(br[3]), Number(br[2]) - 1, Number(br[1])).getTime();
        return Number.isFinite(time) ? time : 0;
    }
...
function otGetTournamentDateValue(t) {
    const candidates = [
        t?.endDate, t?.dataFim, t?.fim, t?.finalDate,
        t?.startDate, t?.dataInicio, t?.inicio, t?.date, t?.data
    ];

    for (const value of candidates) {
        const parsed = otParseTournamentDateValue(value);
        if (parsed) return parsed;
    }

    const year = Number(otGetTournamentYear(t)) || 0;
    return year ? new Date(year, 0, 1).getTime() : 0;
}

function otGetTournamentEditionNumber(t) {
    const candidates = [t?.edition, t?.edicao, t?.season, t?.temporada, t?.stage, t?.etapa, t?.id, otGetTournamentName(t)];
    ...
    for (const value of candidates) {
        ...
        const matches = [...raw.matchAll(/(?:lbff|ffws|liga|s[ée]rie|season|temporada|stage|etapa|split|edi[cç][aã]o)?\s*#?\s*(\d+(?:\.\d+)?)/gi)];
...
function otCompareTournamentsNewestFirst(a, b) {
    const dateDiff = otGetTournamentDateValue(b) - otGetTournamentDateValue(a);
    if (dateDiff) return dateDiff;

    const yearDiff = (Number(otGetTournamentYear(b)) || 0) - (Number(otGetTournamentYear(a)) || 0);
    if (yearDiff) return yearDiff;

    const editionDiff = otGetTournamentEditionNumber(b) - otGetTournamentEditionNumber(a);
    if (editionDiff) return editionDiff;

    return otGetTournamentName(b).localeCompare(otGetTournamentName(a), 'pt-BR', { numeric: true, sensitivity: 'base' });
}
...
function otNormalizeFilterValue(value) {
    return String(value || '')
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
...
function otPrettyRegion(value) {
    const raw = String(value || '').trim();
    if (!raw) return 'Sem região';
    const key = otNormalizeFilterValue(raw);
    if (key === 'brasil' || key === 'brazil') return 'Brasil';
    if (raw.length <= 4 && raw === raw.toUpperCase()) return raw;
    return raw.toLowerCase().replace(/(^|\s|-)([a-záàâãéêíóôõúç])/g, (_, sep, ch) => sep + ch.toUpperCase());
}
...
function otParseMoneyValue(value) {
    ...
    // Liquipedia costuma exportar 105,000 como cento e cinco mil.
    if (hasComma && hasDot) {
        raw = raw.replace(/\./g, '').replace(',', '.');
    } else if (hasComma) {
        const parts = raw.split(',');
        const last = parts[parts.length - 1] || '';
        raw = last.length === 3 ? parts.join('') : raw.replace(',', '.');
    } else if (hasDot) {
        const parts = raw.split('.');
        const last = parts[parts.length - 1] || '';
        if (parts.length > 1 && last.length === 3) raw = parts.join('');
    }
...
function otFormatMoney(value, currency = 'BRL') {
    const num = otParseMoneyValue(value);
...
function otGetTotalPrize(t) {
    ...
    otGetPrizeSections(t).forEach(section => {
        const rows = otCompactRows(section);
        rows.forEach(row => {
            const val = otGetByCol(row, section, 'prize', otGetByCol(row, section, 'localprize', otGetByCol(row, section, 'localPrize', 0)));
            total += otParseMoneyValue(val);
        });
        (section.items || section.slots || []).forEach(item => {
            total += otParseMoneyValue(item.prize ?? item.localprize ?? item.localPrize ?? 0);
        });
    });
    return total;
}
...
function otGetPrizeCurrency(t) {
    ...
    return String(first?.currency || first?.localcurrency || first?.localCurrency || t?.currency || t?.localcurrency || t?.localCurrency || 'BRL').toUpperCase();
}
...
function otFindLogoByAnyName(...names) {
    if (typeof logos === 'undefined' || !logos) return '';

    for (const name of names) {
        const raw = String(name || '').trim();
        if (!raw) continue;
        if (logos[raw]) return logos[raw];
        if (logos[raw.toUpperCase()]) return logos[raw.toUpperCase()];
        if (logos[raw.toLowerCase()]) return logos[raw.toLowerCase()];

        const key = otNormalizeKey(raw);
        const foundKey = Object.keys(logos).find(k => otNormalizeKey(k) === key);
        if (foundKey) return logos[foundKey];
    }
...
function otGetTeamLogo(teamName) {
    ...
    const localNormalized = otNormalizeTeamFromAliases(raw);
    const globalCanonical = otGetGlobalTeamCanonicalName(localNormalized || raw);
    const current = otResolveCurrentTeamName(globalCanonical || localNormalized || raw);
    const tournamentResolved = otResolveTeamForTournament(raw, currentOtData, raw);

    return otFindLogoByAnyName(
        raw,
        localNormalized,
        globalCanonical,
        current,
        tournamentResolved,
        otGetGlobalTeamCanonicalName(raw),
        otResolveCurrentTeamName(raw)
    ) || 'escudo.webp';
}
...
function otGetGlobalTeamCanonicalName(name) {
    ...
    if (typeof getTeamCanonicalName === 'function') {
        ...
        if (canonical && otNormalizeKey(canonical) !== key) return canonical;
    }

    if (typeof dbTeamAliases !== 'undefined' && dbTeamAliases) {
        for (const [canonical, aliases] of Object.entries(dbTeamAliases)) {
            const all = [canonical].concat(Array.isArray(aliases) ? aliases : [aliases]);
            if (all.some(alias => otNormalizeKey(alias) === key)) return canonical;
        }
    }
...
function otGetLbffPlayerEditionKeys(t, existing = null) {
    const configured = existing?.editionKeys || existing?.editions || t?.playerRankingEditionKeys || t?.lbffEditionKeys || t?.killRankingEditions;
    if (Array.isArray(configured) && configured.length) return configured.map(x => String(x || '').trim()).filter(Boolean);

    const name = otGetTournamentName(t);
    const id = String(t?.id || '').toLowerCase();
    if (id === 'wb-2024' || /\bwb\s*2024\b/i.test(name)) return ['WB 2024 F1', 'WB 2024 F2'];

    return [name].filter(Boolean);
}


function otPlayerKey(value) {
    if (typeof normalizePlayerAliasKey === 'function') return normalizePlayerAliasKey(value);
    return otNormalizeKey(value).replace(/0/g, 'O');
}

function otPlayerLooseKey(value) {
    return otPlayerKey(value).replace(/0/g, 'O');
}

function otPlayerNamesMatch(a, b) {
    if (!a || !b) return false;
    if (typeof checkNameMatch === 'function' && checkNameMatch(a, b)) return true;

    const ak = otPlayerKey(a);
    const bk = otPlayerKey(b);
    if (ak && ak === bk) return true;
    if (otPlayerLooseKey(a) && otPlayerLooseKey(a) === otPlayerLooseKey(b)) return true;

    const ca = typeof getCanonicalPlayerName === 'function' ? getCanonicalPlayerName(a) : a;
    const cb = typeof getCanonicalPlayerName === 'function' ? getCanonicalPlayerName(b) : b;
    const cak = otPlayerKey(ca);
    const cbk = otPlayerKey(cb);
    if (cak && cak === cbk) return true;
    if (otPlayerLooseKey(ca) && otPlayerLooseKey(ca) === otPlayerLooseKey(cb)) return true;

    const aliasesA = typeof getPlayerAliasList === 'function' ? getPlayerAliasList(a) : [a, ca];
    const aliasesB = typeof getPlayerAliasList === 'function' ? getPlayerAliasList(b) : [b, cb];
    return aliasesA.some(x => aliasesB.some(y => {
        const xk = otPlayerKey(x);
        const yk = otPlayerKey(y);
        return (xk && xk === yk) || (otPlayerLooseKey(x) && otPlayerLooseKey(x) === otPlayerLooseKey(y));
    }));
}

function otExtractPlayerDisplayName(person) {
    if (!person) return '';
    if (typeof person === 'string') return person.split('|')[0].trim();
    return String(person.n || person.nome || person.player || person.jogador || '').split('|')[0].trim();
}

function otFindPlayerTeamOverride(t, playerName) {
    const overrides = t?.playerTeamOverrides || t?.playerTeams || t?.playersTeams || null;
    if (!overrides || typeof overrides !== 'object') return '';

    const names = new Set([playerName]);
    if (typeof getCanonicalPlayerName === 'function') names.add(getCanonicalPlayerName(playerName));
    if (typeof getPlayerAliasList === 'function') getPlayerAliasList(playerName).forEach(alias => names.add(alias));

    const wantedKeys = [...names].map(otPlayerKey).filter(Boolean);
    for (const [rawName, team] of Object.entries(overrides)) {
        const rawKeys = [rawName];
        if (typeof getCanonicalPlayerName === 'function') rawKeys.push(getCanonicalPlayerName(rawName));
        if (typeof getPlayerAliasList === 'function') getPlayerAliasList(rawName).forEach(alias => rawKeys.push(alias));
        if (rawKeys.map(otPlayerKey).some(k => wantedKeys.includes(k))) return String(team || '').trim();
    }
    return '';
}

function otFindRosterTeamForPlayer(t, playerName, directMap = null, rosterEntries = null) {
    const overrideTeam = otFindPlayerTeamOverride(t, playerName);
    if (overrideTeam) return overrideTeam;

    const key = otPlayerKey(playerName);
    if (directMap && key && directMap[key]) return directMap[key];

    const entries = rosterEntries || [];
    const found = entries.find(entry => otPlayerNamesMatch(entry.player, playerName));
...
function otGetPlayerRanking(t) {
    ...
    if (existing && Array.isArray(existing.rows) && existing.rows.length) return existing;

    if (typeof lbffData === 'undefined' || !lbffData) return existing || null;

    const editionKeys = otGetLbffPlayerEditionKeys(t, existing);
    if (!editionKeys.length) return existing || null;
    ...
    teams.forEach(team => {
        ...
        ['p', 'jogadores', 'players', 'f', 'former', 'ex', 'exJogadores', 'reservas', 'staff', 's'].forEach(listKey => {
            const list = Array.isArray(team?.[listKey]) ? team[listKey] : [];
            list.forEach(person => {
                const playerName = otExtractPlayerDisplayName(person);
                if (!playerName) return;
                rosterEntries.push({ player: playerName, team: teamName });
                const keys = new Set([playerName]);
                if (typeof getCanonicalPlayerName === 'function') keys.add(getCanonicalPlayerName(playerName));
                if (typeof getPlayerAliasList === 'function') getPlayerAliasList(playerName).forEach(alias => keys.add(alias));
                keys.forEach(name => {
                    const k = otPlayerKey(name);
                    if (k && !playerTeam[k]) playerTeam[k] = teamName;
                    const loose = otPlayerLooseKey(name);
                    if (loose && !playerTeam[loose]) playerTeam[loose] = teamName;
                });
            });
        });
    });
    ...
    Object.entries(lbffData).forEach(([name, editions]) => {
        ...
        editionKeys.forEach(key => {
            ...
            if (!data) return;
            const k = Number(data.k ?? data.kills ?? data.abates ?? 0);
            const q = Number(data.q ?? data.quedas ?? data.matches ?? 0);
            if (Number.isFinite(k)) kills += k;
            if (Number.isFinite(q)) matches += q;
            hasData = true;
        });

        if (!hasData || (!kills && !matches)) return;
        const displayName = otExtractPlayerDisplayName(name) || name;
        const team = otFindRosterTeamForPlayer(t, displayName, playerTeam, rosterEntries) || otFindRosterTeamForPlayer(t, name, playerTeam, rosterEntries) || '';
        rows.push([0, displayName, team, team, kills, matches, matches ? Number((kills / matches).toFixed(2)) : null, null, null, null, null, null]);
    });

    if (!rows.length) return existing || null;
    rows.sort((a, b) => (b[4] - a[4]) || ((b[6] || 0) - (a[6] || 0)) || String(a[1]).localeCompare(String(b[1]), 'pt-BR', { sensitivity: 'base' }));
    rows.forEach((row, i) => row[0] = i + 1);

    return {
        type: 'playerRanking',
        source: 'lbffData',
        title: existing?.title || `Ranking de abates - ${otGetTournamentName(t)}`,
        editionKeys,
...
async function loadNovosTorneios() {
    ...
    await otLoadPlayerFlags();
    await otLoadTeamAliases(); // carrega aliases globais sem conflito com home-news-lives.js
    await otLoadSlotsSheet(); // carrega regras de spots para outros torneios

    dbNovosTorneios = loaded.sort(otCompareTournamentsNewestFirst);
    window.dbNovosTorneios = dbNovosTorneios;

    novosTorneiosLoaded = true;
    window.novosTorneiosLoaded = true;
...
function otFillFilterOptions() {
    ...
    dbNovosTorneios.map(otGetTournamentRegion).filter(Boolean).forEach(region => {
        const key = otNormalizeFilterValue(region);
        if (key && !regionMap.has(key)) regionMap.set(key, otPrettyRegion(region));
    });
    ...
    if (regionSelect && regionSelect.dataset.filled !== 'true') {
        const current = regionSelect.value || 'all';
        regionSelect.innerHTML = '<option value="all">Todas as regiões</option>' + regions.map(r => `<option value="${otEscapeHTML(r.value)}">${otEscapeHTML(r.label)}</option>`).join('');
        regionSelect.value = current;
        regionSelect.dataset.filled = 'true';
    }
}
...
function otNormalizeSearchText(value) {
    return String(value || '')
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
...
function otTournamentSearchText(t) {
    const teams = otGetTeams(t).map(team => [team.n, team.nome, team.o, team.nomeOriginal, team.team].filter(Boolean).join(' '));
    const awards = otGetAwards(t).map(a => [a.award, a.premio, a.player, a.jogador, a.team, a.equipe].filter(Boolean).join(' '));
    return [
        t?.id, otGetTournamentName(t), otGetTournamentYear(t), otGetTournamentTier(t), otGetTournamentRegion(t),
        otPrettyRegion(otGetTournamentRegion(t)), otGetTournamentStatus(t), otGetChampionFromFinal(t),
        ...(teams || []), ...(awards || [])
    ].filter(Boolean).join(' ');
}

function otMatchesSearch(t, search) {
    const raw = String(search || '').trim();
    if (!raw) return true;
    const haystack = otNormalizeSearchText(otTournamentSearchText(t));
    const compactHaystack = haystack.replace(/\s+/g, '');
    return otNormalizeSearchText(raw).split(' ').filter(Boolean).every(token => {
...
function renderOutrosTorneiosList() {
    ...
    let list = dbNovosTorneios.filter(t => {
        return otMatchesSearch(t, search)
            && (yearFilter === 'all' || otGetTournamentYear(t) === yearFilter)
            && (tierFilter === 'all' || otGetTournamentTier(t) === tierFilter)
            && (regionFilter === 'all' || otNormalizeFilterValue(otGetTournamentRegion(t)) === regionFilter);
    });
    ...
    container.innerHTML = list.map(t => {
        ...
        const region = otPrettyRegion(otGetTournamentRegion(t));
...
function openOtherTournament(id) {
    ...
    document.getElementById('ot-year').textContent = [otGetTournamentYear(t), otPrettyRegion(otGetTournamentRegion(t)), otGetTournamentStatus(t)].filter(Boolean).join(' • ');
...
function renderOtPrizePool() {
    ...
    container.innerHTML = sections.map((section, index) => {
        const rows = otCompactRows(section);
        const currency = String(section.currency || section.localcurrency || section.localCurrency || otGetPrizeCurrency(currentOtData)).toUpperCase();
        ...
        const tbody = rows.map((row, rowIndex) => {
            ...
            const prize = otGetByCol(row, section, 'prize', otGetByCol(row, section, 'localprize', otGetByCol(row, section, 'localPrize', 0)));
...
function otGetWeekDaysConfig(table, tournament) {
    const raw = table?.weekDays ?? table?.week_days ?? table?.mobileWeekDays ?? tournament?.weekDays ?? tournament?.week_days ?? tournament?.mobileWeekDays ?? tournament?.seasonWeekDays;
    const n = Number(raw);
    if (Number.isFinite(n) && n > 1) return Math.trunc(n);

    const rounds = Array.isArray(table?.rounds) ? table.rounds : [];
    const dayLike = rounds.length >= 8 && rounds.every(r => /^\s*(day|dia)\s*\d+\s*$/i.test(String(r || '')));
    if (dayLike) return 3;

    return 0;
}

function otBuildMobileWeekRounds(table, tournament) {
    const rounds = Array.isArray(table?.rounds) ? table.rounds : [];
    const weekDays = otGetWeekDaysConfig(table, tournament);
    if (!weekDays || rounds.length <= weekDays) return [];

    const weeks = [];
    for (let i = 0; i < rounds.length; i += weekDays) {
        weeks.push({
            label: `W${Math.floor(i / weekDays) + 1}`,
            start: i,
            end: Math.min(i + weekDays, rounds.length)
        });
    }
    return weeks;
}

function otSumWeekResults(results, week) {
    let sum = 0;
    let hasValue = false;
...
function otGetStandingsFilterOptions(t, tables) {
    if (!t || !Array.isArray(tables) || tables.length <= 1) return [];
    const id = String(t.id || '').toLowerCase();
    const name = otNormalizeFilterValue(otGetTournamentName(t));
    const isWb2024 = id === 'wb-2024' || name.includes('wb 2024') || name.includes('ffws br 2024');
    if (!isWb2024) return [];

    return tables.map((table, index) => {
        const title = String(table?.title || '').toLowerCase();
        let id = `t${index}`;
        let label = table?.title || `Tabela ${index + 1}`;
        if (title.includes('anual') || title.includes('overall')) {
            id = 'annual';
            label = 'Pontuação anual';
        } else if (title.includes('fase 1') || title.includes('phase one') || title.includes('phase 1')) {
            id = 'phase1';
            label = 'Pontuação Fase 1';
        } else if (title.includes('fase 2') || title.includes('phase two') || title.includes('phase 2')) {
            id = 'phase2';
            label = 'Pontuação Fase 2';
        }
        return { id, label, index };
    });
}

function setOtStandingsFilter(value) {
    otStandingsFilter = value || 'annual';
    window.otStandingsFilter = otStandingsFilter;
    renderOtStandings();
}

function otRenderStandingsFilter(options) {
    if (!options.length) return '';
    return `
        <div class="ot-standings-filter" aria-label="Filtro de classificação">
            ${options.map(option => `
                <button class="ot-standings-filter-btn ${otStandingsFilter === option.id ? 'active' : ''}" type="button" onclick="setOtStandingsFilter('${otEscapeHTML(option.id)}')">
                    ${otEscapeHTML(option.label)}
                </button>
            `).join('')}
        </div>
    `;
...
function renderOtStandings(customTables = null) {
    const container = document.getElementById('ot-dynamic-container');
    const sourceTables = customTables || otGetStandings(currentOtData);
    if (!container) return;
    ...
    const filterOptions = customTables ? [] : otGetStandingsFilterOptions(currentOtData, sourceTables);
    if (filterOptions.length && !filterOptions.some(option => option.id === otStandingsFilter)) {
        otStandingsFilter = filterOptions[0].id;
        window.otStandingsFilter = otStandingsFilter;
    }

    const tables = filterOptions.length
        ? sourceTables.filter((_, index) => filterOptions.some(option => option.id === otStandingsFilter && option.index === index))
        : sourceTables;

    container.innerHTML = `${otRenderStandingsFilter(filterOptions)}${tables.map((table) => {
        const rounds = table.rounds || [];
        const mobileWeeks = otBuildMobileWeekRounds(table, currentOtData);
        const rows = otCompactRows(table).sort((a, b) => {
            const tb = Number(otGetByCol(b, table, table.cols?.includes('points') ? 'points' : 'total') || 0);
            const ta = Number(otGetByCol(a, table, table.cols?.includes('points') ? 'points' : 'total') || 0);
            if (tb !== ta) return tb - ta;
            const nb = String(otGetByCol(b, table, 'teamOriginal', otGetByCol(b, table, 'team', '')) || '');
            const na = String(otGetByCol(a, table, 'teamOriginal', otGetByCol(a, table, 'team', '')) || '');
        ...
        const tbody = rows.map((row, index) => {
            ...
            const logo = otGetTeamLogo(team);
            const spotRule = otFindSpotRule(currentOtData, table, pos, 'group');
            const spotTypeClass = otSpotClass(spotRule);
            return `
                <tr class="${spotRule ? `ot-row-spot ${spotTypeClass}` : ''}">
                    <td class="ot-pos-cell ${spotRule ? `ot-spot-marker ${spotTypeClass}` : ''}" title="${spotRule ? otEscapeHTML(spotRule.label || '') : ''}">${pos}º</td>
                    ...
                    ${rounds.map((_, i) => `<td class="ot-day-col">${results[i] == null ? '-' : results[i]}</td>`).join('')}
                    ${mobileWeeks.map(week => `<td class="ot-week-col">${otSumWeekResults(results, week)}</td>`).join('')}
                </tr>
            `;
        }).join('');
        ...
        return `
            <section class="ot-section-block">
                <div class="ot-section-head">
                    <h3>${otEscapeHTML(table.title || 'TABELA')}</h3>
                </div>
                ${otRenderSpotLegend(currentOtData, table, 'group')}
                    ...
                    <table class="ot-standings-table ot-table-compact-mobile">
                        <thead>
                            <tr>
                                ...
                                <th>Total</th>
                                ${rounds.map(r => `<th class="ot-day-col">${otHeaderLabel(r, otShortRoundLabel(r))}</th>`).join('')}
                                ${mobileWeeks.map(week => `<th class="ot-week-col">${otEscapeHTML(week.label)}</th>`).join('')}
                            </tr>
                        </thead>
                        <tbody>${tbody}</tbody>
                    </table>
...
function renderOtFinal() {
    ...
    const hasDetailedScoreData = rows.some(row => {
        const hasMainTotals = otGetByCol(row, final, 'kills', null) != null || otGetByCol(row, final, 'placementPts', null) != null;
        const matches = otGetByCol(row, final, 'matches', []) || [];
        const hasMatchDetails = matches.some(match => {
            const placement = getOtFinalMatchValue(row, final, match?.[0], 'placement');
            const kills = getOtFinalMatchValue(row, final, match?.[0], 'kills');
            const placementPts = getOtFinalMatchValue(row, final, match?.[0], 'placementPts');
            return placement != null || kills != null || placementPts != null;
        });
        return hasMainTotals || hasMatchDetails;
    });
    const isPointsOnly = scoreMode === 'pointsOnly' || !hasDetailedScoreData;
    ...
    const tbody = rowsToRender.map((row, index) => {
        ...
        const team = otResolveTeamForTournament(rawTeam, currentOtData, teamOriginal);
        const logo = otGetTeamLogo(team);
        const spotRule = selectedMatch === 'all' ? otFindSpotRule(currentOtData, final, pos, 'final') : null;
        const spotTypeClass = otSpotClass(spotRule);
        const rowSpotClass = spotRule ? `ot-row-spot ${spotTypeClass}` : '';
        const posSpotClass = spotRule ? `ot-spot-marker ${spotTypeClass}` : '';
        const posSpotTitle = spotRule ? otEscapeHTML(spotRule.label || '') : '';
        ...
        if (selectedMatch !== 'all') {
            ...
            if (isPointsOnly) {
                return `
                    <tr class="${rowSpotClass}">
                        <td class="ot-pos-cell ${posSpotClass}" title="${posSpotTitle}">${pos}º</td>
                        <td class="ot-team-click" onclick="openOtTeamRoster('${otEscapeHTML(currentOtData.id)}', '${otEscapeHTML(team)}')">
    ...
    container.innerHTML = `
        <section class="ot-section-block">
            ...
            ${filterHtml}
            ${selectedMatch === 'all' ? otRenderSpotLegend(currentOtData, final, 'final') : ''}
            <div class="table-container ot-wide-table ot-mobile-table-wrap">
...
function renderOtPlayerRanking() {
    ...
    container.innerHTML = `
        <section class="ot-section-block">
            <div class="ot-section-head"><h3>${otEscapeHTML(ranking.title || 'Ranking de abates')}</h3></div>
            <div class="table-container ot-wide-table ot-mobile-table-wrap">
    ...
    const playersHtml = players.length ? players.map(p => {
        ...
        return `
            <button class="ot-roster-line ot-inline-player" data-player="${otEscapeHTML(playerName)}">
                <span>${otPlayerFlagHTML(playerName, flag)}${otEscapeHTML(playerName)}</span>
            </button>
        `;
