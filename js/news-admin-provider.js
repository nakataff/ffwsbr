  ...
  function render(items) {
    ...
    const mobileSlides = slides.map(function (item, index) {
      return '<a class="news-hero-slide ' + (index === 0 ? 'active' : '') + '" href="' + escapeHTML(item.urlInterna) + '">' +
        '<img src="' + escapeHTML(item.imagem || FALLBACK_IMAGE) + '" alt="' + escapeHTML(item.titulo) + '" width="1200" height="628" decoding="' + (index === 0 ? 'sync' : 'async') + '" ' + (index === 0 ? 'loading="eager" fetchpriority="high"' : 'loading="lazy" fetchpriority="low"') + ' onerror="this.src=\'' + FALLBACK_IMAGE + '\'">' +
