...
function handleGlobalSearchFrom(source) {
    ...
    db.players.forEach(p => {
        let lowerName = p.jogador.toLowerCase();
        if(!seen.has(lowerName)) {
            pool.push({ name: p.jogador, sub: p.equipe, type: 'player', isEx: p.isEx });
            seen.add(lowerName);
        }
    });
    ...
    if (typeof lbffData !== 'undefined') {
        Object.keys(lbffData).forEach(name => {
            ...
            if(!seen.has(lowerName)) {
                pool.push({ name: name, sub: 'JOGADOR HISTÓRICO', type: 'player', isEx: true });
                seen.add(lowerName);
            }
        });
    }
    ...
    dbStaff.forEach(s => {
        let lowerName = s.nome.toLowerCase();
        if(!seen.has(lowerName)) {
            pool.push({ name: s.nome, sub: `${s.equipe} (${s.cargo})`, type: 'staff' });
            seen.add(lowerName);
        }
    });
    ...
    let foundPeople = pool.filter(x => getDisplayName(x.name).toLowerCase().includes(input));
    ...
    html += foundPeople.map(p => {
        let img = p.isEx ? 'silhueta.png' : (p.type === 'staff' ? (staffPhotos[p.name] || 'silhueta.png') : (playerPhotos[p.name] || 'silhueta.png'));
        return `
