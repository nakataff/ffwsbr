...
function navPreferSearchResult(current, candidate) {
    ...
    // Se for o mesmo time da SEA, a versão oficial da SEA sempre vence a histórica.
...
function navDedupeSearchResults(results) {
    ...
    results.forEach(item => {
        ...
        const regularKey = `${item.type}:${String(item.name || item.title || '').toLowerCase()}`;
        const key = seaKey ? `sea-team:${seaKey}` : regularKey;
...
function navBuildAllSearchResults(rawInput) {
    ...
    navBuildPeopleSearchPool().forEach(p => {
        ...
        if (score >= 0) {
            ...
            results.push({
                ...
                sub: p.sub,
                img,
                score: score + (p.type === 'player' || p.type === 'sea-player' ? 5 : 0)
            });
        }
    });
    ...
    const grouped = all.reduce((acc, item) => {
        const key = (item.type === 'team' || item.type === 'sea-team') ? 'teams'
...
function navRenderDropdownItem(item) {
    ...
    const fallback = (item.type === 'team' || item.type === 'sea-team') ? 'escudo.webp' : (item.type === 'tournament' ? 'trofeu.webp' : (item.type === 'news' ? 'central free fire.webp' : 'silhueta.png'));
