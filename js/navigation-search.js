...
function navigateAndClose(pageId) {
    ...
    else if (pageId === 'notas-cff') renderNotasCFFPage();
    else if (pageId === 'equipes') renderTeamsDirectory();
    else if (pageId === 'datas') renderSchedule();
    else if (pageId === 'laff-2026-s1' && typeof loadLAFFData === 'function') loadLAFFData();
    ...
    else if (pageId === 'final' && typeof renderFinalPossibilities === 'function') renderFinalPossibilities();
    ...
    if (typeof window.getLAFFPlayers === 'function') {
        ...
        laffPlayers.forEach(p => {
            ...
            if (peopleMap.has(wbKey)) {
                // Jogador já está na WB — atualiza subtítulo para indicar que também joga LAFF
                const existing = peopleMap.get(wbKey);
                existing.sub = `${existing.sub || existing.name} • também na LAFF`;
                ...
                peopleMap.set(laffKey, {
                    ...
                    isEx: false,
                    priority: 4,
                    img: p.photo || (typeof getPlayerPhoto === 'function' ? getPlayerPhoto(name) : 'silhueta.png'),
...
function navRenderDropdownItem(item) {
    ...
    const fallback = (item.type === 'team' || item.type === 'sea-team' || item.type === 'laff-team') ? 'escudo.webp' : (item.type === 'tournament' ? 'trofeu.webp' : (item.type === 'news' ? 'central free fire.webp' : 'silhueta.png'));
