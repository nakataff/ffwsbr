...
function navTournamentMatchesSearch(t, rawInput) {
    ...
    const hasTextToken = tokens.some(token => /[a-z]/.test(token));
    if (!hasTextToken) return false;

    return tokens.every(token => searchable.includes(token) || compactSearchable.includes(token))
        || (compactQuery.length >= 4 && compactSearchable.includes(compactQuery));
}
