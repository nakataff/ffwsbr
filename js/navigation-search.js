...
function navBuildTeamSearchPool() {
    ...
    return Array.from(allTeamNames).filter(Boolean).map(tName => {
        const aliases = navGetTeamAliasesForSearch(tName);
        const isWB = (db.teams || []).some(x => String(x.equipe || '').toUpperCase() === String(tName || '').toUpperCase());
        return {
            type: 'team',
            name: tName,
            title: tName,
            sub: isWB ? 'Equipe Competidora' : 'Equipe Histórica',
            img: (logos && logos[tName]) || 'escudo.webp',
            haystack: aliases.join(' ')
        };
    });
}
...
function navBuildNewsSearchPool() {
    return navGetNewsListForSearch().map(n => ({
        ...
        haystack: [n.titulo, n.resumo, n.conteudo, n.autor].filter(Boolean).join(' ')
    }));
}
