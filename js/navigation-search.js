...
function navGetTournamentSearchText(t) {
    if (typeof otTournamentSearchText === 'function') return otTournamentSearchText(t);
    return [
        t?.id,
        t?.name,
        t?.nome,
...
function navTournamentMatchesSearch(t, rawInput) {
    ...
    const haystack = navNormalizeSearchText(navGetTournamentSearchText(t));
    const compactHaystack = haystack.replace(/\s+/g, '');
    const compactQuery = query.replace(/\s+/g, '');

    return query.split(' ').filter(Boolean).every(token => haystack.includes(token) || compactHaystack.includes(token))
...
function navGetTournamentSubForSearch(t) {
    ...
    const tier = typeof otGetTournamentTier === 'function' ? otFormatTier(otGetTournamentTier(t)) : (t?.tier || '');
    const region = typeof otGetTournamentRegion === 'function'
        ? (typeof otPrettyRegion === 'function' ? otPrettyRegion(otGetTournamentRegion(t)) : otGetTournamentRegion(t))
        : (t?.region || t?.regiao || '');
    ...
    return parts.length ? `Outros Torneios • ${parts.join(' • ')}` : 'Outros Torneios';
}
...
function handleGlobalSearchFrom(source) {
    ...
    let foundTeams = allTeamNames.filter(t => navNormalizeSearchText(t).includes(navNormalizeSearchText(input)));
    let foundPeople = Array.from(peopleMap.values()).filter(x => navNormalizeSearchText(x.haystack).includes(navNormalizeSearchText(input)));
    let foundTournaments = navGetTournamentListForSearch()
        .filter(t => navTournamentMatchesSearch(t, input))
        .slice(0, 8);
    ...
    html += foundTournaments.map(t => {
        const name = navGetTournamentNameForSearch(t);
        const id = t?.id || name;
        const logo = navGetTournamentLogoForSearch(t);
        const sub = navGetTournamentSubForSearch(t);

        return `
            <div class="search-item" data-type="tournament" data-name="${navSearchEscapeHTML(id)}" style="cursor:pointer;">
                <img src="${navSearchEscapeHTML(logo)}" onerror="this.src='trofeu.webp'">
                <div>
                    <span style="font-weight: bold; color: #fff;">${navSearchEscapeHTML(name)}</span><br>
                    <small style="color: #aaa;">${navSearchEscapeHTML(sub)}</small>
                </div>
            </div>`;
    }).join('');
