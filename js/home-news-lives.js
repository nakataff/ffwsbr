...
function cffSheetCacheUrl(url, ttlMs = 30 * 60 * 1000) {
    const sep = String(url || '').includes('?') ? '&' : '?';
    const build = (typeof CFF_BUILD_VERSION !== 'undefined') ? CFF_BUILD_VERSION : 'cff';
    const bucket = Math.floor(Date.now() / ttlMs);
    return `${url}${sep}v=${encodeURIComponent(`${build}-${bucket}`)}`;
}
...
async function loadPhotos() {
    ...
    window.__cffPhotosPromise = (async () => {
        ...
        try {
            const res = await fetch(cffSheetCacheUrl(PHOTOS_TSV_URL), { cache: 'force-cache' });
            const tsvText = await res.text();
            const lines = tsvText.split('\n');
