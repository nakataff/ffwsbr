...
function getHomeConfrontationTeamStats(teamNames) {
    const target = new Set(teamNames.map(t => String(t).toUpperCase()));
    const stats = {};
    teamNames.forEach(team => {
        stats[team] = { equipe: team, pontos: 0, abates: 0, booyah: 0, quedas: 0 };
    });

    if (typeof dbQuedas === 'undefined') return Object.values(stats);

    Object.values(dbQuedas || {}).forEach(day => {
        Object.values(day || {}).forEach(drop => {
            (drop.resultados || []).forEach(result => {
                const teamName = String(result.equipe || '').toUpperCase();
                if (!target.has(teamName)) return;
                const key = teamNames.find(t => String(t).toUpperCase() === teamName) || result.equipe;
    ...
    return Object.values(stats).sort((a, b) => b.pontos - a.pontos || b.abates - a.abates || b.booyah - a.booyah);
}

function getHomeConfrontationPlayerStats(teamNames) {
    const target = new Set(teamNames.map(t => String(t).toUpperCase()));
    const stats = {};

    if (typeof dbJogadoresQuedas === 'undefined') return [];

    Object.values(dbJogadoresQuedas || {}).forEach(day => {
        Object.values(day || {}).forEach(dropPlayers => {
            (dropPlayers || []).forEach(p => {
                const teamName = String(p.equipe || '').toUpperCase();
                if (!target.has(teamName)) return;
                const key = `${p.nome}__${p.equipe}`;
                if (!stats[key]) {
                    stats[key] = { jogador: p.nome, equipe: p.equipe, abates: 0, dano: 0, quedas: 0 };
                }
                stats[key].abates += Number(p.kills) || 0;
...
function buildHomeConfrontationText(teamStats, playerStats) {
    ...
    if (!bestTeam || !bestKiller) {
        return 'Ainda não há dados suficientes para gerar uma leitura forte deste confronto.';
    }

    let text = `${bestTeam.equipe} chega como a equipe mais forte deste confronto, liderando entre os times envolvidos com ${bestTeam.pontos} pontos e ${bestTeam.abates} abates.`;
    if (bestKiller && bestDamage && bestKiller.jogador === bestDamage.jogador) {
        text += ` ${bestKiller.jogador} é o destruidor do confronto: lidera em abates e também aparece como grande nome em dano.`;
    } else {
        text += ` ${bestKiller.jogador} é o destruidor do confronto em abates, enquanto ${bestDamage.jogador} aparece como o maior causador de dano.`;
    }
    return text;
}
...
function ensureHomeConfrontationSummaryButton() {
    ...
    if (!groupsEl || document.getElementById('btn-home-confrontation-summary')) return;

    const btn = document.createElement('button');
    btn.id = 'btn-home-confrontation-summary';
    btn.type = 'button';
    ...
    groupsEl.insertAdjacentElement('afterend', btn);
}
...
function openHomeConfrontationSummary() {
    ...
    const teamStats = getHomeConfrontationTeamStats(teamNames).slice(0, 5);
    const playerStats = getHomeConfrontationPlayerStats(teamNames).slice(0, 5);
    const analysis = buildHomeConfrontationText(teamStats, playerStats);
    ...
    body.innerHTML = `
        <div class="home-confrontation-subtitle">${cffHomeEscapeHTML(match.data)} • Grupos ${cffHomeEscapeHTML(groupsText)}</div>
            ...
            <div class="home-confrontation-block">
                ...
                <div class="table-container home-summary-table-wrap">
                    <table class="home-summary-table">
                        <thead><tr><th>#</th><th>Jogador</th><th>K</th><th>Dano</th></tr></thead>
                        <tbody>${playerRows || '<tr><td colspan="4">Sem dados.</td></tr>'}</tbody>
                    </table>
                </div>
            </div>
            <div class="home-confrontation-block">
                ...
                <div class="table-container home-summary-table-wrap">
                    <table class="home-summary-table">
                        <thead><tr><th>#</th><th>Time</th><th>PTS</th><th>K</th><th>B</th></tr></thead>
                        <tbody>${teamRows || '<tr><td colspan="5">Sem dados.</td></tr>'}</tbody>
                    </table>
                </div>
            </div>
        </div>`;
...
// =============================================
// COMPATIBILIDADE - NOME ANTIGO DO MODAL
// Alguns trechos antigos do HTML ainda chamam:
// openHomeConfrontoResumoModal()
// closeHomeConfrontoResumoModal()
// O JS novo usa:
