...
function renderFinalPossibilities() {
    ...
    if (!probGrid && !tableBody && !playersGrid) return;
    ...
    if (tableBody) {
        tableBody.innerHTML = rows.map(t => `<tr>
            ...
            <td style="text-align:left;"><span class="team-cell clickable" onclick="openTeamProfile('${cffHomeEscapeHTML(String(t.equipe).replace(/'/g, "\\'"))}')"><img class="team-logo" src="${getTeamLogoByAliases(t.equipe)}" onerror="this.src='escudo.webp'">${cffHomeEscapeHTML(t.equipe)}</span></td>
...
function cffFinalQualifiedBaseRows() {
    return cffFinalCalculateProbabilities().map(t => ({
        equipe: t.equipe,
        rank: t.rank,
...
function cffFinalAggregateRows() {
    ...
    qualified.forEach(t => {
        rows[t.equipe] = { equipe: t.equipe, pontos: 0, abates: 0, booyah: 0, quedas: 0, status: 'Na disputa', source: 'final' };
    });
    ...
    Object.keys(rawDrops).sort((a, b) => Number(a) - Number(b)).forEach(day => {
        ...
        Object.keys(rawDrops[day] || {}).sort((a, b) => Number(a) - Number(b)).forEach(round => {
            ...
            (drop?.resultados || []).forEach(res => {
                ...
                if (!rows[res.equipe]) rows[res.equipe] = { equipe: res.equipe, pontos: 0, abates: 0, booyah: 0, quedas: 0, status: 'Na disputa', source: 'final' };
    ...
    const champion = cffFinalSimulateChampion(rawDrops);
    return Object.values(rows)
        .map(t => {
            t.avg = t.quedas ? t.pontos / t.quedas : 0;
            if (champion && champion.equipe === t.equipe) t.status = champion.mode === 'champion-point' ? 'Campeão CP' : 'Campeão pts';
            else if (t.pontos >= CFF_FINAL_RULES.championPoint) t.status = 'Apto CP';
            else t.status = `${Math.max(0, CFF_FINAL_RULES.championPoint - t.pontos)} p/ CP`;
            return t;
        })
...
function cffFinalRenderSummary(rows) {
    ...
    if (!hasData) {
        box.innerHTML = '<strong>Aguardando dados da final.</strong><span>Suba o arquivo dados-final.json no repositório quando as quedas começarem. Por enquanto, a tabela mostra os classificados pela classificatória.</span>';
        return;
    }
    box.innerHTML = `
        <strong>${champion ? `Campeão detectado: ${cffHomeEscapeHTML(champion.equipe)}` : `Líder atual: ${cffHomeEscapeHTML(leader?.equipe || '-')}`}</strong>
        <span>${dropsPlayed}/${CFF_FINAL_RULES.totalDrops} quedas jogadas • Champion Point: ${CFF_FINAL_RULES.championPoint} pts • ${leader ? `${leader.pontos} pts / ${leader.abates} K` : ''}</span>
    `;
}
...
function renderFinalPossibilities() {
    ...
    if (probGrid) {
        const probRows = cffFinalCalculateProbabilities();
        probGrid.innerHTML = probRows.map((t, idx) => {
            const pct = Math.max(0.1, Number(t.probability) || 0);
    ...
    if (tableBody) {
        tableBody.innerHTML = rows.map(t => `<tr>
            ...
            <td><span class="final-status-pill">${cffHomeEscapeHTML(t.status || '-')}</span></td>
        </tr>`).join('');
    }

    if (playersGrid) {
        const sourcePlayers = hasFinalData && typeof cffGetPlayerDailyByStage === 'function'
            ? cffGetPlayerDailyByStage('final')
            : cffFinalBestPlayers(rows);
        const agg = {};
        sourcePlayers.forEach(p => {
            const name = p.jogador || p.nome;
            if (!agg[name]) agg[name] = { jogador: name, equipe: p.equipe, abates: 0, dano: 0 };
            agg[name].abates += Number(p.abates ?? p.kills) || 0;
            agg[name].dano += Number(p.dano) || 0;
        });
        const players = Object.values(agg).sort((a, b) => b.abates - a.abates || b.dano - a.dano).slice(0, 10);
        playersGrid.innerHTML = players.length ? players.map((p, idx) => `<button class="final-player-card" onclick="${_safePPAttr(p.jogador)}">
            <span>${idx + 1}º</span>
            <strong>${cffHomeEscapeHTML(getDisplayName(p.jogador))}</strong>
            <small>${cffHomeEscapeHTML(p.equipe)} • ${Number(p.abates) || 0} K</small>
