...
function openHomeConfrontationSummary() {
    ...
    const playerRows = playerStats.map((p, index) => `
        <tr>
            <td>${index + 1}º</td>
            <td class="home-summary-name">${cffHomeEscapeHTML(p.jogador)}<small>${cffHomeEscapeHTML(p.equipe)} • ${p.quedas} quedas</small></td>
            <td>${p.abates}</td>
    ...
    const teamRows = teamStats.map((t, index) => `
        <tr>
            ...
            <td class="home-summary-team-cell">
                ...
                <span>${cffHomeEscapeHTML(typeof shortNames !== 'undefined' && shortNames[t.equipe] ? shortNames[t.equipe] : t.equipe)}<small>${t.quedas} quedas no confronto</small></span>
            </td>
            <td>${t.pontos}</td>
            <td>${t.abates}</td>
            <td>${t.booyah}</td>
