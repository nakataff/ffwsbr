...
function renderHomeStats() {
    ...
    if (typeof renderHomeChampionPreview === 'function') renderHomeChampionPreview();
    if (typeof renderFinalsPage === 'function') renderFinalsPage();
}
...
function getNovosTorneiosIndividualAwardsForTeam(teamName) {
    const aliasSet = getTeamAliasSet(teamName);
    return getNovosTorneiosSafeList().flatMap(t => (t.awards || []).map(a => {
...
function ensureHomeConfrontationSummaryButton() {
    const existing = document.getElementById('home-confronto-resumo-btn') || document.getElementById('home-confrontation-main-btn');
    if (!existing) return;
    existing.id = 'home-confronto-resumo-btn';
    ...
    existing.innerHTML = '🏆 POSSIBILIDADE DAS FINAIS';
}
...
// =============================================
// FINAL FFWS BR - PROJEÇÃO / HOMEBASE
// =============================================
function cffFinalEscapeHTML(value) {
    return String(value ?? '').replace(/[&<>'"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[c]));
}

function cffFinalTeamLogo(teamName) {
    if (typeof getTeamLogoSafe === 'function') return getTeamLogoSafe(teamName);
    if (typeof logos !== 'undefined' && logos) return logos[teamName] || logos[String(teamName || '').toUpperCase()] || 'escudo.webp';
    return 'escudo.webp';
}

function cffFinalShortTeamName(teamName) {
    return (typeof shortNames !== 'undefined' && shortNames && shortNames[teamName]) ? shortNames[teamName] : teamName;
}

function getFinalsProjectionTeams() {
    if (typeof db === 'undefined' || !Array.isArray(db.teams)) return [];
    const teams = [...db.teams]
        .filter(t => t && String(t.equipe || '').trim())
    ...
    const maxRecent = Math.max(...teams.map(t => {
        const daily = (db.teamDaily && db.teamDaily[t.equipe]) ? db.teamDaily[t.equipe] : [];
        return daily.slice(-4).reduce((sum, d) => sum + (Number(d.pontos) || 0), 0);
    }), 1);

    const raw = teams.map((t, index) => {
        const daily = (db.teamDaily && db.teamDaily[t.equipe]) ? db.teamDaily[t.equipe] : [];
        const recent = daily.slice(-4).reduce((sum, d) => sum + (Number(d.pontos) || 0), 0);
        const ptsScore = (Number(t.pontos) || 0) / maxPts;
        const killScore = (Number(t.abates) || 0) / maxKills;
        const booyahScore = (Number(t.booyah) || 0) / maxBooyah;
        const recentScore = recent / maxRecent;
        const seedBonus = Math.max(0, (12 - index) / 12) * 0.18;
        const score = (ptsScore * 0.44) + (killScore * 0.24) + (booyahScore * 0.16) + (recentScore * 0.16) + seedBonus;
        return { ...t, seed: index + 1, recent, finalScore: score };
    });

    const total = raw.reduce((sum, t) => sum + t.finalScore, 0) || 1;
    return raw.map(t => ({ ...t, chance: (t.finalScore / total) * 100 }))
        .sort((a, b) => b.chance - a.chance || b.pontos - a.pontos);
}

function getFinalsPlayerRanking(finalTeams) {
    if (typeof db === 'undefined' || !Array.isArray(db.players)) return [];
    const finalTeamSet = new Set(finalTeams.map(t => String(t.equipe || '').toUpperCase()));
    return [...db.players]
        .filter(p => p && !p.isEx && finalTeamSet.has(String(p.equipe || '').toUpperCase()))
        .sort((a, b) => (Number(b.abates) || 0) - (Number(a.abates) || 0) || (Number(b.dano) || 0) - (Number(a.dano) || 0))
        .slice(0, 12);
}

function renderHomeChampionPreview() {
    const grid = document.getElementById('home-champion-grid');
    if (!grid) return;
    const finalTeams = getFinalsProjectionTeams();
    if (!finalTeams.length) {
        grid.innerHTML = '<div class="home-champion-loading">Carregando classificados...</div>';
        return;
    }

    grid.innerHTML = finalTeams.slice(0, 12).map((t, index) => `
        <button type="button" class="home-champion-card" onclick="openTeamProfile('${cffFinalEscapeHTML(String(t.equipe || '').replace(/'/g, "\\'"))}')">
            <span class="home-champion-rank">${index + 1}º</span>
            <img src="${cffFinalEscapeHTML(cffFinalTeamLogo(t.equipe))}" alt="${cffFinalEscapeHTML(t.equipe)}" onerror="this.onerror=null;this.src='escudo.webp'">
            <strong>${cffFinalEscapeHTML(cffFinalShortTeamName(t.equipe))}</strong>
            <em>${t.chance.toFixed(1)}%</em>
        </button>
    `).join('');
}

function renderFinalsPage() {
    const titleGrid = document.getElementById('final-title-grid');
    const teamsTbody = document.getElementById('final-teams-tbody');
    const playersTbody = document.getElementById('final-players-tbody');
    if (!titleGrid || !teamsTbody || !playersTbody) return;

    const finalTeams = getFinalsProjectionTeams();
    if (!finalTeams.length) {
        titleGrid.innerHTML = '<div class="final-empty">Carregando finalistas...</div>';
        teamsTbody.innerHTML = '<tr><td colspan="7">Carregando...</td></tr>';
        playersTbody.innerHTML = '<tr><td colspan="8">Carregando...</td></tr>';
        return;
    }

    titleGrid.innerHTML = finalTeams.map((t, index) => `
        <button type="button" class="final-title-card" onclick="openTeamProfile('${cffFinalEscapeHTML(String(t.equipe || '').replace(/'/g, "\\'"))}')">
            <div class="final-title-top">
                <span>${index + 1}º</span>
                <strong>${t.chance.toFixed(1)}%</strong>
            </div>
            <img src="${cffFinalEscapeHTML(cffFinalTeamLogo(t.equipe))}" alt="${cffFinalEscapeHTML(t.equipe)}" onerror="this.onerror=null;this.src='escudo.webp'">
            <h3>${cffFinalEscapeHTML(cffFinalShortTeamName(t.equipe))}</h3>
            <p>${Number(t.pontos) || 0} pts • ${Number(t.abates) || 0} abates • ${Number(t.booyah) || 0} B!</p>
        </button>
    `).join('');

    teamsTbody.innerHTML = finalTeams.map((t, index) => `
        <tr>
            <td class="final-rank-cell">${index + 1}º</td>
            <td class="final-team-cell" onclick="openTeamProfile('${cffFinalEscapeHTML(String(t.equipe || '').replace(/'/g, "\\'"))}')">
                <img src="${cffFinalEscapeHTML(cffFinalTeamLogo(t.equipe))}" alt="${cffFinalEscapeHTML(t.equipe)}" onerror="this.onerror=null;this.src='escudo.webp'">
                <span>${cffFinalEscapeHTML(t.equipe)}</span>
            </td>
            <td>${Number(t.pontos) || 0}</td>
            <td>${Number(t.abates) || 0}</td>
            <td>${Number(t.booyah) || 0}</td>
            <td>${Number(t.quedas) || 0}</td>
            <td class="final-percent-cell">${t.chance.toFixed(1)}%</td>
        </tr>
    `).join('');

    const players = getFinalsPlayerRanking(finalTeams);
    playersTbody.innerHTML = players.map((p, index) => `
        <tr>
            <td class="final-rank-cell">${index + 1}º</td>
            <td class="final-player-cell" onclick="openPlayerProfile('${cffFinalEscapeHTML(String(p.jogador || '').replace(/'/g, "\\'"))}')">
                <strong>${cffFinalEscapeHTML(typeof getDisplayName === 'function' ? getDisplayName(p.jogador) : p.jogador)}</strong>
            </td>
            <td class="final-team-mini" onclick="openTeamProfile('${cffFinalEscapeHTML(String(p.equipe || '').replace(/'/g, "\\'"))}')">
                <img src="${cffFinalEscapeHTML(cffFinalTeamLogo(p.equipe))}" alt="${cffFinalEscapeHTML(p.equipe)}" onerror="this.onerror=null;this.src='escudo.webp'">
                <span>${cffFinalEscapeHTML(cffFinalShortTeamName(p.equipe))}</span>
            </td>
            <td>${Number(p.abates) || 0}</td>
            <td>${formatHomeCompactNumber(Number(p.dano) || 0)}</td>
            <td>${Number(p.assists) || 0}</td>
            <td>${Number(p.quedas) || 0}</td>
            <td>${Number(p.mvp) || 0}</td>
        </tr>
    `).join('') || '<tr><td colspan="8">Sem jogadores encontrados.</td></tr>';
}

// =============================================
// COMPATIBILIDADE / LIMPEZA DO BOTÃO DA HOME
// =============================================
window.openHomeConfrontoResumoModal = function () { navigate('final'); if (typeof renderFinalsPage === 'function') renderFinalsPage(); };
