...
// =============================================
// FINAL WB 2026 S1 - POSSIBILIDADE DAS FINAIS
// =============================================
function cffFinalGetQualifiedTeams() {
    if (typeof db === 'undefined' || !Array.isArray(db.teams)) return [];
    const base = (typeof cffGetClassificatoriaTeams === 'function') ? cffGetClassificatoriaTeams() : [...db.teams];
    return base
...
function cffFinalTeamRecentPoints(teamName) {
    ...
    const sorted = [...daily].filter(d => (typeof cffIsDayInStage === 'function') ? cffIsDayInStage(d.dia, 'classificatoria') : true).sort((a, b) => Number(a.dia) - Number(b.dia));
...
function cffFinalTeamConsistency(teamName) {
    ...
    const pts = daily.filter(d => (typeof cffIsDayInStage === 'function') ? cffIsDayInStage(d.dia, 'classificatoria') : true).map(d => Number(d.pontos) || 0).filter(v => v >= 0);
...
function cffAggregateFinalStandings() {
    const qualified = cffFinalGetQualifiedTeams().map((t, index) => ({
        equipe: t.equipe,
        grupo: t.grupo,
        classificatoriaRank: index + 1,
        classificatoriaPts: Number(t.pontos) || 0,
        pontos: 0,
        booyah: 0,
        abates: 0,
        quedas: 0
    }));
    const byName = new Map(qualified.map(t => [String(t.equipe).toUpperCase(), t]));

    const selectedDays = selectedFinalDays && selectedFinalDays.length ? selectedFinalDays.map(String) : (typeof cffGetDbDaysByStage === 'function' ? cffGetDbDaysByStage('final') : []);
    const selectedDaySet = new Set(selectedDays);
    const mapSelect = document.getElementById('final-map-filter');
    const selectedMap = mapSelect ? normalizeMapName(mapSelect.value) : 'all';

    let hasAnyFinalDrop = false;
    let hasFilteredDrop = false;

    for (const d in (dbQuedas || {})) {
        if ((typeof cffIsDayInStage === 'function') && !cffIsDayInStage(d, 'final')) continue;
        hasAnyFinalDrop = true;
        if (selectedDaySet.size && !selectedDaySet.has(String(d))) continue;

        for (const q in dbQuedas[d]) {
            const drop = dbQuedas[d][q];
            const currentMap = normalizeMapName(drop.mapa);
            if (selectedMap !== 'all' && currentMap !== selectedMap) continue;
            hasFilteredDrop = true;

            (drop.resultados || []).forEach(res => {
                const row = byName.get(String(res.equipe).toUpperCase());
                if (!row) return;
                const ptsPos = posPoints[res.posicao] || 0;
                row.pontos += ptsPos + (Number(res.kills) || 0);
                row.abates += Number(res.kills) || 0;
                row.booyah += Number(res.booyah) || 0;
                row.quedas += 1;
            });
        }
    }

    const rows = qualified.sort((a, b) => {
        if (hasFilteredDrop) return (Number(b.pontos) || 0) - (Number(a.pontos) || 0) || (Number(b.abates) || 0) - (Number(a.abates) || 0) || a.classificatoriaRank - b.classificatoriaRank;
        return a.classificatoriaRank - b.classificatoriaRank;
    }).map((t, idx) => ({ ...t, rank: idx + 1, avg: t.quedas ? t.pontos / t.quedas : 0 }));

    return { rows, hasAnyFinalDrop, hasFilteredDrop };
}

function renderFinalStandingsPage() {
    if (typeof buildDayFilters === 'function') buildDayFilters();

    const tableBody = document.querySelector('#final-teams-table tbody');
    const note = document.getElementById('final-table-note');
    if (!tableBody) return;

    const { rows, hasAnyFinalDrop, hasFilteredDrop } = cffAggregateFinalStandings();
    if (note) {
        if (!hasAnyFinalDrop) {
            note.style.display = 'block';
            note.textContent = 'A final ainda não tem quedas cadastradas no dados.json. Por enquanto, a tabela mostra os 12 classificados pela ordem da classificatória.';
        } else if (!hasFilteredDrop) {
            note.style.display = 'block';
            note.textContent = 'Nenhuma queda encontrada para este filtro. A lista abaixo mantém os 12 classificados.';
        } else {
    ...
    tableBody.innerHTML = rows.map(t => {
        const teamForJs = cffHomeEscapeHTML(String(t.equipe).replace(/'/g, "\\'"));
        return `<tr>
            <td style="color:var(--accent); font-weight:900;">${t.rank}º</td>
            <td style="text-align:left;"><span class="team-cell clickable" onclick="openTeamProfile('${teamForJs}')"><img class="team-logo" src="${getTeamLogoByAliases(t.equipe)}" onerror="this.src='escudo.webp'">${cffHomeEscapeHTML(t.equipe)}</span></td>
...
function renderFinalPossibilities() {
    ...
    const playersGrid = document.getElementById('final-players-grid');
    if (!probGrid && !playersGrid) return;

    const rows = cffFinalCalculateProbabilities();
    if (!rows.length) {
        if (probGrid) probGrid.innerHTML = '<div class="home-confrontation-empty">Ainda não foi possível montar as possibilidades da final.</div>';
        return;
    }

    if (probGrid) {
        probGrid.innerHTML = rows.map((t, idx) => {
            const pct = Math.max(0.1, t.probability);
            return `<button class="final-prob-card" onclick="openTeamProfile('${cffHomeEscapeHTML(String(t.equipe).replace(/'/g, "\\'"))}')">
                ...
                <div class="final-prob-info">
                    ...
                    <span>${t.pontos} pts • ${t.abates} abates • ${t.booyah || 0} booyah</span>
    ...
    renderFinalStandingsPage();


    if (playersGrid) {
        const players = cffFinalBestPlayers(rows);
        playersGrid.innerHTML = players.length ? players.map((p, idx) => `<button class="final-player-card" onclick="${_safePPAttr(p.jogador)}">
            <span>${idx + 1}º</span>
            <strong>${cffHomeEscapeHTML(getDisplayName(p.jogador))}</strong>
            <small>${cffHomeEscapeHTML(p.equipe)} • ${Number(p.abates) || 0} K</small>
        </button>`).join('') : '<div class="home-confrontation-empty">Nenhum jogador encontrado.</div>';
    }
}

window.renderFinalStandingsPage = renderFinalStandingsPage;
