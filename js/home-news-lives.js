...
const TEAM_ALIAS_FALLBACK = {
    ...
    'KEYD STARS': ['KEYD STARS', 'KEYD', 'VIVO KEYD'],
    ...
    'B4STARDOS': ['B4STARDOS', 'B4 ESPORTS', 'B4'],
    'TEAM LIQUID': ['TEAM LIQUID', 'LIQUID'],
    'PAIN GAMING': ['PAIN GAMING', 'PAIN', 'PNG'],
    'BD LOS GRANDES': ['BD LOS GRANDES', 'BD'],
...
function getTeamCanonicalName(nome) {
    ...
    for (const [canonical, aliases] of Object.entries(dbTeamAliases || TEAM_ALIAS_FALLBACK)) {
...
function getTeamAliasSet(nome) {
    ...
    addList(dbTeamAliases[canonical]);
...
function getResultsByTeamAliases(nome) {
    ...
    Object.entries(dbResults || {}).forEach(([teamName, rows]) => {
        if (aliasSet.has(normalizeTeamAlias(teamName))) {
            results.push(...(rows || []));
        }
    });
    return results;
...
function getNovosTorneiosResultsForTeam(nome) {
    ...
    tournaments.forEach(t => {
        const tables = [];
        if (Array.isArray(t.standings)) tables.push(...t.standings);
        if (t.final) tables.push(t.final);

        tables.forEach(table => {
            const rows = table.rows || table.linhas || [];
            rows.forEach(row => {
                const team = getCompactFieldFromCols(row, table, 'team', '');
                const teamOriginal = getCompactFieldFromCols(row, table, 'teamOriginal', '');
                if (!aliasSet.has(normalizeTeamAlias(team)) && !aliasSet.has(normalizeTeamAlias(teamOriginal))) return;

                const pos = getCompactFieldFromCols(row, table, 'pos', getCompactFieldFromCols(row, table, 'posicao', ''));
                results.push({
...
async function openOutraEquipe(nome) {
    ...
    document.getElementById('oep-logo').src = logos[canonicalName] || logos[nome] || 'escudo.webp';
    ...
    const titulos = (titlesData.coletivos || []).filter(t => aliasSet.has(normalizeTeamAlias(t.team)));
