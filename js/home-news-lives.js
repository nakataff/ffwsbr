...
function renderHomeStats() {
    // A Home já usa os dados da Final nos rankings.
    // Quando a Final acaba, só o card "Próxima Partida" vira o banner do campeão/EWC.
    // As áreas "Ao Vivo Agora" e "Próximas Lives" continuam normais.
...
async function loadTeamLogos() {
    ...
    try {
        ...
        // Após carregar as logos, atualiza só a Home de imediato.
        // As páginas ocultas renderizam sob demanda pelo menu para não travar o carregamento inicial.
        if (typeof renderHomeStats === 'function') renderHomeStats();
        if (typeof cffLazyLoadVisibleImages === 'function') cffLazyLoadVisibleImages();

        const refreshActiveLogoConsumers = () => {
            const activePage = document.querySelector('.page.active')?.id || 'home';
            try {
                if (activePage === 'tabela') { renderFullTeams(); renderGroupsTables(); }
                if (activePage === 'selecao-da-semana' && typeof renderSelection === 'function') renderSelection();
                if (activePage === 'stats') {
                ...
                if (activePage === 'notas-cff' && typeof renderNotasCFFPage === 'function') renderNotasCFFPage();
                if (activePage === 'ffws-sea-2026-s1') {
                    ...
                    if (typeof applySEAAbatesFilters === 'function' && typeof seaAbatesParsed !== 'undefined' && seaAbatesParsed?.headers?.length) applySEAAbatesFilters();
                }
            } catch (e) { console.warn('[loadTeamLogos] atualização da página ativa falhou:', e); }
        };
        if (typeof cffRunWhenIdle === 'function') cffRunWhenIdle(refreshActiveLogoConsumers, 900);
        else setTimeout(refreshActiveLogoConsumers, 900);
    } catch (e) {
...
function ensureHomeConfrontationSummaryButton() {
    // A Home não deve mais criar botão de probabilidade/resumo/votação.
    const oldBtn = document.getElementById('home-confrontation-main-btn');
    if (oldBtn) oldBtn.remove();
    const voteCard = document.getElementById('day-vote-card');
    if (voteCard) voteCard.remove();
}
...
function cffHomeRenderFinalStats() {
    ...
    const allFinalRows = cffHomeGetFinalDisplayRows();
    const rawDrops = (typeof cffFinalGetRawDrops === 'function') ? cffFinalGetRawDrops() : {};
    const champion = (typeof cffFinalSimulateChampion === 'function') ? cffFinalSimulateChampion(rawDrops) : null;
    const ewcSet = (typeof cffFinalGetEwcHighlightedTeams === 'function') ? cffFinalGetEwcHighlightedTeams(allFinalRows, champion) : new Set();
    const teamRows = allFinalRows.slice(0, 6);
    ...
    if (teamsBody) {
        teamsBody.innerHTML = teamRows.map((t, i) => {
            const isEwc = ewcSet.has(normalizeTeamAlias(t.equipe));
            return `
            <tr>
                <td style="color:${isEwc ? '#4aa8ff' : 'var(--accent)'}; font-weight:bold;">${i + 1}º</td>
                <td class="clickable" onclick="openTeamProfile('${cffFinalEscapeTeamForClick(t.equipe)}')" style="text-align:left; border-bottom: none;">
                    <div style="display:flex; align-items:center; gap:8px;">
                        ...
                        <span style="font-weight:bold; color:#fff;">${cffHomeEscapeHTML(shortNames[t.equipe] || t.equipe)}${isEwc ? '<span class="home-ewc-plane" title="Classificando para a EWC 2026">✈️</span>' : ''}</span>
                    </div>
                    <div style="font-size: 0.75em; color: #888; margin-left: 28px;">${Number(t.booyah) || 0} B! • ${Number(t.abates) || 0} K • ${Number(t.quedas) || 0} Q</div>
                </td>
                <td style="color:${isEwc ? '#4aa8ff' : 'var(--accent)'}; font-weight:bold;">${Number(t.pontos) || 0}</td>
            </tr>`;
        }).join('');
    }
...
function cffHomeFinalTeamCard(team, opts = {}) {
    ...
    const badgeLabel = opts.badge === 'EWC' ? '✈️ EWC' : opts.badge;
    const badge = opts.badge ? `<span style="font-size:.68rem; font-weight:950; color:${opts.badgeColor || '#4aa8ff'}; border:1px solid rgba(74,168,255,.35); border-radius:999px; padding:3px 8px; background:rgba(74,168,255,.08);">${badgeLabel}</span>` : '';
    return `
...
function cffHomeRenderPostFinalSpotlight() {
    ...
    const nextWidget = document.getElementById('home-next-groups');
    if (!nextWidget) return false;

    const rawDrops = (typeof cffFinalGetRawDrops === 'function') ? cffFinalGetRawDrops() : {};
    const champion = (typeof cffFinalSimulateChampion === 'function') ? cffFinalSimulateChampion(rawDrops) : null;
    const rows = cffHomeGetFinalDisplayRows();
    const championRow = rows.find(t => typeof cffFinalIsSameTeam === 'function' && cffFinalIsSameTeam(t.equipe, champion?.equipe)) || rows[0];
    const ewcSet = (typeof cffFinalGetEwcHighlightedTeams === 'function') ? cffFinalGetEwcHighlightedTeams(rows, champion) : new Set();
    const otherEwc = rows
        .filter(t => ewcSet.has(normalizeTeamAlias(t.equipe)) && !(typeof cffFinalIsSameTeam === 'function' && cffFinalIsSameTeam(t.equipe, championRow?.equipe)))
        .slice(0, 2);
    ...
    const championHtml = championRow ? cffHomeFinalTeamCard(championRow, { big: true, badge: 'CAMPEÃO', badgeColor: '#ffd700' }) : '';
    const ewcHtml = otherEwc.length
        ? otherEwc.map(t => cffHomeFinalTeamCard(t, { badge: 'EWC' })).join('')
        : '<div class="lives-empty">Aguardando definição das vagas.</div>';

    nextWidget.style.width = '100%';
    nextWidget.style.display = 'grid';
    nextWidget.style.gridTemplateColumns = '1fr';
    nextWidget.style.gap = '12px';
...
(window.cffRunWhenIdle || function(cb){ return setTimeout(cb, 1800); })(function () {
    if (typeof cffHomeRenderPostFinalSpotlight === 'function') cffHomeRenderPostFinalSpotlight();
}, 1800);
