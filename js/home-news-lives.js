...
async function loadNoticias() {
    ...
    function parseNoticiasTSV(tsvText) {
        ...
        return rows.slice(1).map((cells, index) => {
            ...
            noticia.urlInterna = `noticia.html?id=${encodeURIComponent(noticia.id)}`;
...
function updateLivesTick() {
    ...
    timeStr = d > 0 ? `${d}d ${hStr}:${mStr}:${sStr}` : `${hStr}:${mStr}:${sStr}`;
        ...
        if (live.ativo || isAutoLive) {
            ...
            // Live encerrada ou sem agendamento futuro: não aparece mais em Próximas Lives.
...
/* =============================================
   HOME - RESUMO DO CONFRONTO ESPECÍFICO
   Calcula desempenho apenas nos dias em que os mesmos grupos se enfrentaram.
   ============================================= */
function _cffEscapeHtml(value) {
    return String(value ?? '').replace(/[&<>"']/g, char => ({
        '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;'
    }[char]));
}

function _cffNormalizeTeamName(value) {
    return String(value || '').trim().toUpperCase();
}

function _cffSortedGroups(groups) {
    return [...new Set((groups || []).map(g => String(g || '').trim()).filter(Boolean))].sort();
}

function _cffSameGroups(a, b) {
    const ga = _cffSortedGroups(a);
    const gb = _cffSortedGroups(b);
    return ga.length === gb.length && ga.every((g, i) => g === gb[i]);
}

function _cffParseBRMatchDate(dateStr) {
    const months = {
        'janeiro': 0, 'fevereiro': 1, 'marco': 2, 'março': 2, 'abril': 3,
        'maio': 4, 'junho': 5, 'julho': 6, 'agosto': 7,
        'setembro': 8, 'outubro': 9, 'novembro': 10, 'dezembro': 11
    };
    const clean = String(dateStr || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
    const match = clean.match(/(\d{1,2})\s+de\s+([a-z]+)/i);
    if (!match) return new Date(2026, 0, 1, 13, 0, 0);
    const day = parseInt(match[1], 10);
    const month = months[match[2]] ?? 0;
    return new Date(2026, month, day, 13, 0, 0);
}

function getHomeNextConfrontoMatch() {
    if (typeof agenda === 'undefined' || !Array.isArray(agenda) || !agenda.length) return null;
    const now = new Date();
    return agenda.find(m => {
        const start = _cffParseBRMatchDate(m.data);
        const end = new Date(start.getTime() + (3.5 * 60 * 60 * 1000));
        return end.getTime() > now.getTime();
    }) || agenda[agenda.length - 1];
}

function getHomeConfrontoDayKeys(targetGroups) {
    if (typeof db === 'undefined' || !db.teams || typeof dbQuedas === 'undefined') return [];
    const teamGroupMap = {};
    db.teams.forEach(t => { teamGroupMap[_cffNormalizeTeamName(t.equipe)] = t.grupo; });

    return Object.keys(dbQuedas || {}).filter(dayKey => {
        const drops = dbQuedas[dayKey] || {};
        const groups = new Set();
        Object.values(drops).forEach(drop => {
            (drop.resultados || []).forEach(res => {
                const group = teamGroupMap[_cffNormalizeTeamName(res.equipe)];
                if (group) groups.add(group);
            });
        });
        return _cffSameGroups([...groups], targetGroups);
    }).sort((a, b) => Number(a) - Number(b));
}

function buildHomeConfrontoResumoData() {
    const match = getHomeNextConfrontoMatch();
    if (!match || !match.grupos) return null;

    const targetGroups = _cffSortedGroups(match.grupos);
    const dayKeys = getHomeConfrontoDayKeys(targetGroups);
    ...
    dayKeys.forEach(dayKey => {
        ...
        Object.values(drops).forEach(drop => {
            (drop.resultados || []).forEach(res => {
                ...
                const group = teamGroupMap[_cffNormalizeTeamName(team)];
                if (!targetGroupSet.has(group)) return;

                if (!teamStats[team]) teamStats[team] = { equipe: team, pontos: 0, abates: 0, booyah: 0, quedas: 0 };
                const pos = Number(res.posicao) || 12;
                const kills = Number(res.kills) || 0;
                const booyah = Number(res.booyah) || 0;
                const posPts = (typeof posPoints !== 'undefined' && posPoints[pos]) ? posPoints[pos] : 0;

                teamStats[team].pontos += posPts + kills;
                teamStats[team].abates += kills;
                teamStats[team].booyah += booyah;
                teamStats[team].quedas += 1;
            });
        });

        const playerDrops = dbJogadoresQuedas?.[dayKey] || {};
        Object.values(playerDrops).forEach(players => {
            (players || []).forEach(p => {
                const team = p.equipe;
                const group = teamGroupMap[_cffNormalizeTeamName(team)];
                if (!targetGroupSet.has(group)) return;

                const key = `${p.nome}||${team}`;
                if (!playerStats[key]) playerStats[key] = { nome: p.nome, equipe: team, kills: 0, dano: 0, assists: 0, mvps: 0, quedas: 0 };
                playerStats[key].kills += Number(p.kills) || 0;
                playerStats[key].dano += Number(p.dano) || 0;
                playerStats[key].assists += Number(p.assists) || 0;
                playerStats[key].mvps += Number(p.mvp) || 0;
                playerStats[key].quedas += 1;
            });
        });
    });

    const topTeams = Object.values(teamStats).sort((a, b) =>
        b.pontos - a.pontos || b.abates - a.abates || b.booyah - a.booyah
    );

    const topPlayersKills = Object.values(playerStats).sort((a, b) =>
        b.kills - a.kills || b.dano - a.dano
    );

    const topPlayersDano = Object.values(playerStats).sort((a, b) =>
        b.dano - a.dano || b.kills - a.kills
    );

    return { match, targetGroups, dayKeys, topTeams, topPlayersKills, topPlayersDano };
}

function ensureHomeConfrontoResumoModal() {
    let modal = document.getElementById('home-confronto-resumo-modal');
    if (modal) return modal;

    modal = document.createElement('div');
    modal.id = 'home-confronto-resumo-modal';
    modal.className = 'home-confronto-modal-overlay';
    modal.onclick = function(event) {
        if (event.target === modal) closeHomeConfrontoResumoModal();
    };
    modal.innerHTML = `
        <div class="home-confronto-modal">
            <div class="home-confronto-modal-header">
                <div>
                    <div class="home-confronto-kicker">Resumo do confronto</div>
                    <h2 id="home-confronto-modal-title">Carregando...</h2>
                    <p id="home-confronto-modal-subtitle"></p>
                </div>
                <button class="home-confronto-close" onclick="closeHomeConfrontoResumoModal()" aria-label="Fechar">×</button>
            </div>
            <div id="home-confronto-auto-text" class="home-confronto-auto-text"></div>
            <div class="home-confronto-grid">
                <div class="home-confronto-box">
                    <h3>🔥 5 melhores jogadores</h3>
                    <div id="home-confronto-top-players"></div>
                </div>
                <div class="home-confronto-box">
                    <h3>🏆 5 melhores times</h3>
                    <div id="home-confronto-top-teams"></div>
                </div>
            </div>
        </div>`;
...
function _cffFormatNumber(value) {
    return Number(value || 0).toLocaleString('pt-BR');
}

function _cffTeamLogo(team) {
    return (typeof logos !== 'undefined' && logos[team]) ? logos[team] : 'escudo.webp';
}

function _cffShortTeam(team) {
    return (typeof shortNames !== 'undefined' && shortNames[team]) ? shortNames[team] : team;
}

function _cffOpenPlayerAction(name) {
    const safe = String(name || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    return `openPlayerProfile('${safe}')`;
}

function renderHomeConfrontoResumoModalContent() {
    const data = buildHomeConfrontoResumoData();
    const title = document.getElementById('home-confronto-modal-title');
    const subtitle = document.getElementById('home-confronto-modal-subtitle');
    const autoText = document.getElementById('home-confronto-auto-text');
    const playersEl = document.getElementById('home-confronto-top-players');
    const teamsEl = document.getElementById('home-confronto-top-teams');

    if (!data || !data.dayKeys.length) {
        if (title) title.textContent = 'Sem dados para este confronto';
        if (subtitle) subtitle.textContent = 'Ainda não encontrei quedas registradas com essa combinação de grupos.';
        if (autoText) autoText.innerHTML = 'Assim que o dados.json tiver resultados desse confronto, o resumo será preenchido automaticamente.';
        if (playersEl) playersEl.innerHTML = '';
        if (teamsEl) teamsEl.innerHTML = '';
        return;
    }

    const groupLabel = data.targetGroups.join(' x ');
    if (title) title.textContent = `Grupos ${groupLabel}`;
    if (subtitle) subtitle.textContent = `Baseado nos dias já registrados com esse confronto: ${data.dayKeys.map(d => `Dia ${d}`).join(', ')}.`;

    const bestTeam = data.topTeams[0];
    const killer = data.topPlayersKills[0];
    const damage = data.topPlayersDano[0];

    if (autoText) {
        autoText.innerHTML = `
            <strong>${_cffEscapeHtml(bestTeam.equipe)}</strong> é a melhor equipe deste confronto até agora, com <strong>${bestTeam.pontos} pontos</strong>, ${bestTeam.abates} abates e ${bestTeam.booyah} booyah(s).
            <strong>${_cffEscapeHtml(killer.nome)}</strong> é o destruidor do confronto, liderando em abates com <strong>${killer.kills} kills</strong>.
            ${damage && damage.nome !== killer.nome ? `<strong>${_cffEscapeHtml(damage.nome)}</strong> é o maior causador de dano, com <strong>${_cffFormatNumber(damage.dano)}</strong> de dano.` : `Ele também aparece forte em dano, com <strong>${_cffFormatNumber(killer.dano)}</strong> de dano.`}
        `;
    }
        ...
        playersEl.innerHTML = data.topPlayersKills.slice(0, 5).map((p, index) => `
            <div class="home-confronto-row" onclick="${_cffOpenPlayerAction(p.nome)}">
                ...
                <div class="home-confronto-main">
                    ...
                    <span>${_cffEscapeHtml(_cffShortTeam(p.equipe))}</span>
                </div>
                <div class="home-confronto-stats">
                    <b>${p.kills}</b><small>KILLS</small>
                    <b>${_cffFormatNumber(p.dano)}</b><small>DANO</small>
                </div>
            </div>
        `).join('');
    }
    ...
    if (teamsEl) {
        teamsEl.innerHTML = data.topTeams.slice(0, 5).map((t, index) => `
            <div class="home-confronto-row" onclick="openTeamProfile('${String(t.equipe).replace(/'/g, "\\'")}')">
                ...
                <div class="home-confronto-main">
                    <strong>${_cffEscapeHtml(_cffShortTeam(t.equipe))}</strong>
                    <span>${t.quedas} quedas no confronto</span>
                </div>
                <div class="home-confronto-stats team-stats">
                    <b>${t.pontos}</b><small>PTS</small>
                    <b>${t.abates}</b><small>K</small>
                    <b>${t.booyah}</b><small>B</small>
                </div>
            </div>
        `).join('');
    }
...
function openHomeConfrontoResumoModal() {
    const modal = ensureHomeConfrontoResumoModal();
    renderHomeConfrontoResumoModalContent();
    modal.classList.add('active');
    document.body.style.overflow = 'hidden';
}

function closeHomeConfrontoResumoModal() {
    const modal = document.getElementById('home-confronto-resumo-modal');
