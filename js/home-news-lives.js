...
function cffFinalQualifiedBaseRows() {
    return cffFinalGetQualifiedTeams().map((t, index) => ({
        ...
        avg: 0,
        probability: 0,
        status: `${CFF_FINAL_RULES.championPoint} p/ CP`,
        source: 'pre-final'
    }));
}
...
function cffFinalCalculateLivePossibilities(rows) {
    ...
    const totalPlayedPoints = list.reduce((sum, t) => sum + (Number(t.pontos) || 0), 0);
    if (totalPlayedPoints <= 0) {
        const even = 100 / list.length;
        return list.map(t => ({ ...t, probability: even }));
    }

    const maxPts = Math.max(...list.map(t => Number(t.pontos) || 0), 1);
    const maxKills = Math.max(...list.map(t => Number(t.abates) || 0), 1);
    const maxBooyah = Math.max(...list.map(t => Number(t.booyah) || 0), 1);
    const scores = list.map(t => {
        const pontos = Number(t.pontos) || 0;
        const kills = Number(t.abates) || 0;
        const booyah = Number(t.booyah) || 0;
        const cpGap = Math.max(0, CFF_FINAL_RULES.championPoint - pontos);
        const cpPressure = pontos >= CFF_FINAL_RULES.championPoint ? 1.55 : 1 + ((CFF_FINAL_RULES.championPoint - cpGap) / CFF_FINAL_RULES.championPoint) * 0.35;
        const pointsPower = Math.pow(Math.max(0.04, pontos / maxPts), 2.15);
        const killFactor = 0.86 + 0.20 * (kills / maxKills);
        const booyahFactor = 0.92 + 0.16 * (booyah / maxBooyah);
        return { ...t, score: pointsPower * killFactor * booyahFactor * cpPressure };
    });
    const totalScore = scores.reduce((sum, t) => sum + (Number(t.score) || 0), 0) || 1;
    return scores.map(t => ({ ...t, probability: ((Number(t.score) || 0) / totalScore) * 100 }));
}
