...
function cffMercadoIsEmptyDestination(value) {
    ...
    if (!v) return true;
    return ['?', '-', 'SEM CLUBE', 'SEM EQUIPE', 'SEM TIME', 'FREE AGENT', 'FREEAGENT', 'SAIU', 'SAIDA', 'SAÍDA', 'DISPENSADO', 'DISPENSADOS', 'LIBERADO', 'LIBERADOS'].includes(v);
}

function cffMercadoIsValidOrigin(value) {
    const v = cffNormalizeMercadoText(value);
    if (!v) return false;
    return !['?', '-', 'SEM CLUBE', 'SEM EQUIPE', 'SEM TIME', 'FREE AGENT', 'FREEAGENT'].includes(v);
}
...
function renderMercadoAvatarStack(d, context = 'full') {
    ...
    if (!isMulti) {
        const item = visible[0] || { name: String((d && d.jogador) || ''), photo: 'silhueta.webp' };
        const cls = context === 'home' ? 'home-mercado-mini-avatar' : 'mercado-card-avatar';
        return `<img class="${cls}" src="${escapeMercadoHtml(item.photo)}" onerror="this.src='silhueta.webp'" alt="${escapeMercadoHtml(item.name)}">`;
    }

    const cls = context === 'home' ? 'home-mercado-avatar-stack' : 'mercado-avatar-stack';
    const imgs = visible.map((item, idx) => `
        <img src="${escapeMercadoHtml(item.photo)}" onerror="this.src='silhueta.webp'" alt="${escapeMercadoHtml(item.name)}" style="--i:${idx};">`).join('');
...
function resolveMercadoPlayerPhoto(playerName, sheetPhoto) {
    ...
    if (!wantedRaw) return 'silhueta.webp';
