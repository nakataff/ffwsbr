...
function cffMercadoIsOfficialExit(row) {
    ...
    return (officialStatus || exitRole) && cffMercadoIsValidOrigin(row.timeOrigem) && cffMercadoIsEmptyDestination(row.timeDestino);
}
...
function cffPlayerHasLeftTeam(playerName, teamName) {
    return !!cffGetMercadoExitInfo(playerName, teamName);
}

function cffGetTeamMercadoDepartures(teamName) {
    const byName = new Map();
    cffGetMercadoExitRows().forEach(row => {
...
function cffApplyMercadoDeparturesToCurrentPlayers() {
    ...
    db.players.forEach(p => {
        const exitInfo = cffGetMercadoExitInfo(p.jogador, p.equipe);
        if (!exitInfo) return;
        p.isEx = true;
        p.dataSaida = p.dataSaida || exitInfo.data || '';
        p.mercadoExit = true;
        p.mercadoExitTeam = exitInfo.equipeOrigem;
    });
}
