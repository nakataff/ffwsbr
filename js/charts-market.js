...
options: {
    ...
    if (typeof updateChartSummary === 'function') {
        const metricLabel = metric === 'pos_geral_dia'
            ? 'Todas as posições por dia'
            : (metric === 'pontos_dia' ? 'Todas as pontuações por dia' : 'Todos os abates por dia');
        const unit = metric === 'pos_geral_dia' ? 'º' : (metric === 'pontos_dia' ? 'PTS' : 'K');
        const summaryItems = [];

        labels.forEach((dayLabel, dayIndex) => {
            datasets.forEach(ds => {
                const value = ds.data ? ds.data[dayIndex] : null;
