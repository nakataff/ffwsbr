...
const teamSelect = document.getElementById('mercado-team-filter');
        ...
        renderMercado(); // Renderiza primeiro com o cache local (para ser rápido)
        renderHomeMercadoResumo(); // Resumo das 5 últimas transferências na home
...
function renderMercado() {
    ...
    list.innerHTML = filtered.map((d, idx) => {
        ...
        // Busca a foto no repositório (playerPhotos) caso a planilha do mercado não tenha
        let repoPhoto = null;
        if (typeof playerPhotos !== 'undefined') {
            const photoKey = Object.keys(playerPhotos).find(k => k.toLowerCase() === d.jogador.toLowerCase().trim());
            if (photoKey) repoPhoto = playerPhotos[photoKey];
        }

        const avatarSrc = d.foto || repoPhoto || 'silhueta.png';
        ...
        // ✅ VERIFICA SE O JOGADOR EXISTE NO BANCO DE DADOS (Ativo ou Histórico)
        const nomeSafe = d.jogador.trim().toLowerCase();
        const temPerfil = db.players.some(x => x.jogador.toLowerCase() === nomeSafe || checkNameMatch(x.jogador, d.jogador)) ||
                          (typeof lbffData !== 'undefined' && Object.keys(lbffData).some(name => name.toLowerCase() === nomeSafe || checkNameMatch(name, d.jogador)));

        // Se tiver perfil, cria um link com hover e o ícone de link. Se não, exibe só o texto normal.
        const nomeHtml = temPerfil
            ? `<span onclick="${_safePPAttr(d.jogador)}" title="Ver perfil do jogador" style="cursor: pointer; transition: 0.2s;" onmouseover="this.style.color='var(--accent)'" onmouseout="this.style.color='inherit'">${d.jogador} 🔗</span>`
            : d.jogador;
        return `
        <div class="mercado-card status-${d.status}" data-status="${d.status}">
            <img class="mercado-card-avatar"
                 src="${avatarSrc}"
                 onerror="this.src='silhueta.png'"
                 alt="${d.jogador}">
...
function renderHomeMercadoResumo() {
    ...
    list.innerHTML = ultimas.map(d => {
        ...
        const avatarSrc = escapeMercadoHomeHtml(d.foto || repoPhoto || 'silhueta.png');
        const jogador = escapeMercadoHomeHtml(d.jogador);
        ...
        return `
            <div class="home-mercado-mini-card" onclick="navigate('mercado')" title="Abrir Mercado da Bala">
                <img class="home-mercado-mini-avatar" src="${avatarSrc}" onerror="this.src='silhueta.png'" alt="${jogador}">
                <div class="home-mercado-mini-main">
                    <div class="home-mercado-mini-name">${jogador}${role}</div>
