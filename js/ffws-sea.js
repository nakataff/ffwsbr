...
function resolveSEATeamLogo(value) {
    ...
    const normalizedCandidates = candidates.map(normalizeSEAKey);
    for (const [logoKey, logoValue] of Object.entries(store || {})) {
        if (normalizedCandidates.includes(normalizeSEAKey(logoKey))) return logoValue;
    }
...
function formatSEAValue(header, value, tableId = '') {
    ...
    if (shouldConvertSEATeamColumn(header)) {
        ...
        return `<span class="int-team-name sea-team-cell sea-clickable-cell" data-sea-team="${escapeHtml(fullName)}" onclick="if(window.openSEATeamProfile) openSEATeamProfile(this.dataset.seaTeam)" title="Abrir perfil da equipe"><img src="${escapeHtml(logoSrc)}" onerror="this.onerror=null;this.src='escudo.webp'" class="team-logo" alt="${escapeHtml(fullName)}"><span class="sea-desktop-only">${escapeHtml(fullName)}</span><span class="sea-mobile-only">${escapeHtml(sigla)}</span></span>`;
    }
    if (shouldCleanSEAPlayerColumn(header)) {
        ...
        return `<span class="int-player-name${playerLenClass} sea-clickable-cell" data-sea-player="${escapeHtml(text)}" onclick="if(window.openSEAPlayerProfile) openSEAPlayerProfile(this.dataset.seaPlayer)" title="Abrir perfil de ${escapeHtml(playerName)}">${escapeHtml(playerName)}</span>`;
    }
