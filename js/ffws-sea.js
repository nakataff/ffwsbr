...
const SEA_GRAND_FINAL_TEAMS = [
    { team: 'Bigetron by Vitality', source: 'Dia 1', rank: '1º', pts: 98 },
    { team: 'All Gamers Global', source: 'Dia 1', rank: '2º', pts: 92 },
    { team: 'RRQ Kazu', source: 'Dia 2', rank: '1º', pts: 93 },
    { team: 'GOW', source: 'Dia 2', rank: '2º', pts: 91 },
    { team: 'Team Falcons', source: 'Dia 3', rank: '1º', pts: 99 },
    { team: 'Twisted Minds', source: 'Dia 3', rank: '2º', pts: 91 },
    { team: 'Team Flash', source: 'Last Chance', rank: '1º', pts: 293 },
    { team: 'Buriram United Esports', source: 'Last Chance', rank: '2º', pts: 249 },
    { team: 'Aurora Gaming', source: 'Last Chance', rank: '3º', pts: 240 },
    { team: 'GAMxPE', source: 'Last Chance', rank: '4º', pts: 217 },
    { team: 'Secret WAG', source: 'Last Chance', rank: '5º', pts: 217 },
    { team: 'EVOS Divine', source: 'Last Chance', rank: '6º', pts: 211 }
];
...
function getSEAPhaseTitle() {
    return currentSEAPhase === 'phase2' ? 'Etapa Eliminatória — Fase 2' : 'Etapa Eliminatória — Fase 1';
}

function getSEAPhaseSubtitle() {
    return currentSEAPhase === 'phase2'
        ? 'Fase 2: 08/05 a 17/05 • disputa em escada por pools, com vagas diretas para a Grande Final.'
...
function getSEAPhaseFormatHtml() {
    if (currentSEAPhase === 'phase2') {
        return `
            ...
            <ul>
                <li><strong>Semana 3 Dia 1 — 08/05:</strong> Pool A joga. Top 2 avança para a Grande Final; os outros seguem para o próximo dia.</li>
                <li><strong>Semana 3 Dia 2 — 09/05:</strong> 10 restantes + Pool B. Top 2 avança para a Grande Final; os outros seguem.</li>
                <li><strong>Semana 3 Dia 3 — 10/05:</strong> 10 restantes + Pool C. Top 2 avança para a Grande Final; os outros vão para a Semana 4.</li>
                <li><strong>Semana 4 — 15/05 a 17/05:</strong> 10 restantes + Pool D. Top 6 avança para a Grande Final; bottom 6 é eliminado.</li>
            </ul>`;
    }
...
function getSEAPhase2StageLabel(stage) {
    const map = {
        w3d1: 'Semana 3 Dia 1 — 08/05',
        w3d2: 'Semana 3 Dia 2 — 09/05',
        w3d3: 'Semana 3 Dia 3 — 10/05',
        w4: 'Semana 4 — 15/05 a 17/05'
    };
    ...
    if (currentSEAPhase === 'phase2') {
        return `
            <div class="sea-legend-inline">
                <span><i class="sea-mini-line sea-line-qualified"></i> Grande Final</span>
                <span><i class="sea-mini-line sea-line-next"></i> Próximo dia de jogo</span>
...
function getSEAStatusSummaryClass(status) {
    const clean = normalizeSEAKey(status);
    if (clean.includes('FINAL')) return 'sea-summary-status-final';
    if (clean.includes('ELIMIN')) return 'sea-summary-status-out';
    return 'sea-summary-status-next';
}

function renderSEAGrandFinalTableHtml() {
    return `
        <section class="sea-finals-card sea-grand-final-card">
            <div class="sea-finals-card-head">
                <div>
                    <h3>Grande Final — 12 classificados</h3>
                    <p>Times que garantiram vaga pela Fase 2 / Last Chance.</p>
                </div>
                <span class="sea-finals-count">12 times</span>
            </div>
            <div class="sea-finals-table-wrap">
                <table class="sea-finals-table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Equipe</th>
                    ...
                    <tbody>
                        ${SEA_GRAND_FINAL_TEAMS.map((item, index) => `
                            <tr>
                                <td class="sea-finals-rank">${index + 1}º</td>
                                <td>${renderSEATeamMini(item.team)}</td>
                                <td>${escapeHtml(item.source)}</td>
                                <td>${escapeHtml(item.rank)}</td>
                                <td><strong>${escapeHtml(item.pts)}</strong></td>
                            </tr>`).join('')}
                    </tbody>
                </table>
            </div>
...
function renderSEAStageResultsHtml() {
    return `
        <section class="sea-finals-card">
            <div class="sea-finals-card-head">
                <div>
                    <h3>Resultados da Fase 2</h3>
                    <p>Resumo dos quatro dias usados para definir a Grande Final.</p>
                </div>
            </div>
            <div class="sea-stage-results-grid">
                ${SEA_STAGE_RESULTS.map(stage => `
                    <article class="sea-stage-result-card">
                        <div class="sea-stage-result-head">
                            <strong>${escapeHtml(stage.title)}</strong>
                            <span>${escapeHtml(stage.subtitle)}</span>
                        </div>
                        <div class="sea-stage-result-list">
                            ${stage.rows.map((row, index) => `
                                <div class="sea-stage-result-row ${getSEAStatusSummaryClass(row.status)}">
                                    <span class="sea-stage-pos">${index + 1}º</span>
                                    ${renderSEATeamMini(row.team)}
                                    <span class="sea-stage-pts">${escapeHtml(row.pts)}</span>
                                    <span class="sea-stage-status">${escapeHtml(row.status)}</span>
                                </div>`).join('')}
                        </div>
                    </article>`).join('')}
            </div>
...
function renderSEAFinalsPanel() {
    const panel = document.getElementById('sea-finals-panel');
    if (!panel) return;
    panel.innerHTML = renderSEAGrandFinalTableHtml() + renderSEAStageResultsHtml();
}
...
function ensureSEAPhasePanel() {
    ...
    panel.innerHTML = `
            ...
            <div class="sea-phase-card-head">
                ...
                <label id="sea-phase2-stage-wrap" class="sea-phase-stage-wrap" style="display:none;">
                    ...
                    <select id="sea-phase2-stage" onchange="setSEAPhase2Stage(this.value)">
                        <option value="w3d1">Semana 3 Dia 1</option>
                        <option value="w3d2">Semana 3 Dia 2</option>
                        <option value="w3d3">Semana 3 Dia 3</option>
        ...
        <div id="sea-finals-panel" class="sea-finals-panel"></div>`;
...
function getSEAClassificationStatus(rank) {
    ...
    if (currentSEAPhase2Stage === 'w4') {
        if (rank <= 6) return { label: 'Grande Final', className: 'sea-status-qualified', rowClass: 'sea-row-qualified' };
        return { label: 'Eliminado', className: 'sea-status-eliminated', rowClass: 'sea-row-eliminated' };
    }

    if (rank <= 2) return { label: 'Grande Final', className: 'sea-status-qualified', rowClass: 'sea-row-qualified' };
    if (currentSEAPhase2Stage === 'w3d3') return { label: 'Avança para Semana 4', className: 'sea-status-next', rowClass: 'sea-row-next' };
    return { label: 'Próximo dia de jogo', className: 'sea-status-next', rowClass: 'sea-row-next' };
}
