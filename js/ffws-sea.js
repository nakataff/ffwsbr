...
function getSEALegendHtml() {
    if (currentSEAPhase === 'phase2') {
        return `
            <div class="sea-legend-item sea-qualified"><span class="sea-legend-icon">⌃</span> Classificado para <strong>Grand Finals</strong></div>
            <div class="sea-legend-item sea-next"><span class="sea-legend-icon">━</span> Avança para o <strong>próximo matchday</strong></div>
            <div class="sea-legend-item sea-eliminated"><span class="sea-legend-icon">☠</span> Eliminado <em>(Week 4 only)</em></div>`;
    }

    return `
        <div class="sea-legend-item sea-pool-a"><span class="sea-pool-badge">A</span> Pool A — <strong>Top 12</strong></div>
        <div class="sea-legend-item sea-pool-b"><span class="sea-pool-badge">B</span> Pool B — <strong>13º–14º</strong></div>
        <div class="sea-legend-item sea-pool-c"><span class="sea-pool-badge">C</span> Pool C — <strong>15º–16º</strong></div>
        <div class="sea-legend-item sea-pool-d"><span class="sea-pool-badge">D</span> Pool D — <strong>17º–18º</strong></div>`;
}
...
function renderSEATable(tableId, parsed, emptyMessage) {
    ...
    const statusHeader = isClassificationTable ? '<th class="sea-col-status">Destino</th>' : '';
    ...
    }).join('')}${statusHeader}</tr>`;

    tbody.innerHTML = normalizedParsed.rows.map((row, index) => {
        const rank = index + 1;
        const status = isClassificationTable ? getSEAClassificationStatus(rank) : { rowClass: '' };
        const rankCell = hasDynamicRank ? `<td class="int-rank-cell">${rank}º</td>` : '';
        ...
        const statusCell = isClassificationTable ? renderSEAStatusCell(rank) : '';
        return `<tr class="${status.rowClass}">${rankCell}${cells}${statusCell}</tr>`;
    }).join('');
