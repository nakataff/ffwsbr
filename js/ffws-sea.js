...
function convertJsonAbatesRowsToParsed(rows) {
    return {
        headers: ['JOGADOR', 'FUNÇÃO', 'ABATES', 'DANO', 'ASSISTENCIAS', 'QUEDAS', 'MVP'],
        rows: (rows || []).map(row => ({
            JOGADOR: String(row.jogador || '').trim(),
...
function convertSEAAbatesToAverage(parsed) {
    ...
    return {
        headers: ['JOGADOR', 'FUNÇÃO', 'K/Q', 'DANO/Q', 'AST/Q', 'QUEDAS', 'MVP/Q'],
...
function getSEAAbatesDisplayParsed() {
    ...
    return sortSEARowsByAbatesDesc(stripSEAPositionColumns(parsed));
}
