...
function cffCalculateDailyNota(totalKills, totalDano, totalMvps) {
...
function cffCollectNotaRows() {
    ...
    return Object.values(aggregated).map(p => ({
        ...p,
        nota: (dayFilter !== 'all' && selectedDrops.length === 0) ? Number(cffCalculateDailyNota(p.kills, p.dano, p.mvp).toFixed(1)) : (p.quedas ? Number((p.somaNota / p.quedas).toFixed(1)) : 0),
...
function renderNotasCFFStatsPaged() {
    ...
    const dailyRatings = Object.values(playerDailyAggregator).map(day => {
        ...
        const notaDia = cffCalculateDailyNota(totalKills, totalDano, totalMvps);
