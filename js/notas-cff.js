...
function cffMoveRecordsPage(direction) {
    ...
    cffRecordsPage = (cffRecordsPage + direction + cards.length) % cards.length;
    cffUpdateRecordsPager();
}
...
function cffUpdateRecordsPager() {
    ...
    if (!cards.length) return;
    cards.forEach((card, index) => { card.style.display = index === cffRecordsPage ? 'block' : 'none'; });
    if (label) label.textContent = `${cffRecordsPage + 1} / ${cards.length}`;
}
...
function toggleNotasCFFSort() {
    ...
    if (btn) btn.textContent = cffNotasSortDir === 'desc' ? '↓' : '↑';
    renderNotasCFFRanking();
}
