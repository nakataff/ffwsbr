...
if (liveList) {
  ...
  function injectCss() {
    ...
    style.textContent = `
      ...
      .cff-standings-dialog{width:min(1080px,100%);max-height:94vh;overflow:auto;background:#09111d;color:#edf7ff;border:1px solid rgba(0,200,255,.23);border-radius:18px;box-shadow:0 28px 80px rgba(0,0,0,.5)}
      ...
      .cff-standings-body{padding:18px 20px 22px}.cff-standings-toggle{display:flex;align-items:center;gap:9px;padding:11px 13px;margin-bottom:9px;border-radius:10px;border:1px solid rgba(0,200,255,.18);background:rgba(0,200,255,.055);font-weight:850}.cff-standings-toggle input{width:18px;height:18px}
      .cff-standings-fields{display:grid;grid-template-columns:minmax(0,1fr) 220px 220px;gap:12px;margin:13px 0 12px}.cff-standings-field{display:grid;gap:6px}.cff-standings-field label{font-size:.71rem;color:#86a1bc;font-weight:900;text-transform:uppercase}.cff-standings-field input,.cff-standings-field textarea{box-sizing:border-box;width:100%;border:1px solid rgba(255,255,255,.12);border-radius:10px;background:#07101a;color:#fff;padding:11px 12px;font:inherit}.cff-standings-field textarea{min-height:255px;resize:vertical;font-family:Consolas,monospace;font-size:.78rem;line-height:1.4}
      ...
      .cff-standings-team-wrap{border:1px solid rgba(255,255,255,.09);border-radius:12px;overflow:hidden;background:#07101a}.cff-standings-team-head,.cff-standings-team-row{display:grid;grid-template-columns:50px minmax(100px,.65fr) minmax(160px,1.35fr) 80px 72px 62px;gap:8px;align-items:center;padding:9px 11px}.cff-standings-team-head{background:#101a28;color:#7894b0;font-size:.66rem;font-weight:950;text-transform:uppercase}.cff-standings-team-row{border-top:1px solid rgba(255,255,255,.06)}
      .cff-standings-team-row.is-cp{background:rgba(0,132,255,.12);border-top-color:rgba(65,166,255,.26)}.cff-standings-team-row.is-cp .cff-rank,.cff-standings-team-row.is-cp .cff-points{color:#7cc8ff}
      .cff-rank{font-weight:950;color:#839db7}.cff-code{font-family:Consolas,monospace;color:#75ddff;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.cff-name{min-width:0;border:1px solid rgba(255,255,255,.11);border-radius:8px;background:#0c1623;color:#fff;padding:8px 9px;font:inherit;font-size:.82rem;font-weight:850}.cff-points,.cff-kills{text-align:right;font-weight:950}.cff-kills{color:#87a0b8}.cff-cp{font-size:.65rem;font-weight:950;color:#7995af;text-align:center}.cff-cp.on{color:#62bdff}.cff-empty{padding:24px;text-align:center;color:#7892ac;font-size:.8rem}
  ...
  function modal() {
    ...
    box.innerHTML = `
      <section class="cff-standings-dialog" role="dialog" aria-modal="true">
        ...
        <div class="cff-standings-body">
          <label class="cff-standings-toggle"><input id="cff-standings-enabled" type="checkbox"> Exibir esta tabela na home</label>
          <label class="cff-standings-toggle"><input id="cff-standings-keep" type="checkbox"> Manter a tabela visível entre os dias do campeonato</label>
          <div class="cff-standings-fields">
            ...
            <div class="cff-standings-field"><label>Formato</label><input value="2 colunas • 6 times" readonly></div>
            <div class="cff-standings-field"><label>Champion Point</label><input id="cff-standings-matchpoint" value="Não detectado" readonly></div>
          </div>
          ...
          <div class="cff-standings-toolbar"><button id="cff-standings-parse" class="admin-btn admin-btn-ghost" type="button">Ler código</button><span id="cff-standings-status" class="cff-standings-status">Cole o código para detectar times, quedas, pontuação e Champion Point.</span></div>
    ...
    let timer = 0;
    $('#cff-standings-code', box).addEventListener('input', () => {
      clearTimeout(timer);
      timer = setTimeout(() => parseEditor(false), 300);
    });
  ...
  function parseLiquipedia(raw) {
    ...
    const mpMatch = source.match(/\|\s*matchpoint\s*=\s*(\d+(?:[.,]\d+)?)/i);
    const matchPoint = toNum(mpMatch?.[1], 0);
    const placement = {};
    ...
    let m;
    while ((m = opponentRx.exec(source))) starts.push({ opponent:Number(m[1]), code:clean(m[2]), index:m.index });
    ...
    const teams = starts.slice(0,12).map((item, index) => {
      ...
      while ((score = scoreRx.exec(segment))) {
        ...
        const placementPoints = toNum(placement[pos]);
        const killPoints = ab * pKill;
        const mapPoints = placementPoints + killPoints;
        points += mapPoints;
        kills += ab;
        mapRows.push({ map, placement:pos, kills:ab, placementPoints, killPoints, points:mapPoints });
      ...
      mapRows.forEach(row => {
        ...
        if (!reachedAt && matchPoint > 0 && cumulative >= matchPoint) reachedAt = row.map;
        row.championPointReached = Boolean(matchPoint > 0 && cumulative >= matchPoint);
      });
      return {
        ...
        points,
        kills,
        mapsPlayed:mapRows.length,
        mapRows,
        championPointActive:Boolean(matchPoint > 0 && points >= matchPoint),
        championPointReachedAt:reachedAt
      };
    }).filter(t => t.code);

    teams.sort((a,b) => (b.points-a.points) || (b.kills-a.kills) || (a.opponent-b.opponent));
    teams.forEach((t,i) => { t.rank = i + 1; });
    return { ok:true, pKill, matchPoint, placement, completedMaps, totalMaps, suggestedTitle, teams };
  }

  function setStatus(text, kind='') {
    const el = $('#cff-standings-status', modal());
    if (el.textContent !== text) el.textContent = text;
    const cls = `cff-standings-status${kind ? ` ${kind}` : ''}`;
  ...
  function parseEditor(explicit=false) {
    ...
    if (!result.ok) {
      parsed = null;
      $('#cff-standings-matchpoint', box).value = 'Não detectado';
      renderTeams();
      setStatus(result.error, explicit ? 'error' : 'warn');
      return;
    }
    parsed = result;
    $('#cff-standings-matchpoint', box).value = result.matchPoint > 0 ? `${result.matchPoint} pontos` : 'Não detectado';
    ...
    const cp = result.matchPoint > 0 ? ` • CP ${result.matchPoint}` : '';
    setStatus(`${result.teams.length} times detectados • ${maps} • ${result.pKill} pt por abate${cp}`, result.teams.length === 12 ? 'ok' : 'warn');
  }

  function renderTeams() {
    const wrap = $('#cff-standings-team-wrap', modal());
    if (!parsed?.teams?.length) {
      wrap.innerHTML = '<div class="cff-empty">Nenhum time detectado ainda.</div>';
      return;
    }
    wrap.replaceChildren();
    const head = document.createElement('div');
    head.className = 'cff-standings-team-head';
    head.innerHTML = '<span>#</span><span>Código</span><span>Nome exibido</span><span>CP</span><span style="text-align:right">PTS</span><span style="text-align:right">AB</span>';
    wrap.appendChild(head);
    parsed.teams.forEach(team => {
      const row = document.createElement('div');
      row.className = `cff-standings-team-row${team.championPointActive ? ' is-cp' : ''}`;
      ...
      const cp = document.createElement('span'); cp.className = `cff-cp${team.championPointActive ? ' on' : ''}`; cp.textContent = team.championPointActive ? 'ATIVO' : '—';
      ...
      row.append(rank, code, name, cp, pts, kills);
  ...
  async function open(id) {
    ...
    try {
      ...
      $('#cff-standings-keep', box).checked = Boolean(live.standingsKeepVisible);
      $('#cff-standings-code', box).value = String(st.rawCode || '');
      const title = $('#cff-standings-label', box);
      title.value = String(st.title || '');
      ...
      if (st.rawCode) parseEditor(false);
      else {
        parsed = null;
        $('#cff-standings-matchpoint', box).value = 'Não detectado';
        renderTeams();
      ...
      setStatus(error?.message || 'Não foi possível carregar.', 'error');
    }
  }
  ...
  async function save() {
    ...
    const payload = {
      ...
      teams: parsed.teams.slice(0,12).map(t => ({
        code:t.code,
        name:clean(names.get(t.key), t.code.toUpperCase()),
        points:t.points,
        kills:t.kills,
        maps:t.mapsPlayed,
        rank:t.rank,
        championPointActive:t.championPointActive,
        championPointReachedAt:t.championPointReachedAt,
        mapRows:t.mapRows
      })),
      updatedAt: Date.now()
    };
    const keepVisible = $('#cff-standings-keep', box).checked;
    const button = $('#cff-standings-save', box);
    button.disabled = true;
    button.textContent = 'Salvando...';
    try {
      await update(ref(database, `adminLives/${currentId}`), { standings: payload, standingsKeepVisible: keepVisible });
      states.set(currentId, { configured:true, enabled:payload.enabled });
      scheduleDecorate();
      setStatus(payload.enabled ? 'Tabela salva e ativada.' : 'Tabela salva, mas desativada.', 'ok');
      setTimeout(close, 500);
    } catch (error) {
      console.error(error);
      setStatus('Não foi possível salvar. Confira sua sessão administrativa.', 'error');
    } finally {
      button.disabled = false;
      button.textContent = 'Salvar tabela';
    }
  }
  ...
  async function removeTable() {
    ...
    try {
      await update(ref(database, `adminLives/${currentId}`), { standings:null, standingsKeepVisible:false });
      states.set(currentId,{configured:false,enabled:false});
      scheduleDecorate();
      close();
    } catch (error) {
      console.error(error);
  ...
  async function refreshStates() {
    try {
      ...
      const data = snap.val() || {};
      states = new Map(Object.entries(data).map(([id, live]) => [String(id), {
  ...
  function decorate() {
    $$('[data-edit-live]', liveList).forEach(edit => {
      const id = String(edit.dataset.editLive || '');
      if (!id) return;
      const parent = edit.parentElement;
      if (!parent) return;
      let button = [...parent.querySelectorAll('[data-standings-live]')].find(b => b.dataset.standingsLive === id);
      if (!button) {
        button = document.createElement('button');
        button.type = 'button';
        button.className = 'admin-btn admin-btn-ghost';
        button.dataset.standingsLive = id;
        edit.insertAdjacentElement('afterend', button);
      }
      const st = states.get(id);
      const label = st?.enabled ? 'Tabela ✓' : (st?.configured ? 'Tabela •' : 'Tabela');
      if (button.textContent !== label) button.textContent = label;
      const shouldEnable = Boolean(st?.enabled);
      if (button.classList.contains('is-enabled') !== shouldEnable) button.classList.toggle('is-enabled', shouldEnable);
      const title = st?.enabled ? 'Tabela ativa na home' : (st?.configured ? 'Tabela configurada, mas desativada' : 'Configurar tabela pela Liquipedia');
  ...
  liveList.addEventListener('click', e => {
    const button = e.target.closest('[data-standings-live]');
    if (!button) return;
    e.preventDefault();
    open(button.dataset.standingsLive);
  });

  const observer = new MutationObserver((mutations) => {
    if (mutations.some(m => [...m.addedNodes].some(node => node.nodeType === 1 && (node.matches?.('[data-edit-live]') || node.querySelector?.('[data-edit-live]'))))) scheduleDecorate();
  });
