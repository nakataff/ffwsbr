...
if (liveList) {
  ...
  function injectCss() {
    ...
    style.textContent = `
      ...
      .cff-standings-modal{position:fixed;inset:0;z-index:99999;display:grid;place-items:center;padding:18px;background:rgba(2,7,14,.82);backdrop-filter:blur(7px)}
  ...
  function renderTeams() {
    ...
    if (!parsed?.teams?.length) {
      if (wrap.innerHTML !== '<div class="cff-empty">Nenhum time detectado ainda.</div>') wrap.innerHTML = '<div class="cff-empty">Nenhum time detectado ainda.</div>';
      return;
    }
  ...
  async function removeTable() {
    ...
    try {
      await update(ref(database, `adminLives/${currentId}`), { standings:null });
  ...
  const observer = new MutationObserver((mutations) => {
    if (mutations.some(m => [...m.addedNodes].some(node => node.nodeType === 1 && (node.matches?.('[data-edit-live]') || node.querySelector?.('[data-edit-live]'))))) {
      scheduleDecorate();
    }
  });
