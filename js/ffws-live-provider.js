(()=>{
  ...
  window.fetch=async(...args)=>{
    ...
    if(!response.ok||!/ffws-br-2026-s2\/(?:stages|player-stats)\.json/i.test(url))return response;
    try{const data=await live();if(!data)return response;const payload=await response.clone().json();return responseWith(response,url.includes('stages.json')?mergeStages(payload,data):mergeStats(payload,data))}catch(error){console.warn('[CFF] Dados ao vivo indisponíveis:',error);return response}
  };
})();
