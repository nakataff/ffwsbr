(function(){
  ...
  const VERSION='20260909-news-comments-v3';
  ...
  function randomId(){const bytes=new Uint8Array(12);crypto.getRandomValues(bytes);return 'c_'+Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('')}
  function clientId(){
  ...
  function injectStyles(){
    ...
    style.textContent='.cff-comments{margin:28px 0 0;border:1px solid rgba(0,200,255,.18);border-radius:18px;background:rgba(9,17,31,.86);overflow:hidden;color:#f4f9ff}.cff-comments-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:18px 20px;border-bottom:1px solid rgba(0,200,255,.14);background:linear-gradient(135deg,rgba(0,200,255,.09),rgba(255,255,255,.018))}.cff-comments-kicker{color:#00c8ff;font-size:.72rem;font-weight:1000;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:3px}.cff-comments-title{margin:0;color:#fff;font-size:1.12rem;line-height:1.1;font-weight:1000;text-transform:uppercase}.cff-comments-count{display:inline-flex;align-items:center;justify-content:center;min-width:42px;height:32px;padding:0 10px;border:1px solid rgba(0,200,255,.25);border-radius:999px;background:rgba(0,200,255,.07);color:#7edfff;font-size:.8rem;font-weight:1000}.cff-comments-body{padding:18px 20px 20px}.cff-comment-form{display:grid;grid-template-columns:minmax(160px,.72fr) minmax(0,1.7fr) auto;gap:10px;align-items:end;padding:14px;border:1px solid rgba(255,255,255,.08);border-radius:14px;background:rgba(255,255,255,.025)}.cff-comment-field{display:grid;gap:6px;min-width:0}.cff-comment-field label{color:#9bb9d8;font-size:.69rem;font-weight:950;letter-spacing:.65px;text-transform:uppercase}.cff-comment-input,.cff-comment-text{width:100%;min-width:0;border:1px solid #253650;border-radius:11px;background:#08111f;color:#fff;outline:none;padding:10px 11px;font:inherit}.cff-comment-text{height:44px;min-height:44px;max-height:120px;resize:vertical;line-height:1.35}.cff-comment-input:focus,.cff-comment-text:focus{border-color:#00c8ff;box-shadow:0 0 0 3px rgba(0,200,255,.1)}.cff-comment-submit{min-height:44px;border:0;border-radius:11px;padding:0 16px;background:#00c8ff;color:#00131b;font-weight:1000;white-space:nowrap;cursor:pointer}.cff-comment-submit:disabled{opacity:.48;cursor:not-allowed}.cff-comment-note{grid-column:1/-1;min-height:17px;color:#7897b8;font-size:.72rem;line-height:1.4}.cff-comment-note.is-error{color:#ff8f9b}.cff-comment-note.is-success{color:#72e6ad}.cff-comments-list{display:grid;gap:10px;margin-top:15px}.cff-comment{padding:13px 14px;border:1px solid rgba(255,255,255,.075);border-radius:13px;background:#0b1422}.cff-comment-top{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:6px}.cff-comment-author{min-width:0;color:#fff;font-size:.9rem;font-weight:1000;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cff-comment-time{flex:0 0 auto;color:#6f8dac;font-size:.66rem;font-weight:800}.cff-comment-text-body{color:#dcecff;font-size:.91rem;line-height:1.5;white-space:pre-wrap;overflow-wrap:anywhere}.cff-comments-empty,.cff-comments-loading{padding:18px 8px 5px;color:#7897b8;text-align:center;font-size:.82rem;font-weight:800}@media(max-width:760px){.cff-comments{margin-top:20px;border-radius:15px}.cff-comments-head{padding:14px 15px}.cff-comments-body{padding:14px}.cff-comment-form{grid-template-columns:1fr;padding:11px;gap:9px}.cff-comment-note{grid-column:1}.cff-comment-submit{width:100%}.cff-comment-text{height:76px;min-height:76px}.cff-comment-top{align-items:flex-start;flex-direction:column;gap:2px}}';
  ...
  function build(){
    ...
    section.innerHTML='<div class="cff-comments-head"><div><div class="cff-comments-kicker">Comunidade</div><h2 class="cff-comments-title">Comentários</h2></div><span class="cff-comments-count" id="cff-comments-count">0</span></div><div class="cff-comments-body"><form class="cff-comment-form" id="cff-comment-form"><div class="cff-comment-field"><label for="cff-comment-name">Nome ou @</label><input class="cff-comment-input" id="cff-comment-name" maxlength="36" autocomplete="nickname" placeholder="@seuinstagram" required></div><div class="cff-comment-field"><label for="cff-comment-text">Seu comentário</label><textarea class="cff-comment-text" id="cff-comment-text" maxlength="600" placeholder="Comente sobre a notícia..." required></textarea></div><button class="cff-comment-submit" id="cff-comment-submit" type="submit">COMENTAR</button><div class="cff-comment-note" id="cff-comment-note">Comentários são públicos. Máximo de 600 caracteres.</div></form><div class="cff-comments-list" id="cff-comments-list"><div class="cff-comments-loading">Carregando comentários...</div></div></div>';
    return section;
  }
  ...
  if(!mount(section)){
    let attempts=0;
    const timer=setInterval(()=>{if(mount(section)||++attempts>50) clearInterval(timer)},200);
  }
